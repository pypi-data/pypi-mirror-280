...
class Payment(BaseModel):
    ...
    def event_version_must_be_three(cls, v):
        if float(v) != 3.0:
            raise ValidationError('event must have 3.0 as version')
        return v.title()
    ...
    def is_payment_creation_needed(self):
        if self.status != PaymentStatus.STATUS_CREATION_PENDING:
            logger.info(f'Event {self.id} from application {self.remote_collection.id}: Payment creation not required')
            return False
        return True
