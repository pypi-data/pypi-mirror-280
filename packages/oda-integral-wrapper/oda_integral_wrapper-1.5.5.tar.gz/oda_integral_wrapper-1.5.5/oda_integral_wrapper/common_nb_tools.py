...
def extract_parameters_from_cells(logger, nb_inp_fname='NoName'):
    parameters = {}
    get_nbname(logger,parameters,nb_inp_fname=nb_inp_fname)
    from nb2workflow.nbadapter import NotebookAdapter
    this_nb = NotebookAdapter(parameters['origin_notebook'].split('/')[-1])
    complex_parameters = this_nb.extract_parameters()

    for _,par in complex_parameters.items():
        if par is not None:
            if par['value'] is not None and par['name'] != 'parameters':
