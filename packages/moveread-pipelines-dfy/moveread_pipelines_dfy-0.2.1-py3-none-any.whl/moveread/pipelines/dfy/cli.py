...
def main():
  ...
  parser.add_argument('--logpipe', required=True, help='Output log file path')
  ...
  import os
  from dslog import loggers, formatters
  ...
  Qs = pipe.connect(Qout, get_queue)
  storage = local_storage(base_path)
  
  artifs = pipe.run(Qs)(logger=logger, token=args.token, tfserving=tfparams, **storage)

  artifs.api.mount('/queues', http.mount(pipe, Qout, get_queue))
  artifs.api.mount('/blobs', kv.rest.api(storage['blobs']))
  artifs.api.mount('/logs', kv.rest.api(FilesystemKV(args.logs)))
