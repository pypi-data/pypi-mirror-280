...
class InputVal(Wrapped[Input, Any, InputValidation.Input, InputValidation.Output, InputValidation.Queues, InputValidation.Artifacts]):
  def __init__(self):
  ...
  def post(self, inp: Input, out: InputValidation.Output):
    ...
    title = inp.title if gid == inp.gameId else f'Changed to: {gid["group"]}/{gid["round"]}/{gid["board"]} from'
...
class Preprocessing(Wrapped[PreprocessInput, Preprocessed, GamePreprocess.Input, GamePreprocess.Output, GamePreprocess.Queues, GamePreprocess.Artifacts]):
  def __init__(self):
...
class Prediction(Wrapped[Preprocessed, Predicted, ocr.Input, ocr.Preds, ocr.OCRPredict.Queues, ocr.OCRPredict.Artifacts]):
  def __init__(self):
...
class Correction(Wrapped[Predicted, Any, gamecorr.Input, gamecorr.Output, gamecorr.GameCorrection.Queues, gamecorr.GameCorrection.Artifacts]):
  def __init__(self):
...
class ArtifactsProto(Protocol):
  def __call__(self, *, logger: Logger, **params: Unpack[Params]) -> Artifacts:
    ...

class DFYPipeline(Workflow[Input, Output, Pipelines, Queues, ArtifactsProto]): # type: ignore
  Queues = Queues
  ...
  def run(self, queues: Queues) -> ArtifactsProto:
    def bound(*, logger: Logger, **params: Unpack[Params]):
      inpval_api = self.pipelines['inputval'].run(queues['inputval'])(logger=logger.prefix('[INPUT VAL]'), images_path=params.get('images_path'))
      gamepre = self.pipelines['preprocess'].run(queues['preprocess'])(logger=logger.prefix('[PREPROCESS]'), **kw.take(GamepreParams, params))
      ocr_coro = self.pipelines['predict'].run(queues['predict'])(logger=logger.prefix('[OCR]'), blobs=params['blobs'], **params.get('tfserving', {}))
      corr_api = self.pipelines['correct'].run(queues['correct'])(logger=logger.prefix('[GAME CORRECT]'), images_path=params.get('images_path'))
