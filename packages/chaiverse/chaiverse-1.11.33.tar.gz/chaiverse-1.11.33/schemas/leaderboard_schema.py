...
def _add_ranking_column(df):
    ...
    empty_indexes = df[df['propriety_score'].isnull() | df['celo_rating'].isnull()].index
    ...
    df['ranking'] = (elo_rank + alignment_rank) / 2
    # TODO: Remove this later. This is for understanding the ranking calculation and diagnosing issues
    df['elo_norm'] = elo_norm
    df['alignment_norm'] = alignment_norm

    return df

def _aggregate_max_columns(df):
    for column in ["celo_rating", "win_ratio", "safety_score"]:
