...
def _service(env, command):
    """List existing workspaces for beacons"""
    ...
    service_unit = f"""
[Unit]
Description=beacons Supervisor Service
...
def service(env, command):
    """Install a reset service beacons"""
...
def autocomplete(env):
    ...
    subprocess.run(
        [
            "_BEACONS_COMPLETE=bash_source beacons > ~/.beacons-complete.bash"
        ],
        shell=True,
    )
    ...
    print("source ~/.beacons-complete.bash")
