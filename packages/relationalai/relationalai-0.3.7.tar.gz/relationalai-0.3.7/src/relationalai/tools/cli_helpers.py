...
from relationalai.tools.constants import PROFILE # noqa: F401
...
def get_color_by_state(state:str) -> str:
    if state and isinstance(state, str):
        match state.lower():
            case "aborted" | "quarantined":
                return "red"
            case "completed":
                return "white"
            case "running" | "cancelling" | "syncing" | "pending" | "processing":
                return "bold yellow"
            case "suspended":
                return "dim"
            case _:
...
def format_value(value) -> dict:
    match value:
        ...
        case timedelta():
            return humanized_duration(value.total_seconds() * 1000)
        case _:
            return value
...
def show_dictionary_table(dict, format_fn:callable=None):
