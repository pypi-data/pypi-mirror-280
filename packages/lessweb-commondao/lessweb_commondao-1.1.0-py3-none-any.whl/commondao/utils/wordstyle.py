...
from commondao.utils.common import captitalize
import re
...
class WordSeed(NamedTuple):
    ...
    def __str__(self) -> str:
        if not self.tokens:
            ...
            return self.tokens[0] + ''.join(token.capitalize()
    ...
    def of(cls, word: str) -> 'WordSeed':
        if '_' in word:
            if re.match(r'^[a-z0-9_]+$', word):
                ...
                return WordSeed(WordStyle.upper_snake,
                                [seg.lower() for seg in word.split('_')])
        elif '-' in word:
            if re.match(r'^[a-z0-9-]+$', word):
                return WordSeed(WordStyle.lower_minus, word.split('-'))
            elif re.match(r'^[A-Z0-9-]+$', word):
                return WordSeed(WordStyle.upper_minus,
                                [seg.lower() for seg in word.split('-')])
        elif re.match(r'^[A-Z][A-Za-z0-9]+$', word):
            return WordSeed(
                WordStyle.upper_camel,
                [seg.lower() for seg in re.findall(r'[A-Z][a-z0-9]*', word)])
        elif re.match(r'^[a-z][A-Za-z0-9]+$', word):
            return WordSeed(WordStyle.lower_camel, [
                seg.lower()
                for seg in re.findall(r'[A-Z][a-z0-9]*', captitalize(word))
            ])
...
def replace_dict(oldword: str, newword: str):
    ...
    return {
        str(WordSeed(word_style, old_word_seed.tokens)):
        str(WordSeed(word_style, new_word_seed.tokens))
        for word_style in [
            WordStyle.upper_snake,
            WordStyle.lower_snake,
            WordStyle.upper_minus,
            WordStyle.lower_minus,
            WordStyle.upper_camel,
            WordStyle.lower_camel,
        ]
    }
