...
# These imports are used to generate the schema
...
def schema(fmt):
    ...
    concrete_classes = [
        klass for klass in get_all_subclasses(Node) if not klass.__subclasses__()
    ]
    # sort classes based on the ORDER attribute in reverse order
    concrete_classes = sorted(concrete_classes, key=lambda x: x.ORDER)

    if fmt == "markdown":
        display_schema_markdown(*concrete_classes)
    elif fmt == "html":
        display_schema_html(*concrete_classes)


def display_schema_markdown(*klasses):
    stream = io.StringIO()
    stream.write("# Storm DSL Schema - Version {}\n\n".format(__schema_version__))
