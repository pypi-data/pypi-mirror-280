...
def func_sig(f):
    ...
    for param in sig.parameters.values():
        if param.kind is inspect.Parameter.POSITIONAL_OR_KEYWORD:
            ...
            raise XlwingsError("_tdxlwings does not support UDFs with keyword arguments")
...
async def custom_functions_call(data, module, sio=None):
    ...
    if data["version"] != __version__:
        raise XlwingsError(
            "_tdxlwings version mismatch: please restart Excel or "
    ...
    if inspect.isasyncgenfunction(func):
        ...
        async def task():
            try:
                async for result in func(*args):
                    ...
                    await sio.emit(
                        f"_tdxlwings:set-result-{task_key}",
                        {"result": result},
                    )
        ...
        if task_key not in background_tasks:
            mytask = asyncio.create_task(task(), name=f"_tdxlwings-{task_key}")
...
def custom_functions_code(
    module, custom_functions_call_path="/_tdxlwings/custom-functions-call"
):
...
async def sio_disconnect(sid):
    ...
    active_tasks = [
        ...
        if task.get_name().startswith("_tdxlwings")
    ]
    logger.info(f"Active _tdxlwings tasks:" f"{active_tasks}")
