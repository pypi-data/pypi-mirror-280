  ...
  if (socket !== null) {
    ...
    socket.on("connect", () => {
      ...
      for (let body of bodies) {
        socket.emit("_tdxlwings:function-call", body);
      }
    });
  }
...
async function base() {
  ...
  if (isStreaming) {
    if (socket === null) {
      console.error(
        "To enable streaming functions, you need to load the socket.io js client before _tdxlwings.min.js and custom-functions-code"
      );
      return;
    }
