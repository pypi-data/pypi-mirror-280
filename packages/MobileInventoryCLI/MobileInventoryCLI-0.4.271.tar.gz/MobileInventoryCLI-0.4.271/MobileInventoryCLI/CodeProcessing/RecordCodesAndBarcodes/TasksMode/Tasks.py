...
class TasksMode:
    ...
    def NewEntrySchematic(self):
        ...
        def mkT(text,self):
                return str(text)
        section=Prompt.__init2__(None,func=mkT,ptext="Section Name, if any?",helpText=" the h2 header of the schematic",data=self)
        if section in [None,]:
            return
        while True:
            code=''
...
mode was used to create the entry:
