...
from fastapix.common.pydantic import model_validate, model_dump, model_fields, ModelField
from fastapix.crud._models import create_schema_create, create_schema_read, create_schema_update, FOREIGN_FIELD_NAME
...
class SQLAlchemyCrud:
    ...
    def __init__(
            ...
            foreign_models: Optional[List[Type[SQLModel]]] = None,
    ):
        self.Model = model
        ...
        self.ReadModel: Type[BaseModel] = create_schema_read(model, foreign_models)
        self.UpdateModel: Type[BaseModel] = create_schema_update(model)

    def select(self, foreign: List[Type[SQLModel]]) -> Select:
        if foreign:
    ...
    def table_to_pyobj(self, obj: Union[TableModel, Row], foreign: list = None) -> BaseModel:
        if isinstance(obj, Row):
            obj, *f = obj
            if foreign:
                foreign = {k.__tablename__: v for k, v in zip(foreign, f)}
                obj = obj.dict()
                obj.update({FOREIGN_FIELD_NAME: foreign})
