...
def _build_sawtooth_aggregation_subtree(
    ...
    Only applicable to stream fvs that use compaction."""
    sawtooth_tile_sizes = fdw.get_batch_sawtooth_tile_sizes()
    assert len(sawtooth_tile_sizes) > 0, "Not supported for feature views that do not use sawtooth aggregations."
    sawtooth_agg_data = compaction_utils.SawtoothAggregationData(batch_sawtooth_tile_sizes=sawtooth_tile_sizes)
    ...
    partial_agg_node = AddAnchorTimeColumnsForSawtoothIntervalsNode(
        dialect=dialect,
        compute_mode=compute_mode,
        input_node=partial_agg_node,
        timestamp_field=anchor_time(),
        anchor_time_column_map=sawtooth_agg_data.get_anchor_time_to_timedelta_map(use_zero_timedelta=True),
        data_delay_seconds=fdw.online_store_data_delay_seconds,
        feature_store_format_version=fdw.get_feature_store_format_version,
    ).as_ref()
    partial_agg_column_to_bool_map = {column: True for column in sawtooth_agg_data.get_identifier_partition_columns()}
    partial_agg_node = AddBooleanPartitionColumnsNode(
        dialect=dialect,
        compute_mode=compute_mode,
        input_node=partial_agg_node,
        column_to_bool_map=partial_agg_column_to_bool_map,
    ).as_ref()
    ...
    batch_spine_node = AddAnchorTimeColumnsForSawtoothIntervalsNode(
        ...
        timestamp_field=fdw.timestamp_key,
        anchor_time_column_map=sawtooth_agg_data.get_anchor_time_to_timedelta_map(),
        data_delay_seconds=fdw.online_store_data_delay_seconds,
        feature_store_format_version=fdw.get_feature_store_format_version,
    ).as_ref()
    batch_spine_node = ExplodeEventsAndSelectDistinctNode(
        ...
        explode_columns=sawtooth_agg_data.get_anchor_time_columns(),
        explode_columns_to_boolean_columns=sawtooth_agg_data.get_anchor_time_to_partition_columns_map(),
    ).as_ref()
    ...
    # Placeholder node for stream part with left and right as dummy values.
    # TODO(samantha): implement this.
    stream_part_subtree = UnionNode(
        dialect=dialect, compute_mode=compute_mode, left=spine_node, right=partial_agg_node
    ).as_ref()

    # Build final aggregation values. This is a placeholder node for now.
    sawtooth_agg_node = AsofJoinSawtoothAggNode(
        dialect=dialect,
        compute_mode=compute_mode,
        batch_input_node=batch_part_subtree,
        stream_input_node=stream_part_subtree,
    ).as_ref()
