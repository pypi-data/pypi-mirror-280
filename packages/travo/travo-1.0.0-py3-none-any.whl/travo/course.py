...
class CourseAssignment(Assignment):
    ...
    def submission_path_components(
        ...
        Example:
            ...
            >>> assignment.submission_path_components(username="john.doo")
            ('john-doo-travo', 'TestCourse', '2020-2021', 'SubCourse', 'Assignment1')

            >>> course.path='TestModule/TestCourse'
            >>> assignment.submission_path_components()
            ('travo-test-etu-travo', 'TestModule', 'TestCourse', '2020-2021', 'SubCourse', 'Assignment1')
        """
    ...
    def submission_path(self, username: Optional[str] = None) -> str:
        ...
        More examples with grouped submissions:
            ...
            'travo-test-etu-travo/TestCourse/2020-2021/SubCourse/Assignment1'
    ...
    def ensure_submission_repo(self, leader_name: Optional[str] = None) -> Project:
        ...
        if self.course.group_submissions:
            ...
            path_components = self.submission_path_components()
            name_components = self.submission_name_components()
            current_path = ""
            for path, name in list(zip(path_components, name_components))[:-1]:
                if current_path:
                    current_path += "/"
                current_path += path
                self.forge.ensure_group(current_path, name=name, visibility="private")
        return super().ensure_submission_repo(leader_name=leader_name)
...
class Course:
    ...
    def submit(
        ...
        assignment_dir = self.work_dir(assignment_name)
        the_assignment.submit(assignment_dir=assignment_dir, leader_name=leader_name)
    ...
    def share_with(
        ...
        try:
            repo = self.assignment(assignment_name).submission_repo()
