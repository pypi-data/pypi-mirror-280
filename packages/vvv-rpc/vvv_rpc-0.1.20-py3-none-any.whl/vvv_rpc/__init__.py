...
def remote_client(host, port=19999, debug=False):
    def myget(url):
            ...
            while True:
                ...
                if method in self.irun:
                    for xid in self.irun[method]:
                        self.irun[method][xid].put(rdata['params'])
        ...
        def cdp(self, protocal, data):
            ...
            while True:
                try:
                    ...
                    continue
        def catch_method(self, method, timeout=10):
            ...
            while True:
                try:
                    ret = self.irun[method].get(xid, 0.15)
                    self.irun[method].pop(xid, None)
                    return ret
                except:
                    if time.time() - start > timeout:
                        raise Exception('catch_method {} over time'.format(method))
                    continue
        def go_url(self, url):
            self.cdp("Page.navigate", {"url": url})
            self.catch_method("Page.domContentEventFired")
