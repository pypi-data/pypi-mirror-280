...
class Pipeline():
    def __init__(self, name:str, description:str, 
                 ...
                 id:str = None, branch_id:str = None, **kwargs):
        self.id = id
        ...
        for n in self.nodes.values():
            ...
            for input_name, outputs in n._inputs.items():
                if outputs == []:
                    print(f'WARNING: {n.__class__.__name__} node did not receive any inputs for input field {input_name}')
    ...
    def to_json_rep(self, generic:bool=False) -> dict:
        ...
        try:
            ...
            raise ValueError(f'Error when converting to JSON: {e}. This has likely happened because of improper instantation of nodes. Each node should be assigned to its own variable and should not be constructed within other node constructors.')
        node_jsons = []
    ...
    def add_node(self, node:NodeTemplate):
        ...
        for input_name, outputs in node._inputs.items():
            if outputs == []:
                print(f'WARNING: {node.__class__.__name__} node did not receive any inputs for input field {input_name}')
            # an input could have aggregated several NodeOutputs
            for output in outputs:
                # Edges are specifically defined by source/target handles, 
                # derived from the node ids
    ...
    def save(self, api_key: str = None, public_key: str = None, private_key: str = None, 
        ...
        if response.status_code != 200:
            raise Exception(response.text)
        response = response.json()
        self.id = response.get('pipeline', {}).get('id')
        self.branch_id = response.get('branch', {}).get('id')
    ...
    def share(self, shared_users:list[dict[str, str]], append:bool=False, api_key:str = None, public_key:str = None, private_key:str = None):
        if not self.id:
            raise ValueError('Error sharing: Pipeline does not have an existing ID. It must be saved in order to be shared')
        for u in shared_users:
            if 'user_id' not in u or 'org_id' not in u or 'permissions' not in u:
                raise ValueError('Error sharing: Each shared user should contain keys "user_id", "org_id", and "permissions"')
            if u['permissions'] not in PIPELINE_SHARE_PERMISSIONS:
                raise ValueError(f'Error sharing: Invalid permissions value {u["permissions"]}. Permissions should be one of {PIPELINE_SHARE_PERMISSIONS}')
        response = requests.post(
    ...
    def share_with_permissions(self, shared_users: list[dict[str, str]], permissions: str='Edit', append=False):
        for u in shared_users:
            u['permissions'] = permissions 
        return self.share(shared_users, append)
    
    def get_shared_users(self, api_key:str = None, public_key:str = None,private_key:str = None) -> dict:
        if not self.id:
            raise ValueError('Error: Pipeline does not have an existing ID. It must be saved in order to retrieve shared users')
        response = requests.get(
    ...
    def delete(self, api_key: str = None, public_key: str = None, private_key: str = None):
        if not self.id:
            raise ValueError('Pipeline: Pipeline object does not exist on the VectorShift platform.')
