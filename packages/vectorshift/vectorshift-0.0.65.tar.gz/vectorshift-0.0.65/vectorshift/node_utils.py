...
\t{init_args_str}\n)'
    ...
    def construction_strs(self):
        ...
        _var_name = (re.sub('(?!^)([A-Z]+)', r'_\1', self._id)) \
            .replace('-', '_').lower()
        var_name = _var_name.replace('vector_store', 'knowledge_base')
...
def format_node_output_dict (outputs:dict[str, list[NodeOutput]], indicate_id=False, unwrap_singleton_list:bool=False) -> str:
    d = {}
    ...
    if type(output) == NodeOutput:
        ...
        if not t.intersects(expected_t):
            ...
            if expected_t.intersects(FILE_TYPE):
                error_msg += f". If your input comes from an InputNode, make sure the input type is 'file' and the 'process_files' flag is set to False"
        ...
        if not str_ok or type(output) != str:
            raise ValueError(f'Invalid input to {name}: Not a NodeOutput. If you are constructing another Node object within the constructor of this node, move the constructor outside to declare the Node object as its own variable')
