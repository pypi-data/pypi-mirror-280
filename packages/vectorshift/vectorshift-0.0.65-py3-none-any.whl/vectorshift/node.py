...
class InputNode(NodeTemplate):
    ...
    def output(self) -> NodeOutput:
        ...
        if self.input_type == 'file':
            if not self.process_files: output_data_type = FILE_TYPE
...
class PipelineNode(NodeTemplate):
    ...
    def setup_pipeline_data(self, 
        ...
        if sorted(list(inputs.keys())) != sorted(self.input_names):
            raise ValueError(f'PipelineNode: inputs do not match expected input names: (expected f{self.input_names}, got {list(inputs.keys())}).')
...
class TransformationNode(NodeTemplate):
    ...
    def __init__(self, transformation_name:str, inputs:dict[str, NodeOutput],
        ...
        if sorted(list(inputs.keys())) != sorted(input_names):
            raise ValueError(f'TransformationNode: inputs do not match expected input names (expected {input_names}, got {list(inputs.keys())}).')
        if not 'skip_typecheck' in kwargs or not kwargs['skip_typecheck']:
...
class IntegrationNode(NodeTemplate):
        ...
        if self.integration_type == 'Airtable' and self.action == 'read_tables':
            ...
            for t in kwargs.get('airtable_tables', []):
                if 'base_id' not in t or 'base_name' not in t or 'table_id' not in t:
                    raise ValueError('IntegrationNode: Airtable integration is missing base_id, base_name, or table_id keys in the airtable_tables input argument.') 
            ...
            for field in kwargs.get('table_fields', []):
                if field not in list(inputs.keys()):
                    raise ValueError(f'IntegrationNode: Airtable integration received a value of {field} in the table_fields argument, which was not supplied among the inputs {list(inputs.keys())}.')
            ...
            for field in kwargs.get('database_fields', []):
                if field not in list(inputs.keys()):
                    raise ValueError(f'IntegrationNode: Notion integration received a value of {field} in the database_fields argument, which was not supplied among the inputs {list(inputs.keys())}.')
            ...
            for field in kwargs.get('sheet_fields', []):
                if field not in list(inputs.keys()):
                    raise ValueError(f'IntegrationNode: Google Sheets integration received a value of {field} in the sheet_fields argument, which was not supplied among the inputs {list(inputs.keys())}.')
        # Check that the inputs supplied match the inputs expected
        if sorted(list(inputs.keys())) != sorted(self.input_names):
            raise ValueError(f'IntegrationNode: supplied inputs do not match expected input names (expected {self.input_names}, got {inputs.keys()}).')
    ...
    def __init__(self, 
            integration_type:str, integration_id:str=None, action:str=None, 
        ...
        if not 'skip_typecheck' in kwargs or not kwargs['skip_typecheck']:
            ...
            for k, v in self._inputs.items():
                if len(v) > 0 and type(v[0]) != NodeOutput:
                    raise ValueError(f'IntegrationNode: value provided for input name {k} is not a NodeOutput. Make sure you have instantiated NodeOutput objects earlier and are passing their outputs (via the output() or outputs() methods) into the inputs dict.')
    ...
    def init_args_strs(self, indicate_id=False):
        args_strs = [
            f"integration_type='{self.integration_type}'" if self.__class__ == IntegrationNode else None,
...
class SystemPromptLLMNode(NodeTemplate):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        args_strs = [
            f"llm_family='{self.llm_family}'" if self.__class__ == SystemPromptLLMNode else None,
            f"model='{self.model}'",
            system_arg_str,
            prompt_arg_str,
            ...
            f'stream_response={self.stream_response}' \
                if self.stream_response else None,
            f'json_response={self.json_response}' \
                if self.json_response else None,
...
class PromptLLMNode(NodeTemplate):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        args_strs = [
            f"llm_family='{self.llm_family}'" if self.__class__ == PromptLLMNode else None,
            f"model='{self.model}'",
            prompt_arg_str, 
            f'max_tokens={self.max_tokens}' \
                if self.max_tokens != DEFAULT_MAX_TOKENS else None,
...
class OpenAIVisionNode(OpenAILLMNode):
    ...
    def __init__(self, model:str, 
        ...
        if not 'skip_typecheck' in kwargs or not kwargs['skip_typecheck']:
            check_type(f'OpenAIVisionNode input image', image_input, IMAGE_FILE_TYPE)
...
class GoogleVisionNode(PromptLLMNode):
    ...
    def __init__(self, model:str, 
        ...
        **kwargs):
        ...
        super().__init__(llm_family='google', model=model, prompt_input=prompt_input, text_inputs=text_inputs, max_tokens=max_tokens, temperature=temperature, top_p=top_p, stream_response=stream_response)
        if not 'skip_typecheck' in kwargs or not kwargs['skip_typecheck']:
            check_type(f'GoogleVisionNode input image', image_input, IMAGE_FILE_TYPE)
        self._inputs['image'] = [image_input]
...
class DataLoaderNode(NodeTemplate):
        ...
        match self.loader_type:
            ...
            case 'CSV Query':
                ...
                check_type(
                    ...
                    inputs['csv'][0], UnionType(CSV_FILE_TYPE, FILE_TYPE, ListType(CSV_FILE_TYPE), ListType(FILE_TYPE))
                )
            case 'URL':
                check_type(
    ...
    def __init__(self, loader_type:str, 
        ...
        if loader_type in ['Notion', 'Confluence']:
            raise ValueError(f'DataLoaderNode: dataloader type {loader_type} is deprecated.')
        input_names = DATALOADER_PARAMS[loader_type]['input_names']
        if sorted(list(inputs.keys())) != sorted(input_names):
            raise ValueError(f'DataLoaderNode: inputs do not match expected input names (expected {input_names}, got {list(inputs.keys())}).')
        ...
        if inputs:
            for k, v in inputs.items():
                if type(v) != list:
                    raise ValueError(f'DataLoaderNode: value provided for input name {k} is not a list. You should provide a list of NodeOutputs or a singleton list of a string.')
                if len(v) == 0:
                    raise ValueError(f'DataLoaderNode: received no inputs for input name {k}. You should provide a list of NodeOutputs or a singleton list of a string.')
                if type(v[0]) == NodeOutput or v[0] is None:
                    ...
                    raise ValueError(f'DataLoaderNode: value provided for input name {k} must be a list of NodeOutputs or a singleton list of a string.')
        if not 'skip_typecheck' in kwargs or not kwargs['skip_typecheck']:
            self.typecheck_inputs()
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        return [
            f"loader_type='{self.loader_type}'" if self.__class__ == DataLoaderNode else None,
    ...
    def _to_json_rep(self, generic:bool=False):
        input_strs = {k: v[0] for k, v in self._input_strs.items() if k not in self._inputs}
        return {
...
class CSVQueryLoaderNode(DataLoaderNode):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        if 'query' in self._inputs:
            query_arg_str = format_node_output_with_name('query_input', self._inputs['query'][0], indicate_id)
        csv_arg_str = format_node_output_with_name('csv_input', self._inputs['csv'][0], indicate_id)
...
class URLLoaderNode(DataLoaderNode):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        if 'url' in self._inputs:
            url_arg_str = format_node_output_with_name('url_input', self._inputs['url'][0], indicate_id)
...
class SerpAPILoaderNode(DataLoaderNode):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        if 'apiKey' in self._inputs:
            api_key_arg_str = format_node_output_with_name('api_key_input', self._inputs['apiKey'][0], indicate_id)
        query_arg_str = format_node_output_with_name('query_input', self._inputs['query'][0], indicate_id)
...
class GitLoaderNode(DataLoaderNode):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        if 'repo' in self._inputs:
            repo_arg_str = format_node_output_with_name('repo_input', self._inputs['repo'][0], indicate_id)
...
class YouDotComLoaderNode(DataLoaderNode):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        if 'query' in self._inputs:
            query_arg_str = format_node_output_with_name('query_input', self._inputs['query'][0], indicate_id)
        return [
            f"loader_type='{self.you_loader_type}'",
            query_arg_str
        ] + args_strs[2:]
...
class ExaAILoaderNode(DataLoaderNode):
    ...
    def init_args_strs(self, indicate_id=False):
        ...
        if 'query' in self._inputs:
            query_arg_str = format_node_output_with_name('query_input', self._inputs['query'][0], indicate_id)
        return [
            f"loader_type='{self.exa_loader_type}'",
            query_arg_str
        ] + args_strs[2:]
...
class VectorDBLoaderNode(NodeTemplate):
    def __init__(self, documents_input:list[NodeOutput], 
        ...
        if chunk_overlap < 0: raise ValueError('VectorDBLoaderNode: invalid chunk_overlap value.')
        if chunk_overlap >= chunk_size:
...
class LogicConditionNode(NodeTemplate):
    ...
    def __init__(self, inputs:dict[str, NodeOutput], 
        ...
        for cond in conditions:
            if cond[1] not in input_names:
                print(f'WARNING: LogicConditionNode returned value {cond[1]} of condition {cond[0]} was not specified in inputs. Interpreting as a string.')
                cond = (cond[0], f'\"{cond[1]}\"')
        if else_value not in input_names:
            print(f'WARNING: LogicConditionNode returned value {else_value} of else condition was not specified in inputs. Interpreting as a string.')
    ...
    def outputs(self):
        ...
        if else_output_source_name in self._inputs:
            else_output_data_type = self._inputs[else_output_source_name][0].output_data_type
        else_o = NodeOutput(
            source=self, 
...
class ChatMemoryNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'ChatMemoryNode':
        ...
        if not memory_window: memory_window = 0
...
class AgentNode(NodeTemplate):
    ...
    def __init__(self, agent_id:str=None, agent_name:str=None,
        ...
        if sorted(list(inputs.keys())) != sorted(input_names):
            raise ValueError(f'AgentNode: inputs do not match expected input names (expected {input_names}, got {list(inputs.keys())}).')
        self._inputs = {
            input_name: [inputs[input_name]] for input_name in input_names
        }
        if not 'skip_typecheck' in kwargs or not kwargs['skip_typecheck']:
