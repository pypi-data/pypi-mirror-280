    ...
    def typecheck_inputs(self):
        if self.output_type in ['text', 'formatted_text']:
            ...
            check_type(
                'OutputNode',
                self._inputs['value'][0],
                UnionType(IMAGE_FILE_TYPE, ListType(IMAGE_FILE_TYPE)),
            )
...
class TextNode(NodeTemplate):
    ...
    def set_text(self, text: str):
        ...
        self._inputs = {k: v for k, v in self.all_inputs if k in self.text_vars}
    ...
    def __init__(
        ...
        **kwargs,
        ...
        response = requests.get(
            ...
            data={'transformation_name': self.transformation_name},
            headers={
...
class SystemPromptLLMNode(NodeTemplate):
    ...
    def set_system(self, system_input: str | NodeOutput):
        if isinstance(system_input, NodeOutput):
            ...
            new_inputs = {
                k: v
                for k, v in self.all_text_inputs
...
class ChatMemoryNode(NodeTemplate):
    ...
    def __init__(self, memory_type: str, memory_window: int = None, **kwargs):
        ...
        if memory_window is not None:
            ...
            if CHAT_MEMORY_TYPES[self.memory_type] == 0:
                raise ValueError(
                    'ChatMemoryNode: memory window should not be specified if the chat memory is the full text.'
                )
    ...
    def set_memory_window(self, memory_window: int):
        ...
        if CHAT_MEMORY_TYPES[self.memory_type] == 0:
            raise ValueError(
                "ChatMemoryNode: memory window shouldn't be specified if the chat memory is the full text."
            )
        self.memory_window = memory_window
