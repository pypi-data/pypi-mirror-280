...
def use_backend(backend, reloading=True, silent=False):
    ...
    if backend == "graph-tool":
        ...
        raise ValueError(f"Invalid graph library requested: '{backend}'.")

    # always reload analysis (necessary if a requested library failed partway)
    if nngt._initialized:
    ...
    if reloading:
        # reload the others if necessary
    ...
    # make sure all other config values are unchanged
