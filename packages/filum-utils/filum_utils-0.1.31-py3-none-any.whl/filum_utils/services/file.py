...
from filum_utils.enums import FileCSVDelimiter
...
class FileService:
    ...
    def _get_csv_file_data(cls, file_content_bytes: bytes, file_extension: str) -> List[Dict[str, Any]]:
        ...
        if not delimiter:
            raise BaseError(
                message="Can not get the delimiter in csv file."
            )
    ...
    def _decode_bytes(default_bytes: bytes) -> str:
        ...
        try:
            decoding_code = chardet.detect(default_bytes)
            return default_bytes.decode(decoding_code)
