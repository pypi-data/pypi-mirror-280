...
:LastEditTime: 2024-06-24 09:36:19
...
class ApplyRefundHandler(SevenBaseHandler):
    ...
    def post_async(self):
        ...
        order_model = OrderModelEx().get_dict(where="order_id = %s and shop_id = %s",params=[order_id,shop_id],field="status,postage,real_pay_price,price,coupon_price,coupon_id")
        if not order_model:
            return self.reponse_json_error("无法获取订单信息")
...
class RefundDetailHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        refund_model = OrderRefundModel().get_dict(where="refund_order_id = %s and shop_id = %s",params=[refund_order_id,shop_id],field="order_id,refund_order_id,refund_type,goods_refund_price,real_refund_price,postage,reason,status,add_time,pass_time,fail_remark")
        if not refund_model:
