...
def ignore_bipolar_deprecation_warning():
    # Suppress this warning because we are intentionally testing the bipolar variables.
...
def test_vtype_sent(model_s3_client, mock_titanq_client, vtype):
    ...
    ignore_bipolar_deprecation_warning()
...
def test_using_bipolar_variable_type_warns_as_deprecated(model_s3_client):
    with pytest.warns(DeprecationWarning, match="bipolar"):
        model_s3_client.add_variable_vector("x", 1, Vtype.BIPOLAR)
