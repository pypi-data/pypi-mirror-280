...
class ProviderDirectoryManager:
    ...
    def load_category_tree_json(self, *, revision_id: RevisionId | None = None) -> CategoryTreeJson | None:
        ...
        category_tree_json_data = (
            self._load_category_tree_json_data_from_file(parser=parser)
            if revision_id is None
            else self._load_category_tree_json_data_from_git(parser=parser, revision_id=revision_id)
        )
    ...
    def load_provider_json(self, *, revision_id: RevisionId | None = None) -> ProviderJson:
        ...
        provider_json_data = (
            self._load_provider_json_data_from_file(parser=parser)
            if revision_id is None
            else self._load_provider_json_data_from_git(parser=parser, revision_id=revision_id)
        )
    ...
    def load_releases_json(self, *, revision_id: RevisionId | None = None) -> ReleasesJson | None:
        ...
        releases_json_data = (
            self._load_releases_json_data_from_file(parser=parser)
            if revision_id is None
            else self._load_releases_json_data_from_git(parser=parser, revision_id=revision_id)
        )
    ...
    def _load_category_tree_json_data_from_file(self, *, parser: JsonParser) -> Json | None:
        category_tree_json_path = self.category_tree_json_path

        try:
            return parser.parse_file(category_tree_json_path)
    ...
    def _load_category_tree_json_data_from_git(self, *, parser: JsonParser, revision_id: RevisionId) -> Json | None:
        if self._git_provider_directory_manager is None:
            raise RevisionsNotAvailable
        ...
        try:
            return self._git_provider_directory_manager.load_json_file(
                category_tree_json_path, parser=parser, revision_id=revision_id
            )
        except BlobNotFound:
            return None
        except JsonParseError as exc:
    ...
    def _load_provider_json_data_from_file(self, *, parser: JsonParser) -> Json:
        provider_json_path = self.provider_json_path

        try:
            return parser.parse_file(provider_json_path)
        except FileNotFoundError as exc:
            raise ProviderJsonNotFound(provider_code=self.provider_code, provider_json_path=provider_json_path) from exc
        except JsonParseError as exc:
            raise ProviderJsonLoadError(
    ...
    def _load_provider_json_data_from_git(self, *, parser: JsonParser, revision_id: RevisionId) -> Json:
        if self._git_provider_directory_manager is None:
            raise RevisionsNotAvailable
        ...
        try:
            return self._git_provider_directory_manager.load_json_file(
                provider_json_path, parser=parser, revision_id=revision_id
            )
    ...
    def _load_releases_json_data_from_file(self, *, parser: JsonParser) -> Json | None:
        releases_json_path = self.releases_json_path

        try:
            return parser.parse_file(releases_json_path)
        except FileNotFoundError:
            return None
        except JsonParseError as exc:
            raise ReleasesJsonLoadError(
    ...
    def _load_releases_json_data_from_git(self, *, parser: JsonParser, revision_id: RevisionId) -> Json | None:
        if self._git_provider_directory_manager is None:
            raise RevisionsNotAvailable
        ...
        try:
            return self._git_provider_directory_manager.load_json_file(
                releases_json_path, parser=parser, revision_id=revision_id
            )
        except BlobNotFound:
            return None
        except JsonParseError as exc:
