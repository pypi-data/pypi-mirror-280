...
def get_centers(gem, CellIDs, GeneIDs):
    ...
    """
    W = np.zeros((len(CellIDs) * 2, len(GeneIDs)))
    i = 0
    for c in CellIDs:
        j = 0
...
def DeriveRotation(W, X, Mask, CellUIDs, adata):
    ...
    Args:
        W (numpy.ndarray): The input array of shape (N, M) representing gene expression levels.
        X (numpy.ndarray): The input array of shape (N, 3) representing cell positions.
        Mask (numpy.ndarray): The input array of shape (F, M) representing the gene mask for each cell.
    ...
    Returns:
        Rotation (numpy.ndarray): The array of shape (F, 3, 3) representing the rotation matrices for each cell.
        W (numpy.ndarray): The updated array of shape (N', M) after removing cells.
...
def UpdateX(RM, W, GeneUID, *X_old):
    """
    Update the coordinates of configuration based on the given rotation matrices RM and observation matrix of transcription centers W.

    Parameters:
    - RM (numpy.ndarray): A 3-dimensional matrix representing the rotation matrices.
    - W (numpy.ndarray): A 2-dimensional matrix representing the observation matrix of transcription centers.
    - GeneUID (list): A list of gene unique identifiers for transcription centers.
    - X_old (numpy.ndarray): The old coordinates of configuration X.

    Returns:
    - newX (numpy.ndarray): A 2-dimensional matrix representing the updated coordinates of X.
    - W (numpy.ndarray): A 2-dimensional matrix representing the filtered observation matrix of transcription centers W after removing some genes.
    - GeneUID (list): A list of gene unique identifiers after removing some genes.
    - X_old (numpy.ndarray): A 2-dimensional matrix representing the filtered old coordinates of X after removing some genes.

    Note:
    - If X_old is not provided, the function will only update newX, W, and GeneUID.
    - If X_old is provided, the function will update X_old as well.
    """
    F = int(W.shape[0] / 2)
    pop_indices = []
...
def craft(
    ...
    W = get_centers(gem, CellUIDs, GeneUIDs)
