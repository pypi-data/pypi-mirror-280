...
def Store(sheet):
    ...
    SheetSummaries.loc[len(SheetSummaries.index)] = [sheet.Reference, sheet.Name, sheet.Version, "No test done" if sheet.GlobalStatus == None else "Passed" if sheet.GlobalStatus else "FAILED", sheet.Statistics.PassedCount, sheet.Statistics.FailedCount]
