...
async def write_mp3_with_metadata(data):
    ...
    for index, metadata in enumerate(metadata_array):
        input_file = os.path.join(os.path.dirname(__file__), "..", metadata["album"], f"{index}.mp3")
        ...
        output_file = os.path.join(os.path.dirname(__file__), "..", metadata["album"], f"{sanitized_title}.mp3")
