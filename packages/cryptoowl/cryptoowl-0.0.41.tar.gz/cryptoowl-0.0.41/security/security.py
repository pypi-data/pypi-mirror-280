...
from security.constants import HONEYPOT_FINDER_URL, GO_PLUS_URL, INSERT_OR_UPDATE_ETH_SECURITY_DATA_QUERY, \
    ...
    SOCIAL_INTELLIGENCE_DB_SECRET_ID, SECURITY_DATA_DICT
...
class Honeypot:
    ...
    @classmethod
    def __store_security_data(cls, security_data, chain_id):
        if security_data:
            ...
            filled_by = security_data.get("filled_by")

            if chain_id in (1, "eth"):
    ...
    def get_security_data(self, chain_id, token_id):
        ...
        security_data_dict = self.get_data_from_goplus(chain_id=chain_id, token_id=token_id,
                                                       security_data_dict=SECURITY_DATA_DICT)
    ...
    @classmethod
    def get_data_from_goplus(cls, chain_id, token_id, security_data_dict={}):
        ...
        try:
            if response.status_code not in [500, 404]:
                ...
                if result:
                    ...
                    if not security_data_dict.get("is_filled"):
                        security_data_dict["token_id"] = token_id
                        security_data_dict["honeypot"] = is_honey_pot
                        security_data_dict["buy_tax"] = float(buy_tax) * 100 if buy_tax else buy_tax
                        security_data_dict["sell_tax"] = float(sell_tax) * 100 if sell_tax else sell_tax
                        security_data_dict["holder_count"] = holders_count
                        security_data_dict["can_mint"] = is_mintable
                        security_data_dict["contract_creator"] = contract_creator
                        security_data_dict["contract_owner"] = contract_owner
                        security_data_dict["lp_lock_percentage"] = float(lp_lock_percentage) * 100 if lp_lock_percentage else lp_lock_percentage
                        security_data_dict["is_filled"] = True
                        security_data_dict["filled_by"] = "goplus"
    ...
    @classmethod
    def get_data_from_honeypot(cls, chain_id, token_id, security_data_dict={}):
        ...
        try:
            if response.status_code not in [500, 404]:
                ...
                if not security_data_dict.get("is_filled"):
                    security_data_dict["token_id"] = token_id
                    security_data_dict["honeypot"] = is_honey_pot
                    security_data_dict["buy_tax"] = buy_tax
                    security_data_dict["sell_tax"] = sell_tax
                    security_data_dict["is_filled"] = True
                    security_data_dict["filled_by"] = "honeypot.io"
                    cls.__store_security_data(security_data=security_data_dict, chain_id=chain_id)
    ...
    def get_data_from_quickintel(self, chain_id, token_id, security_data_dict={}):
        if isinstance(chain_id, int):
            chain_id = id_to_chain.get(chain_id)
        ...
        try:
            if response.status_code not in [500, 404]:
                ...
                if not security_data_dict.get("is_filled"):
                    security_data_dict["token_id"] = token_id
                    security_data_dict["honeypot"] = honeypot
                    security_data_dict["buy_tax"] = buy_tax
                    security_data_dict["sell_tax"] = sell_tax
                    security_data_dict["lp_burned"] = lp_burned
                    security_data_dict["is_scam"] = is_scam
                    security_data_dict["can_burn"] = can_burn
                    security_data_dict["can_mint"] = can_mint
                    security_data_dict["can_freeze"] = can_freeze
                    security_data_dict["contract_creator"] = contract_creator
                    security_data_dict["contract_owner"] = contract_owner
                    security_data_dict["lp_lock_percentage"] = lp_lock_percentage
                    security_data_dict["lp_unlock_date"] = lp_unlock_date
                    security_data_dict["is_filled"] = True
                    security_data_dict["filled_by"] = "quick_intel"
