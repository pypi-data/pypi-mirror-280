...
def columns_to_rows(column_table):
	return list(map(list, zip(*column_table)))
...
class MapUI(object):
	...
	def add_menu(self, table_object):
      ...
      Copyright %s, R Dreas Nielsen
		...
		stats_menu.add_command(label="Cosine similarity matrix", command = cos_matrix, underline=4)
...
class QueryDialog(object):
	def __init__(self, parent, column_headers, db_conn, init_sql=""):
		...
		help_btn.grid(row=0, column=0, sticky=tk.W, padx=(6,3))
		self.dlg.bind("<Alt-h>", self.do_help)
		self.apply_btn = ttk.Button(btn_frame, text="Apply", command=self.do_apply, underline=0)
		self.apply_btn.grid(row=0, column=1, sticky=tk.E, padx=(3,3))
		if init_sql is None or len(init_sql) == 0:
			self.apply_btn["state"] = tk.DISABLED
		self.dlg.bind('<Alt-a>', self.do_apply)
		self.ok_btn = ttk.Button(btn_frame, text="OK", command=self.do_select, underline=0)
		self.ok_btn.grid(row=0, column=2, sticky=tk.E, padx=(3,3))
		if init_sql is None or len(init_sql) == 0:
			self.ok_btn["state"] = tk.DISABLED
		self.dlg.bind('<Alt-o>', self.do_select)
		cancel_btn = ttk.Button(btn_frame, text="Cancel", command=self.do_cancel, underline=0)
		cancel_btn.grid(row=0, column=3, sticky=tk.E, padx=(3,6))
		self.dlg.bind("<Escape>", self.do_cancel)
		self.dlg.bind("<Alt-c>", self.do_cancel)
		self.dlg.bind("<Alt-o>", self.do_select)
...
class AggregateDialog(object):
	def __init__(self, parent, column_specs):
		...
		aggreg_sel["values"] = ["Minimum", "Maximum", "Median", "Arithmetic mean", "Geometric mean", "Harmonic mean", "Sum"]
		...
		self.help_btn.grid(row=0, column=0, sticky=tk.W, padx=(6,3))
		self.dlg.bind("<Alt-h>", self.do_help)
		self.data_btn = ttk.Button(btn_frame, text="Source Data", state=tk.DISABLED, command=self.show_data)
		self.data_btn.grid(row=0, column=1, sticky=tk.W, padx=(3,3))
		close_btn = ttk.Button(btn_frame, text="Close", command=self.do_close, underline=0)
		close_btn.grid(row=0, column=2, sticky=tk.E, padx=(6,6))
		self.dlg.bind("<Alt-c>", self.do_close)
	...
	def get_data(self):
		...
		if self.n_dataset_columns > 0:
			...
			if dataset is None or len(dataset[0]) == 0:
				...
				groups = sorted(list(set(dataset[self.n_dataset_columns])))
...
class ColocatedDialog(object):
	def __init__(self):
		...
		self.dlg.bind("<Alt-o>", self.do_select)
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/find_colocated.html", new=2, autoraise=True)
	def do_cancel(self, *args):
		self.canceled = True
		self.dlg.destroy()
	def do_select(self, *args):
...
class VariableDistDialog(object):
		...
		if dataset is None:
			...
			if self.ylog_var.get() == '1' and self.ylog_ck["state"] != tk.DISABLED:
				...
				for col in range(self.n_dataset_columns):
					for i in range(len(dataset[col])):
						try:
							log_data[col].append(math.log10(dataset[col][i]))
						except:
							log_error = True
							self.ylog_var.set("0")
							break
				if not log_error:
					self.plot_data = log_data
	...
	def redraw(self):
		...
		if plot_type == "Box plot":
			self.dlg.bind("<Alt-r>", self.set_rotated)
			self.plot_axes.boxplot(self.plot_data, labels=grplbls, vert=not self.rotated)
			if not self.rotated:
				self.plot_axes.set_ylabel(self.ylabel or value_lbl)
			else:
				self.plot_axes.set_xlabel(self.ylabel or value_lbl)
		elif plot_type == "Kernel density (KD) plot":
			self.dlg.bind("<Alt-a>", self.set_alpha)
			sns.kdeplot(data=plt_data, multiple="layer", fill=True, alpha=self.alpha, ax=self.plot_axes,
					warn_singular=False)
			self.plot_axes.set_ylabel(self.ylabel or "Density")
		elif plot_type == "Stripchart":
			self.dlg.bind("<Alt-a>", self.set_alpha)
			self.dlg.bind("<Alt-r>", self.set_rotated)
			orientation = "v" if not self.rotated else "h"
			sns.stripplot(data=plt_data, alpha=self.alpha, ax=self.plot_axes, orient=orientation)
			if not self.rotated:
				self.plot_axes.set_ylabel(self.ylabel or value_lbl)
			else:
				self.plot_axes.set_xlabel(self.ylabel or value_lbl)
			...
			try:
				sns.violinplot(data=plt_data, vert=orientation, ax=self.plot_axes)
				if not self.rotated:
					self.plot_axes.set_ylabel(self.ylabel or value_lbl)
				else:
					self.plot_axes.set_xlabel(self.ylabel or value_lbl)
			except:
				sns.violinplot(data=plt_data, ax=self.plot_axes)
				self.plot_axes.set_ylabel(self.ylabel or value_lbl)
		if self.xlabel is not None:
			if self.rotated:
				self.plot_axes.set_ylabel(self.xlabel)
			else:
				self.plot_axes.set_xlabel(self.xlabel)
		if self.plot_title is not None:
			self.plot_axes.set_title(self.plot_title)
			self.plot_axes.title.set_visible(True)
		else:
...
class CosineSimilarityDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.data_btn = ttk.Button(btn_frame, text="Source Data", state=tk.DISABLED, command=self.show_data)
		self.data_btn.grid(row=0, column=1, sticky=tk.W, padx=(3,3))
		self.agg_data_btn = ttk.Button(btn_frame, text="Aggregates", state=tk.DISABLED, command=self.show_agg)
		self.agg_data_btn.grid(row=0, column=2, sticky=tk.W, padx=(3,3))
		close_btn = ttk.Button(btn_frame, text="Close", command=self.do_close, underline=0)
	...
	def show_agg(self, *args):
		if self.agg_data is not None:
			show_columnar_table(self.dlg, "Aggregated Data", "Selected data, aggregated for analysis:", self.agg_data, self.data_labels[0:len(self.dataset)], \
					"Aggregaetd data for covariance matrix")
	...
	def q_redraw(self, *event, get_data=True):
		self.clear_output()
		if (len(self.column_table.selection()) > 1 and self.groupby_var.get() != '') and get_data:
			self.get_data()
	...
	def get_data(self):
		...
		if self.n_dataset_columns > 0:
			...
			if n_groups < 2:
				...
				groups = sorted(list(set(clean_data[self.n_dataset_columns])))
				# Convert all numerics to floats
				for i in range(self.n_dataset_columns):
					clean_data[i] = [conv_float(v) for v in clean_data[i]]
				#
	...
	def redraw(self):
		...
		if self.agg_data is not None and len(self.agg_data) > 1 and len(self.agg_data[0]) > 1:
			...
			caxes = self.plot_axes.matshow(cosmat, cmap="BrBG", vmin=-1.0, vmax=1.0)
			self.plotfig.colorbar(caxes)
			self.plot_axes.set_xticks(range(ncases), labels=self.agg_data_groups, rotation=25)
			self.plot_axes.set_yticks(range(ncases), labels=self.agg_data_groups)
			if self.show_labels:
				for i in range(ncases):
					for j in range(ncases):
						v = cosmat[i,j]
						c = "white" if abs(v) > 0.40 else "black"
						self.plot_axes.text(j, i, f"{cosmat[i,j]:.2f}", ha="center", va="center", color=c)
			self.plotfig_canvas.draw()
			self.loading_dlg.hide()
...
class TSNEDialog(object):
	def __init__(self, parent, column_specs, prohibited_columns):
		...
		self.calc_btn.grid(row=3, column=1, sticky=tk.E, padx=(3,6), pady=(3,3))
