...
from shyft.hydrology.repository.netcdf.utils import (_clip_ensemble_of_geo_timeseries, _limit_1D, _numpy_to_geo_ts_vec,
                                                     calc_P, calc_RH, series_type, utm_proj)
from shyft.time_series import (POINT_INSTANT_VALUE, Calendar, GeoGridSpec, GeoPoint, GeoPointVector, TimeAxis,
                               UtcPeriod, UtcTimeVector, time, utctime_now)
...
class ConcatDataRepository(interfaces.GeoGridTsRepository):
    ...
    def grid_spec(self) -> GeoGridSpec:
        ...
        return GeoGridSpec(epsg=int(epsg_str), points=GeoPointVector(gps))

    @property
    def data_period(self) -> UtcPeriod:
        """
        TODO: What is the purpose, and robust definition of this property ?
        Should it be the [start..end), as in:

         start = first point covered, e.g. first t0 + lead_time[n_to_skip]
         end   = last t0  plus length of the forecast (taken into account the n_to_skip and n_to_keep)

         as pr usual:  A period should always be half open interval [ .. )
         and for simplicity, we could interpret all values as staircase start of step.
         or we could also have extend-flat for linear-between points.

         Also take into account that data_period does NOT say anything of the completeness,
         quality etc. of data in the repository, just a time-bracket for what might be there.
         The current def returns the data period for the FIRST forecast in the set of forecasts.
        """
        dt = 0  # as for now, keep old (and not consistent, useful def) Cecilie better def: int(self.lead_times_in_sec[1] - self.lead_times_in_sec[0])
        return UtcPeriod(int(self.time[0]), int(self.time[0] + self.lead_times_in_sec[-1]) + dt)
