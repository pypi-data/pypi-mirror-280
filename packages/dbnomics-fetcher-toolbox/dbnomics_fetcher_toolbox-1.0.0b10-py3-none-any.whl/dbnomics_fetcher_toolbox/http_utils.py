...
def download_http_url(
    url: str, *, output_file: Path, response_dump_dir: Path | None = None, session: Session | None = None
) -> None:
    ...
    source_bytes_iter = fetch_http_url(url, response_dump_file=response_dump_file, session=session)
    write_chunks(source_bytes_iter, output_file=output_file)


def fetch_http_url(
    url: str, *, response_dump_file: Path | None = None, session: Session | None = None
) -> Iterator[bytes]:
    source = RequestsHttpSource(url, session=session)
