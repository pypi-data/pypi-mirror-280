...
Coordinate = Tuple[int, int] # Coordinate is a tuple of two integers representing (row, column).
Board = List[List[str]]
...
class Go_game:
  def __init__(self, size: int, board: Board | None = None, color: str = BLACK) -> None:
    ...
    self.idx: int = 0
    self.ko_spot: Coordinate | None = None
  ...
  def play_move(self, y: int, x: int) -> Board:
    move: Coordinate = (y, x)
