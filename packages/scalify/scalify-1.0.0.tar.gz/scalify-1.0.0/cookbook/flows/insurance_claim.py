...
hypothetical car insurance claim using Scalify. We use prefect to define
...
def scalify_extract_damages_from_url(image_url: str) -> list[DamagedPart]:
    return scalify.extract(
        data=scalify.Image.from_url(image_url),
        target=DamagedPart,
        instructions=(
...
def process_damage_report(car: Car):
    damaged_parts = sorted(
        scalify_extract_damages_from_url(car.image_url), key=lambda x: x.part
    )
