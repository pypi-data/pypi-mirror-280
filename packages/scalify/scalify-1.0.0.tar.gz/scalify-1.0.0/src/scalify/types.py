...
from scalify.settings import settings
...
class Function(ScalifyType, Generic[T]):
    name: str
...
class ToolSet(ScalifyType, Generic[T]):
...
class FunctionCall(ScalifyType):
    name: str


class ImageUrl(ScalifyType):
    url: str = Field(
...
class ImageFileContentBlock(ScalifyType):
...
class TextContentBlock(ScalifyType):
...
class ResponseModel(ScalifyType):
    model: type
...
class TranscriptRequest(ScalifyType):
...
class ChatResponse(ScalifyType):
...
class ImageRequest(ScalifyType):
    prompt: str
...
class SpeechRequest(ScalifyType):
    input: str
...
class Run(ScalifyType, Generic[T]):
    id: str
    thread_id: str
    created_at: int
    status: str
...
class StreamingChatResponse(ScalifyType):
...
class Audio(ScalifyType):
    ...
    async def save_async(self, path: str, format: str = None):
        ...
        if self.format != format:
            ...
            audio = scalify.audio.convert_audio(self.data, self.format, format)
        else:
            audio = self.data
    ...
    async def play_async(self):
        ...
        if self._data_stream is not None:
            await scalify.audio.stream_audio(self._stream())

        else:
            scalify.audio.play_audio(self.data, format=self.format)
