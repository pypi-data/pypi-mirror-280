...
async def get_token() -> str:
    ...
    try:
        token = (
            scalify.settings.slack_api_token
        )  # set `SCALIFY_SLACK_API_TOKEN` in `~/.scalify/.env
    except AttributeError:
        if token := os.getenv("SCALIFY_SLACK_API_TOKEN"):
            return token
        try:  # TODO: clean this up
            ...
            pass
        raise ValueError(
            "`SCALIFY_SLACK_API_TOKEN` not found in environment."
            " Please set it in `~/.scalify/.env` or as an environment variable."
        )
    return token
