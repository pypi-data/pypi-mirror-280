...
def cast_to_model(
    ...
    Example:
        ...
        ```python
        from scalify.utilities.pydantic import cast_to_model
...
def parse_as(
    ...
    Example:
        ...
        parsed = parse_as(ExampleModel, {"name": "Scalify"})
        assert isinstance(parsed, ExampleModel)
        assert parsed.name == "Scalify"

        # parsing json strings
        parsed = parse_as(
            list[ExampleModel],
            '[{"name": "Scalify"}, {"name": "Arthur"}]',
            mode="json"
        )
        assert all(isinstance(item, ExampleModel) for item in parsed)
        assert parsed[0].name == "Scalify"
