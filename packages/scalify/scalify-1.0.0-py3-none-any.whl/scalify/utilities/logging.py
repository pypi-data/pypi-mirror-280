...
def get_logger(
    ...
    Example:
        ...
        logger = get_logger("scalify.test")
        logger.info("This is a test") # Output: scalify.test: This is a test

        debug_logger = get_logger("scalify.debug")
        debug_logger.debug_kv("TITLE", "log message", "green")
        ```
    """
    parent_logger = logging.getLogger("scalify")

    if name:
        # Append the name if given but allow explicit full names e.g. "scalify.test"
        # should not become "scalify.scalify.test"
...
def setup_logging(
    ...
    if level is not None:
        ...
        logger.setLevel(scalify.settings.log_level)
