...
import scalify
from scalify import settings
from scalify.types import (
...
async def should_fallback(e: NotFoundError, request: ChatRequest) -> bool:
    if (
        ...
        and request.model == scalify.settings.openai.chat.completions.model
    ):
        get_logger().warning(
            "Scalify's default chat model is"
            f" {scalify.settings.openai.chat.completions.model!r}, which your"
            ...
            " `SCALIFY_CHAT_COMPLETIONS_MODEL=<accessible model>` in"
            f" `~/.scalify/.env` - for example, `gpt-3.5-turbo`.\n\n {e}"
        )
        return True
    else:
        return False
...
class ScalifyClient(pydantic.BaseModel):
    ...
    def generate_image(
        ...
        **kwargs: Any,
        ...
        # validate request
        request = ImageRequest(**scalify.settings.openai.images.model_dump() | kwargs)
...
class AsyncScalifyClient(pydantic.BaseModel):
    model_config = pydantic.ConfigDict(
        arbitrary_types_allowed=True, protected_namespaces=()
    )
...
def get_default_client(**client_kwargs) -> ScalifyClient:
    client_cls = scalify.settings.default_client_cls or ScalifyClient
    return client_cls(**client_kwargs)


def get_default_async_client(**client_kwargs) -> AsyncScalifyClient:
    client_cls = scalify.settings.default_async_client_cls or AsyncScalifyClient
    return client_cls(**client_kwargs)
