...
try:
    ...
    raise ImportError(
        ...
        " it with `pip install chromadb` or `pip install scalify[chroma]`."
    )
...
try:
    HOST, PORT = (
        getattr(scalify.settings, "chroma_server_host"),
        getattr(scalify.settings, "chroma_server_http_port"),
    )
    DEFAULT_COLLECTION_NAME = getattr(
        scalify.settings, "chroma_default_collection_name", "scalify"
    )
except AttributeError:
    HOST = os.environ.get("SCALIFY_CHROMA_SERVER_HOST", "localhost")  # type: ignore
    PORT = os.environ.get("SCALIFY_CHROMA_SERVER_HTTP_PORT", 8000)  # type: ignore
    DEFAULT_COLLECTION_NAME = os.environ.get(
        "SCALIFY_CHROMA_DEFAULT_COLLECTION_NAME", "scalify"
    )
...
def create_openai_embeddings(texts: list[str]) -> list[float]:
    ...
    from scalify.client.openai import ScalifyClient

    embedding: "CreateEmbeddingResponse" = ScalifyClient().client.embeddings.create(
        input=[text.replace("\n", " ") for text in texts],
