"""Settings for configuring `scalify`."""
...
class ScalifySettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_file="" if os.getenv("SCALIFY_TEST_MODE") else ("~/.scalify/.env", ".env"),
        extra="allow",
...
class ChatCompletionSettings(ScalifySettings):
    model_config = SettingsConfigDict(
        env_prefix="scalify_chat_completions_", extra="ignore"
    )
...
class ChatSettings(ScalifySettings):
    model_config = SettingsConfigDict(env_prefix="scalify_chat_", extra="ignore")
    completions: ChatCompletionSettings = Field(default_factory=ChatCompletionSettings)


class ImageSettings(ScalifySettings):
    ...
    model_config = SettingsConfigDict(env_prefix="scalify_image_", extra="ignore")
...
class SpeechSettings(ScalifySettings):
...
class AssistantSettings(ScalifySettings):
    ...
    model_config = SettingsConfigDict(env_prefix="scalify_assistant_")
...
class AudioSettings(ScalifySettings):
    """Settings for the audio API."""

    model_config = SettingsConfigDict(env_prefix="scalify_audio_", extra="ignore")

    speech: SpeechSettings = Field(default_factory=SpeechSettings)


class OpenAISettings(ScalifySettings):
    ...
    Example:
        ...
        scalify.settings.openai.api_key = "sk-..."

        assert scalify.settings.openai.api_key.get_secret_value() == "sk-..."
        ```
    """

    model_config = SettingsConfigDict(
        env_prefix="scalify_openai_",
        extra="ignore",
    )
...
class TextAISettings(ScalifySettings):
    model_config = SettingsConfigDict(env_prefix="scalify_ai_text_", extra="ignore")
    generate_cache_token_cap: int = Field(600)


class AISettings(ScalifySettings):
    model_config = SettingsConfigDict(env_prefix="scalify_ai_", extra="ignore")
...
class Settings(ScalifySettings):
    """Settings for `scalify`.

    This is the main settings object for `scalify`.
    ...
    Example:
        ...
        scalify.settings.log_level = "INFO"
    ...
    auto_import_beta_modules: bool = Field(
        True,
        description="If True, the scalify.beta module will be automatically imported when scalify is imported.",
    )
    ...
    def set_log_level(cls, v):
        from scalify.utilities.logging import setup_logging
...
def temporary_settings(**kwargs: Any):
    """
    Temporarily override Scalify setting values, including nested settings objects.
    ...
    Example:
        ...
        with temporary_settings(log_level="INFO"):
            assert scalify.settings.log_level == "INFO"
        assert scalify.settings.log_level == "DEBUG"

        # Override nested settings
        with temporary_settings(openai__api_key="new-api-key"):
            assert scalify.settings.openai.api_key.get_secret_value() == "new-api-key"
        assert scalify.settings.openai.api_key.get_secret_value().startswith("sk-")
        ```
    """
