...
import scalify
import scalify.utilities.openai
import scalify.utilities.tools
from scalify.beta.assistants.handlers import PrintHandler
from scalify.tools.assistants import AssistantTool
from scalify.types import Tool
from scalify.utilities.asyncio import (
    ExposeSyncMethodsMixin,
    expose_sync_method,
    run_async,
    run_sync,
)
from scalify.utilities.jinja import Environment as JinjaEnvironment
...
class Assistant(BaseModel, ExposeSyncMethodsMixin):
    ...
    def default_model(cls, model):
        if model is None:
            model = scalify.settings.openai.assistants.model
        return model
    ...
    def get_tools(self) -> list[AssistantTool]:
        return [
            (
                ...
                else scalify.utilities.tools.tool_from_function(tool)
            )
    ...
    async def say_async(
        ...
        **run_kwargs,
        ...
        from scalify.beta.assistants.runs import Run
    ...
    async def create_async(self, _auto_delete: bool = False):
        ...
        client = scalify.utilities.openai.get_openai_client()
