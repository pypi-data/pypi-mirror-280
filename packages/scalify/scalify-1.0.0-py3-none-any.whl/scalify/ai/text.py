...
import scalify
import scalify.utilities.tools
from scalify._mappings.types import (
    cast_labels_to_grammar,
    cast_type_to_labels,
    cast_type_to_list,
)
from scalify.ai.prompts.text_prompts import (
    CAST_PROMPT,
    CLASSIFY_PROMPT,
    EXTRACT_PROMPT,
    FUNCTION_PROMPT,
...
from scalify.client.openai import (
    AsyncScalifyClient,
    ChatCompletion,
    get_default_async_client,
)
from scalify.types import ChatRequest, ChatResponse, Image
from scalify.utilities.asyncio import run_sync
from scalify.utilities.context import ctx
from scalify.utilities.jinja import Transcript
from scalify.utilities.logging import get_logger
from scalify.utilities.mapping import map_async
from scalify.utilities.python import PythonFunction
...
def prepare_data(data: FN_INPUT_TYPES) -> list[str]:
    ...
    for item in data:
        if isinstance(item, Image):
            ...
            clean_data.append(scalify.utilities.tools.output_to_string(item))
    return "\n".join(clean_data)
...
async def generate_llm_response(
    ...
    client: Optional[AsyncScalifyClient] = None,
) -> ChatResponse:
    """
    ...
    if scalify.settings.log_verbose:
...
def _get_tool_outputs(request: ChatRequest, response: ChatCompletion) -> list[Any]:
    ...
    for tool_call in tool_calls:
        tool_output = scalify.utilities.tools.call_function_tool(
            tools=request.tools,
...
async def _generate_typed_llm_response_with_tool(
    ...
    Args:
        ...
        client (ScalifyClient, optional): The client to use for the AI function.
    ...
    model_kwargs = model_kwargs or {}
    prompt_kwargs = prompt_kwargs or {}
    tool = scalify.utilities.tools.tool_from_type(type_, tool_name=tool_name)
...
async def cast_async(
    ...
    Args:
        ...
        model_kwargs (dict, optional): Additional keyword arguments for the
            language model. Defaults to None.
        client (AsyncScalifyClient, optional): The client to use for the AI
            function.
...
async def classify_async(
    ...
    Args:
        ...
        model_kwargs (dict, optional): Additional keyword arguments for the
            language model. Defaults to None.
        client (AsyncScalifyClient, optional): The client to use for the AI function.
...
async def generate_async(
    ...
    for r in list(cached_responses) if use_cache else []:
        if tokens > scalify.settings.ai.text.generate_cache_token_cap:
            continue
...
def fn(
    ...
    async def async_wrapper(*args, _model_kwargs: dict = None, **kwargs):
        ...
        post_processor = scalify.settings.post_processor_fn
...
def classifier(cls=None, *, instructions=None, model_kwargs=None):
    ...
    `scalify.classify` function instead of the default Enum behavior. This allows
