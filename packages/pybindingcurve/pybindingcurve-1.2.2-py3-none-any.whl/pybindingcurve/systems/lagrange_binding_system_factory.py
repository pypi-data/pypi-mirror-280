...
class LagrangeBindingSystemFactory:
    ...
    def __init__(
        self, system_string, add_nonzero_constraints=False, output_filename=None
    ):
        ...
        [
            self.original_reactions_dictionary[p].append(
                [r1, r2, self._get_correct_kd((r1, r2, p))]
            )
            for r1, r2, p in reactions
        ]

        self.reactions_dictionary = self._simplify_reactions_dictionary(
            self.original_reactions_dictionary
        )
        ...
        self.fundamental_species = [
            ...
            if x in set(self.species) - set(self.reactions_dictionary.keys())
        ]

        assert (
            len(self.fundamental_species) > 0
        ), "Malformed system, no fundamental species"
        assert (
            len(self.reactions_dictionary.keys()) > 0
        ), "Malformed species, no products"
        # Find fundamental species counts in products
        self.fundamental_species_in_products = {}
        for product, counts in self._get_num_fundamental_species_by_products(
            self.original_reactions_dictionary
        ).items():
            for fundamental_species, count in counts.items():
                if (
                    fundamental_species
                    not in self.fundamental_species_in_products.keys()
                ):
                    self.fundamental_species_in_products[fundamental_species] = {}
                if (
                    product
                    not in self.fundamental_species_in_products[
                        fundamental_species
                    ].keys()
                ):
                    self.fundamental_species_in_products[fundamental_species][
        ...
        self.custom_function_arguments = list(
            signature(self.binding_function).parameters.keys()
        )
    ...
    def get_func_string(self):
        ...
        custom_lagrange_definition += (
            ...
            + ", "
            + ", ".join([x[2] for _, r in self.reactions_dictionary.items() for x in r])
            + "):\n"
        )
        ...
        for constraint_num in range(1, len(self.fundamental_species) + 1):
            ...
            line += (
                "+".join(
                    [
                        f"{count}*{product}"
                        for product, count in self.fundamental_species_in_products[
                            self.fundamental_species[constraint_num - 1]
                        ].items()
                    ]
                )
                + ")"
        ...
        if self._add_nonzero_constraints:
            constraints.append(
                f"\t\tnonzero_constraint={'-'.join([f'(constraint{x}-abs(constraint{x}))' for x in range(1, len(self.fundamental_species)+1)])}\n"
            )
        ...
        return_statements.append(
            "+".join(
                [
                    f"X[{i+len(self.fundamental_species)}]*constraint{i+1}"
                    for i in range(len(self.fundamental_species))
                ]
            )
        )
        if self._add_nonzero_constraints:
        ...
        # Write derivation
        custom_lagrange_definition += (
            "\tderivative_function = grad(F) # Gradients of the Lagrange function\n"
        )

        custom_lagrange_definition += (
            "\t"
            + ", ".join([f"{s}" for s in self.fundamental_species])
            + ", "
            + ", ".join(
                [
                    f"lam{x}"
        ...
        custom_lagrange_definition += (
            "\treturn {"
            + ", ".join([f"'{s}':{s}" for s in self.fundamental_species])
            + ", "
            + ", ".join(
                [
                    f"'{mb.split('=')[0].strip()}':{mb.split('=')[1].strip()}"
                    for mb in long_mass_balances
                ]
            )
            + "}\n"
        )
    ...
    def _get_num_fundamental_species_by_products(self, original_reactions_dict):
        ...
        while True:
            ...
            for product, species_dict in fundamental_species_in_products.items():
                non_fundamental = [
                    x for x in species_dict.keys() if x not in self.fundamental_species
                ]
                if len(non_fundamental) == 0:
                    continue
    ...
    def _get_correct_kd(self, reaction):
        ...
        if (
            (not (any(map(lambda x: x.isdigit(), r1))))
            and (not (any(map(lambda x: x.isdigit(), r2))))
            and (not (any(map(lambda x: x.isdigit(), p))))
        ):
        ...
        missing_from_r1 = list(
            set(species_numbers_r2_in_p) - set(species_numbers_r2_in_r1)
        )
        missing_from_r2 = list(
            set(species_numbers_r1_in_p) - set(species_numbers_r1_in_r2)
        )

        if "" in missing_from_r1:
            missing_from_r1.remove("")
        if "" in missing_from_r2:
            missing_from_r2.remove("")

        assert (
            len(missing_from_r1) + len(missing_from_r2) == 1
        ), "A reaction should add one species to another in a clearly understandable way"
                ...
                for i_r, r in enumerate(v):
                    for tuple_it in range(2):
                        if r[tuple_it] in simplified_reactions_dict.keys():
                            ...
                            simplified_reactions_dict[k][i_r][
                                tuple_it
                            ] = f"{replacement[0]}*{replacement[1]}/{replacement[2]}"
            if not something_changed:
                break
