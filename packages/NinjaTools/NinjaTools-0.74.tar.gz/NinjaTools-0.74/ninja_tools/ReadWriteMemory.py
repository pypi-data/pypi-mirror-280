...
class Process(object):
    ...
    def __init__(self, name: str = '', pid: int = -1, handle: int = -1, error_code: str = None):
        ...
        :param handle: The process handles.
    ...
    def get_modules(self) -> List[int]:
        ...
        :return: A list of the process's modules addresses in decimal.
    ...
    def read(self, lp_base_address: int) -> Any:
        ...
        :return: The data from the process's memory if succeed is not raises an exception.
        """
        try:
            ...
            error = {
                'msg': str(error), 'Handle': self.handle, 'PID': self.pid,
    ...
    def readString(self, lp_base_address: int, length: int) -> Any:
        ...
        try:
            ...
            lp_number_of_bytes_read = ctypes.c_ulong(0)
            ctypes.windll.kernel32.ReadProcessMemory(self.handle, lp_base_address, read_buffer, length,
                                                     lp_number_of_bytes_read)
            buffer_array = bytearray(read_buffer)
            found_terminator = buffer_array.find(b'\x00')
            if found_terminator != -1:
                return buffer_array[:found_terminator].decode('utf-8')
            print(f'[ReadMemory/Error]: terminator not found.\naddress: {hex(lp_base_address)}')
            return ""
        except (BufferError, ValueError, TypeError) as error:
            if self.handle:
                self.close()
    ...
    def readByte(self, lp_base_address: int, length: int = 1) -> List[hex]:
        ...
        try:
            ...
            n_size = ctypes.sizeof(read_buffer)
            lp_number_of_bytes_read = ctypes.c_ulong(0)
            return [hex(read_buffer.value) for x in range(length) if
                    ctypes.windll.kernel32.ReadProcessMemory(self.handle, ctypes.c_void_p(lp_base_address + x),
                                                             lp_buffer, n_size, lp_number_of_bytes_read)]
    ...
    def write(self, lp_base_address: int, value: int) -> bool:
        ...
        :return: It returns True if succeed is not it raises an exception.
        """
        try:
    ...
    def writeByte(self, lp_base_address: int, _bytes: List[hex]) -> bool:
        """
        Write data to the process's memory.
        :param lp_base_address: The process' pointer {don't use offsets}.
        :param _bytes: The byte(s) to be written to the process's memory
        :return: It returns True if succeed is not it raises an exception.
        """
        try:
            for x in range(len(_bytes)):
                write_buffer = ctypes.c_ubyte(_bytes[x])
                lp_buffer = ctypes.byref(write_buffer)
                n_size = ctypes.sizeof(write_buffer)
                lp_number_of_bytes_written = ctypes.c_ulong(0)
...
class ReadWriteMemory:
    ...
    def set_privileges():
        ...
        win32security.AdjustTokenPrivileges(token, False, (
            (p[0], 2) if p[0] == win32security.LookupPrivilegeValue(remote_server, "SeBackupPrivilege") or p[
                0] == win32security.LookupPrivilegeValue(remote_server, "SeDebugPrivilege") or p[
                             0] == win32security.LookupPrivilegeValue(remote_server, "SeSecurityPrivilege") else (
                p[0], p[1]) for p in win32security.GetTokenInformation(token, TokenPrivileges)))
