...
from .dag_builder import PkgDAGBuilder, ADV_PARAM_WIDTH, ADV_PARAM_HEIGHT, ADV_PARAM_CHANNEL, \
...
from .update_pbtxt import PBTXT_NAME, ModelConfig, KEY_MAX_BATCH_SIZE
...
class ModifyPackageFiles(object):
    ...
    def __init__(self,
        ...
        self.max_batch_size = int(algo_param_dict['inferenceMaxBatchSize']) \
            if 'inferenceMaxBatchSize' in algo_param_dict else None
    ...
    def modify_model_config(self, model_type: str):
        """
        ...
        if width is not None and height is not None and channel is not None:
            for v in input_names:
                if len(pbtxt.get_shape_by_name(v)) == 4:
                    ...
                    pbtxt.set_model_input_warmup_shape(v, width, height, channel, batch_size)
        ...
        if 'ocrnet' in model_type and width is not None and height is not None:
    ...
    def modify_connect_config(self,
                              ensemble_name: str,
                              model_name: str,
        ...
        self.advanced_parameters.update({ADV_PARAM_MODEL_TYPE: model_type})
        sub_models, extra_models = builder.modify_connect_model(model_name=template_model_name,
                                                                advanced_parameters=self.advanced_parameters)
        bcelogger.info(f"modify connect sub models {sub_models} extra models {extra_models}")
        ...
        if parse_model_name not in sub_models:
            sub_models[parse_model_name] = ""

        return sub_models, extra_models
    ...
    def _modify_parse_yaml(self, ensemble_name: str, model_name: str, template_model_name: str, model_display_name: str):
        """
