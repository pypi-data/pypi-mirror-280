...
def send_request(method: str, headers: dict = None, url: str = '', payload: dict[str, Any] | str = None,
                 files: [str, [str, bytes, str]] = None, json_payload: dict[str, Any] = None) -> Response | dict[
    ...
    try:
        if method == 'post':
            response = requests.post(url, headers=headers, data=payload, files=files, json=json_payload, verify=False,
                                     timeout=3000)
        elif method == 'get':
            response = requests.get(url, headers=headers, verify=False, timeout=3000)
        elif method == 'put':
            response = requests.put(url, headers=headers, data=payload, files=files, json=json_payload, verify=False,
                                    timeout=3000)
        elif method == 'delete':
