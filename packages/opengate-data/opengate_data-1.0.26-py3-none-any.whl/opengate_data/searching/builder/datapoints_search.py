...
class DataPointsSearchBuilder(SearchBuilderBase, SearchBuilder):
    ...
    def with_mapped_transpose(self, mapping: dict[str, dict[str, str]]) -> 'DataPointsSearchBuilder':
        ...
        Example:
            ~~~python
                ...
                builder.with_mapped_transpose('mappingData')
            ~~~
        """
    ...
    def execute(self):
        ...
        Example:
            ~~~python
                builder.with_filter(filtro).with_filter(filter).with_select(select).with_format("pandas").build().execute()
                builder.with_filter(filtro).with_mapped_transpose(mappingData).build().execute()
                builder.with_filter(filtro).with_transpose().build().execute()
            ~~~
        """
        if 'build' in self.method_calls:
