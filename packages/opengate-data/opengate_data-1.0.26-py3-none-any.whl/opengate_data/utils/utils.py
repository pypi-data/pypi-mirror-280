...
def send_request(method: str, headers: dict = None, url: str = '', data: dict[str, Any] | str = None,
    ...
    try:
        if method == 'post':
            response = requests.post(url, headers=headers, data=data, files=files, json=json_payload, verify=False,
                                     timeout=3000)
        elif method == 'get':
