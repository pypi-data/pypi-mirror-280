...
from opengate_data.utils import validate_type, set_method_call, send_request, handle_exception
...
class ProvisionBulkBuilder:
    ...
    def execute(self, include_payload=False):
        ...
        Example:
            ~~~python
                bulk.with_organization_name('example').from_csv('path_to_csv.csv').build().execute(True)
                bulk_xls.with_organization_name('example').from_excel('path_to_excel.xls').build().execute()
                bulk_xlsx.with_organization_name('example').csv('path_to_excel.xlsx').build_execute()
            ~~~
        """
    ...
    def _execute_bulk_provisioning(self, include_payload):
        ...
        if self.method_calls.count('from_excel') > 0:
            files = self.file
            if include_payload:
                raise Exception(f'No se puede añadir un payload para ficheros excel.')
        else:
        ...
        try:
            ...
            response = send_request(method='post', headers=self.client.headers, url=url, data=data, files=files)
