...
def run_command_and_get_output():
    ...
    print(type(output))
    return json.dumps(json.loads(output),separators=(',', ':'))
...
""".format(
    ...
    docker_list.append(add_labels('run.cog.openapi_schema',run_command_and_get_output()))
...
def build_cpu_image(image_tag, config_map, tag, platform="linux/amd64", port=8000):
    ...
    if config_map["build"]["python_requirements"]:
        ...
        docker_list.append(
            add_run(
                "pip install -r {} {}".format('requirements.txt','-i https://pypi.tuna.tsinghua.edu.cn/simple --extra-index-url https://pypi.org/simple/')
            )
        )
...
def build_final_image(config_map, port=8000):
    ...
    if config_map["build"]["gpu"] is True:
        ...
        if framework == "tensorflow":
            ...
            build_docker_image(
                image_tag, config_map, config_map["image"], port=port, install_cmd=None,is_tensorflow=True
            )
