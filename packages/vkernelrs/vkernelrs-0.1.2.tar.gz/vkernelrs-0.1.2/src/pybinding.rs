...
impl PyUserPool {
    #[new]
    pub fn new() -> PyResult<PyUserPool> {
        Ok(PyUserPool(Arc::new(Mutex::new(crate::pool::UserPool::default()))))
    }
}
...
impl PyTerminalSession {
    #[new]
    pub fn new(pool: PyUserPool) -> PyResult<PyTerminalSession> {
        let pool = pool.0.clone();

        Ok(PyTerminalSession {
            session: Mutex::new(
                crate::pool::TerminalSession::new(pool)
    ...
    pub fn write(&self, data: &[u8]) -> PyResult<()> {
        self.session.lock()
            .unwrap()
            .write(data)
            .map_err(|_| PyValueError::new_err("Failed to write to terminal session"))
    }
    ...
    /// 
    pub fn read(&self, py: Python, buffer: usize) -> PyResult<PyObject> {
        let mut buffer = vec![0u8; buffer];
        let result = self.session.lock()
            .unwrap()
            .read_stdout(&mut buffer)
            .map_err(|_| PyValueError::new_err("Failed to read from terminal session"))?;
    ...
    /// 
    pub fn read_err(&self, py: Python, buffer: usize) -> PyResult<PyObject> {
        let mut buffer = vec![0u8; buffer];
        let result = self.session.lock()
            .unwrap()
