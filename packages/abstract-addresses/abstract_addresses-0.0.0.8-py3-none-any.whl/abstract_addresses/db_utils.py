...
class db_path_mgr(metaclass=SingletonMeta):
    def __init__(self,db_dir=None,db_file=None,dbType=None):
        ...
        self.db_path = os.path.join(self.db_dir,db_file) if db_file else selfget_db_path(dbType)
    def get_db_path(dbType=None):
        db_js = {"master_addresses":"master_addresses.db"}
        return os.path.join(self.get_dbs_dir(),db_js.get(dbType or 'master_addresses',db_js.values()[0]))
    def get_dbs_dir():
      return self.db_dir
...
def create_table(cursor=None,conn=None,types_dict=None,db_path=None,dbType=None):
    ...
    return cursor
def insert_key_values(db_insert_js,cursor=None,conn=None,db_path=None,db_type=None,types_dict=None):
    conn = conn or connect_to_db(db_path=db_path,dbType=db_type)
    cursor = cursor or create_table(cursor=cursor,conn=conn,types_dict=types_dict,db_path=db_path,dbType=db_type)
    question_set=[]
    value_set = []
    key_set = ""
    for key,value in db_insert_js.items():
        ...
        if key == 'uuid' and str(value).lower() in ['',[],'nan','none']:
            value = generate_uuid()
        value_set.append(if_types(types_js,value,key) or None)
...
def fetch_all(cursor, columns, values, strict=True):
    ...
    return results
def search_query(address_dict,layer=None,conn=None,db_path=None,dbType=None):
