...
def complementar_e_filtrar_tabelas_contingencia_NEW(data_frame_multi_nivel, funcao_analise_estatistica):
    """
    Complementa tabelas de contingência em um DataFrame multi-nível com análise estatística e filtra resultados específicos.
    Este processo é feito agrupando o DataFrame por todos os níveis de índice, exceto o último, aplicando a análise estatística,
    e finalmente filtrando os resultados por critérios específicos.
...
def criar_tabela_contingencia_expandida_llm(data_df, groups, categories, model):
    """
    Creates an expanded contingency table for statistical analysis, incorporating observed frequencies (Freq), expected frequencies (expected), 
    common residuals (common_residuals), standardized residuals (standardized_residuals), and adjusted standardized residuals (adjusted_standardized_residuals) based on specified groups and categories.
    Indices and columns are renamed to reflect the provided categories for more intuitive interpretation of results.
    
    Parameters:
    - data_df: DataFrame containing the data for analysis.
    - groups: List containing the names of the columns representing the groups for grouping.
    - categories: Dictionary with group columns as keys and lists with category names as values for renaming.
    - model: Model for estimating expected frequencies.
    
    Returns:
    - final_table: DataFrame with the expanded contingency table, including the columns Freq, expected, common_residuals, standardized_residuals, and adjusted_standardized_residuals.
    """

    # Calculating expected frequencies
    data_df['expected'] = model.predict(data_df)
    
    # Calculating common residuals
    data_df['common_residuals'] = data_df['Freq'] - data_df['expected']
    
    # Calculating standardized residuals
    data_df['standardized_residuals'] = data_df['common_residuals'] / np.sqrt(data_df['expected'])
    
    # Adjusting standardized residuals
    n = data_df.shape[0]
    k = len(groups)
    data_df['adjusted_standardized_residuals'] = data_df['standardized_residuals'] / np.sqrt((n - 1) / (n - k))

    data_df['expected'] = data_df['expected'].round(2)
    data_df['common_residuals'] = data_df['common_residuals'].round(2)
    data_df['standardized_residuals'] = data_df['standardized_residuals'].round(2)
    data_df['adjusted_standardized_residuals'] = data_df['adjusted_standardized_residuals'].round(2)

    tables = []
    values = ['Freq', 'expected', 'common_residuals', 'standardized_residuals', 'adjusted_standardized_residuals']

    for value in values:
        contingency_table = pd.crosstab(
            index=[data_df[group] for group in groups[:-1]],
            columns=data_df[groups[-1]],
    ...
    # Combining the tables of Freq, expected, common_residuals, standardized_residuals, and adjusted_standardized_residuals
    final_table = pd.concat(tables, keys=values, axis=1)

    # Updating index levels
    for i, group in enumerate(groups[:-1]):
        new_index = final_table.index.levels[i].map(lambda x: categories[group][x-1])
        final_table.index = final_table.index.set_levels(new_index, level=i)
    
    # Adjusting the names of categories directly without accessing nonexistent levels
    new_columns = [(value, categories[groups[-1]][int(col)-1]) for value in values for col in final_table.columns.get_level_values(1).unique()]
    final_table.columns = pd.MultiIndex.from_tuples(new_columns, names=[None, groups[-1]])

    final_table.columns = pd.MultiIndex.from_tuples([('observed' if col[0] == 'Freq' else col[0], col[1]) for col in final_table.columns])
