    ...
    if mime_type and mime_type.startswith('text'):
        try:
            ...
            try:
                ...
                # If both encodings fail, ignore this file
                return f"[Unable to read {os.path.basename(file_path)}]"
    else:
...
def generate_md_content(path, output_dir, indent='', tree_only=False, relative_path='', single_file=False):
    ...
    for item in sorted(os.listdir(path)):
        ...
        if os.path.isdir(item_path):
            ...
            content.append(f"{indent}- {item}")
            if not tree_only:
                file_content = get_file_content(item_path, output_dir, item_relative_path, single_file)
                if file_content:
                    content.append(f"{indent}  {file_content}")
