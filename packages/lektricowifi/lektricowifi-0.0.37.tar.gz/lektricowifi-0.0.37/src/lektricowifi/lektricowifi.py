...
class Device:
    ...
    async def device_info(self, device_type: str) -> Info:
        ...
        if device_type == self.TYPE_1P7K or device_type == self.TYPE_3P22K:
            ...
            tasks.append(asyncio.create_task(self._request_get("charger_info.get")))
            tasks.append(asyncio.create_task(self._request_get("app_config.get")))
            ...
            # data_info: dict[str, Any] = tasks[0].result()
            # data_dyn: dict[str, Any] = tasks[1].result()
            # print("0:")
            # s = "a"
            # print(type(s))
            # print("1:")
            ...
            # data: dict[str, Any] = dict(data_info, **data_dyn)
            data: dict[str, Any] = dict(a, **b)
            
            if data["has_active_errors"]:
                task1 = asyncio.create_task(self._request_get("active_errors.get"))
                await task1
                data_new = task1.result()
            else:
                data_new = {
            ...
            task1 = asyncio.create_task(self._request_get("dynamic_current.get"))
            await task1
            data_new = task1.result()
            data.update(data_new)
            ...
            task1 = asyncio.create_task(self._request_get("Meter_info.get"))
            task2 = asyncio.create_task(self._request_get("App_config.get"))
            task3 = asyncio.create_task(self._request_get("Sw_version.get"))
            await task1
            await task2
            await task3
            data_info = task1.result()
            data_dyn = task2.result()
            data_new = task3.result()
    ...
    async def device_config(self) -> Settings:
        ...
        # get device's type
        task1 = asyncio.create_task(self._request_get("Device_id.get"))
        await task1
        data = task1.result()
        ...
        if _device_id.startswith(self.TYPE_1P7K):
            ...
            task1 = asyncio.create_task(self._request_get("charger_config.get"))
            await task1
            data = task1.result()
            ...
            task1 = asyncio.create_task(self._request_get("M2w_config.get"))
            await task1
            data = task1.result()
