...
from wa_cloud_py.components.conversational import Command
...
class WhatsApp:
    ...
    def __init__(
        ...
        self.flows_url = (
            f"https://graph.facebook.com/{self.version}/{self.phone_number_id}/flows"
        )
        self.automation_url = f"https://graph.facebook.com/v19.0/{self.phone_number_id}/conversational_automation"
    ...
    def parse(self, request_data: dict) -> Union[UserMessage, None]:
        """
        ...
        Returns:
            Union[InteractiveListMessage, InteractiveButtonMessage, TextMessage, OrderMessage, None]: The parsed message object, or None if no messages were found.
        """
        ...
        if len(msg_value.get("messages", [])) > 0:
            ...
            if message_type == MessageType.TEXT:
                ...
                return LocationMessage(msg_value.get("messages")[0], user=user)

            elif (
                msg_value.get("messages")[0].get("interactive", {}).get("type", None)
                == MessageType.INTERACTIVE_LIST
            ):
                return InteractiveListMessage(msg_value.get("messages")[0], user=user)

            elif (
                msg_value.get("messages")[0].get("interactive", {}).get("type", None)
                == MessageType.INTERACTIVE_BUTTON
            ):
                return InteractiveButtonMessage(msg_value.get("messages")[0], user=user)
    ...
    def send_location_request(self, to: str, text: str) -> Tuple[bool, dict]:
        """
        Sends a location request to the specified phone number using the WhatsApp Cloud API.

        Args:
            to (str): The phone number to send the message to.
            text (str): The text to include with the location request.
    ...
    def send_interactive_buttons(
        ...
        if header:
            data["interactive"]["header"] = header.to_dict()
    ...
    def update_conversational_automation(
        ...
        """
        Updates the conversational components for WhatsApp conversational automation

        Args:
            enable_welcome_message (bool): A boolean indicating whether the welcome message should be enabled.
            commands (list[Command]): A list of Command objects representing the conversational commands to be used on the chat screen.
            prompts (list[str]): A list of prompts to be used in the chat screen.

        Returns:
            Tuple[bool, dict]: A tuple containing a boolean indicating whether the conversational components were updated successfully and
            a dictionary containing the response data.
        """
        ...
        res = requests.post(self.automation_url, headers=self.headers, json=data)

        if res.status_code == 200:
            if self.verbose:
                logger.success(f"Conversational components updated successfully")
            return True, res.json()

        if self.verbose:
            logger.error(
                f"Failed to update conversational components.\nReason: {res.json()}"
            )
        return False, res.json()

    def get_conversational_automation(self):
        """
        Retrieves the current conversational component configuration for WhatsApp.

        Returns:
            Tuple[bool, dict]: A tuple containing a boolean indicating whether the conversational components were retrieved
            successfully and a dictionary containing the response data.
        """

        url = f"https://graph.facebook.com/v19.0/{self.phone_number_id}?fields=conversational_automation"
        ...
        if self.verbose:
            logger.error(
                f"Failed to retrieve conversational components.\nReason: {res.json()}"
            )
        return False, res.json()
