...
def feature_selection(df_feats: pd.DataFrame, labels: dict = None, context: dict = None, external_feat :pd.DataFrame = None):
    ...
    if labels:
        ...
        params = context
        new_params = simple_grid_search(df_train_features, y_train, df_test_features, params, external_feat=external_feat)
        ...
        top_features = features_scoring_selection(df_feats, [], mode='simple', top_k=1,
                                                  strategy='none', pfa_values=context['pfa_value'],external_feat=external_feat)
    return top_features
...
def simple_grid_search(df_base: pd.DataFrame, y_base: list, df_complete: pd.DataFrame, params: dict, external_feat: pd.DataFrame=None):
    ...
    for new_params in tqdm(ParameterGrid(grid_params)):
        if not k_best:
            ...
            top_features = features_simple_selection(df_base, y_base, top_k=new_params['top_k'],external_feat=external_feat)
