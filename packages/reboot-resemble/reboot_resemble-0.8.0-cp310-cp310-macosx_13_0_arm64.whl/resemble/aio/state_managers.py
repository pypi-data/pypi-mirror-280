...
class StateManager(ABC):
    ...
    @abstractmethod
    async def actors(self) -> AsyncIterator[dict[StateTypeName, Set[ActorId]]]:
...
class SidecarStateManager(
        ...
        *,
        ...
        if context.transaction_id is None:
            ...
            ongoing_transaction: Optional[
                StateManager.Transaction
            ] = self._participant_transactions[state_type_name].get(actor_id)
            ...
            assert transaction is not None and not transaction.prepared(
                ...
                f"for state type '{state_type_name}' and actor '{actor_id}'"
            )

        state: Optional[Message] = self._states[state_type_name].get(actor_id)
        ...
        if state is not None and from_constructor:
            logger.error(
                f"Actor '{actor_id}' for state type '{state_type_name}' already constructed"
            )
            ...
            load = self._loads[state_type_name].get(actor_id)
            if load is None:
                load = asyncio.Event()
                self._loads[state_type_name][actor_id] = load

                try:
                    data: Optional[
                        bytes] = await self._sidecar_client.load_actor_state(
                            state_type_name, actor_id
                        )

                    if data is not None:
                        if from_constructor:
                            logger.error(
                                f"Actor '{actor_id}' for state type '{state_type_name}' "
                                "already constructed"
                            )
                            raise SystemAborted(ActorAlreadyConstructed())
                        ...
                        self._states[state_type_name][actor_id] = state
                        ...
                        logger.error(
                            f"Actor '{actor_id}' for state type '{state_type_name}' not "
                    ...
                    del self._loads[state_type_name][actor_id]
                    load.set()
            else:
    ...
    async def _store(
        ...
        *,
        ...
        if effects is not None:
            ...
            assert self._mutator_locks[state_type][actor_id].locked()
        ...
        if effects is not None:
            ...
            if transaction is not None:
                ...
                transaction.actors[state_type].add(actor_id)

            else:
                self._states[state_type][actor_id] = state_copy
                queues = self._streaming_readers[state_type].get(actor_id)

                # Record the presence of this actor.
                self._actors[state_type].add(actor_id)
    ...
    async def transactionally(
        ...
        transaction: Optional[
            StateManager.Transaction
        ] = self._participant_transactions[state_type].get(actor_id)
        ...
        if transaction is not None and transaction.id == context.transaction_id:
            ...
            self._participant_transactions[state_type][actor_id] = transaction

            # Wait for any non-transactional writers!
            await self._mutator_locks[state_type][actor_id].acquire()
    ...
    async def streaming_reader_idempotency_key(
        ...
        try:
            ...
            # added to it anymore.
            if transaction is None:
                self._streaming_readers[state_type_name][actor_id].remove(
                    queue
                )
            else:
    ...
    async def writer(
        ...
        *,
        ...
        if transaction is not None:
            assert self._mutator_locks[state_type_name][actor_id].locked()

        mutator_or_transaction_writer_lock: asyncio.Lock = (
            self._mutator_locks[state_type_name][actor_id]
            if transaction is None else
            self._transaction_writer_locks[state_type_name][actor_id]
        )
    ...
    async def task_workflow(
        ...
        async def complete(
            ...
            async with self._mutator_locks[state_type][actor_id]:
    ...
    async def transaction(
        ...
        *,
        ...
        # "transactionally".
        assert self._participant_transactions[state_type_name][actor_id
                                                              ] == transaction
        ...
        async def complete(response: Message) -> None:
            # It's possible that we've already stored the transaction,
            # e.g., if we had an inline writer, but we need to try and
            # store the response if we have an idempotency key.
            async with transaction.lock:
                if (
        ...
        try:
            ...
            ), f'{state_type_name} {actor_id} is not the transaction coordinator'
    ...
    def check_for_idempotent_mutation(
        ...
        if context.transaction_id is not None:
            transaction = self._participant_transactions[
                context.state_type_name].get(context.actor_id)
    ...
    async def Prepare(
        ...
        transaction_id = uuid.UUID(bytes=request.transaction_id)
        transaction = self._participant_transactions[state_type].get(actor_id)
        if transaction is None:
            raise RuntimeError(
    ...
    async def transaction_participant_commit(
        ...
        async with transaction.lock:
            if not transaction.finished():
                ...
                self._states[state_type][actor_id] = transaction.state
                ...
                if len(transaction.actors) > 0:
                    for (state_type, actor_ids) in transaction.actors.items():
                        self._actors[state_type].update(actor_ids)
                ...
                _ = self._participant_transactions[state_type].pop(actor_id)

                semaphore = self._participant_transactions_semaphore[state_type
                                                                    ][actor_id]
                ...
                assert self._mutator_locks[state_type][actor_id].locked()
                self._mutator_locks[state_type][actor_id].release()
    ...
    async def recover(
        ...
        *,
        ...
        for actor in recover_response.actors:
            self._actors[actor.state_type].add(actor.actor_id)
            for event in self._actors_changed_events:
                event.set()
        ...
        for sidecar_transaction in recover_response.participant_transactions:
            middleware = self._middleware_for_state_type_name(
                ...
                f"'{str(uuid.UUID(bytes=sidecar_transaction.transaction_id))}'",
            )
            ...
            await self._participant_transactions_semaphore[
                transaction.state_type][transaction.actor_id].acquire()
