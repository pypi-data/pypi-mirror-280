...
from resemble.aio.auth.tokens import Token
from resemble.aio.contexts import ReaderContext, WriterContext
from resemble.aio.secrets import SecretNotFoundException, Secrets
from resemble.aio.tasks import TaskEffect
from resemble.integrations.mailgun import (
    MAILGUN_EVENT_API_CONSISTENCY_DELAY_SEC,
)
from resemble.integrations.mailgun.v1.mailgun_rsm import (
    Message,
    SendRequest,
    SendResponse,
    SendTaskRequest,
...
from resemble.v1alpha1.errors_pb2 import Ok, PermissionDenied, Unauthenticated
...
class _AbstractMessageServicer(Message.Interface):
    ...
    class TokenVerifier(resemble.aio.auth.token_verifiers.TokenVerifier):

        def __init__(self, secrets: Optional[Secrets]):
        ...
        async def verify_token(
            ...
            if self._secrets is None:
                # TODO(rjh): remove this case when secrets are also supported on
                #            the Cloud.
                return None
            try:
    ...
    def token_verifier(
        self,
    ) -> Optional[resemble.aio.auth.token_verifiers.TokenVerifier]:
    ...
    async def Send(
        ...
        return Message.SendEffects(
            state=state,
            tasks=[task],
            response=SendResponse(task_id=task.task_id),
        )
...
class MessageServicer(_AbstractMessageServicer):
    ...
    async def get_api_key(self) -> str:
        if self._secrets is None:
            # TODO(rjh): remove this when secrets are also supported on the cloud.
            return ""
        try:
            api_key = await self._secrets.get(MAILGUN_API_KEY_SECRET_NAME)
