...
class ResembleProtocPlugin(ProtocPlugin):
    ...
    def _proto_resemble_services(
        ...
        if len(state_names) == 0:
            # We rely on states, rather than services, to determine which
            # services are resemble services.
            return []
        resemble_services = []
        ...
        def _check_method_options(
            service: ServiceDescriptor, is_resemble_service: bool
        ) -> None:
            for method in service.methods:
                ...
                if is_resemble_service and not is_resemble_method:
                    ...
                    raise ValueError(
                        ...
                        "not a resemble service. (According to the Resemble naming convention, "
                        f"Resemble service should be named as '{state_name}Interface'"
                    )
        ...
        for service in file.services_by_name.values():
            ...
            is_resemble_service = state_name in state_names

            _check_method_options(service, is_resemble_service)

            if is_resemble_service:
                resemble_service_found_by_state_name[state_name] = True
                resemble_services.append(_convert_service_to_proto(service))

        def _find_service_in_dependencies(
            state_name: str, dependencies: list[FileDescriptor]
        ) -> ServiceDescriptor:
            for dep in dependencies:
                service_name = f"{state_name}Interface"
                if service_name in dep.services_by_name:
                    return dep.services_by_name[service_name]

            return None

        for state_name, found in resemble_service_found_by_state_name.items():
            if not found:
                service = _find_service_in_dependencies(
                    state_name, file.dependencies
                )
                if service:
                    resemble_service_found_by_state_name[state_name] = True
                    resemble_services.append(
                        _convert_service_to_proto(service)
                    )
                else:
                    raise ValueError(
                        f"Missing Resemble service for state '{state_name}'. "
                        f"Define a service with the name '{state_name}Interface' and make sure "
                        "it is accessible from the proto file, where your state is defined."
                    )
