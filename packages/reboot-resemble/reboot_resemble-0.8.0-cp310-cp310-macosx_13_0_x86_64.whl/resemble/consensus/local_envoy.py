...
class LocalEnvoy:
    ...
    def __init__(
        ...
        *,
        ...
        certificate = os.environ.get(ENVVAR_LOCAL_ENVOY_TLS_CERTIFICATE_PATH)
        key = os.environ.get(ENVVAR_LOCAL_ENVOY_TLS_KEY_PATH)

        assert certificate is None or key is not None

        self._using_localhost_direct = certificate is None
    ...
    async def _wait_for_grpc_health_check_through_envoy(self, target: str):
        while True:
    ...
    async def _docker_run_envoy(self) -> None:
        ...
        local_envoy_run_command += [
            ...
            f'--volume={self._tmp_envoy_crt}:/etc/envoy/tls.crt',
            f'--volume={self._tmp_envoy_key}:/etc/envoy/tls.key',
        ...
        # TODO: also do a health check for a custom certificate, which
        # will also require knowing or extracting the allowable names
        # to try and connect to with that certificate.
