...
class MuVI(PyroModule):
    ...
    def _setup_model_guide_predict(self, weights, scale_elbo: bool):
        
        self._model_pred = MuVIPredictiveModel(
            self.n_samples,
            n_features=[self.n_features[vn] for vn in self.view_names],
            n_factors=self.n_factors,
            weights=weights,
        ...
        self._guide_pred = MuVIPredictiveGuide(self._model_pred)
    ...
    def _setup_svi(
        ...
        if predictive:
            ...
            guide = self._guide_pred

        svi = pyro.infer.SVI(
            model=pyro.poutine.scale(model, scale=scaler),
            guide=pyro.poutine.scale(guide, scale=scaler),
            optim=optimizer,
            loss=loss,
        )
    ...
    def _setup_training_data(self, observations, covariates=None):
        ...
        """
        train_obs = torch.cat(
            [torch.Tensor(observations[vn]) for vn in self.view_names], 1
        )
        ...
        train_covs = None
        if covariates is not None:
            train_covs = torch.Tensor(covariates)
    ...
    def _fit_predict(
        self,
        observations: Optional[MultiView] = None,
        covariates: Optional[SingleView] = None,
        weights: Optional[MultiView] = None,
        batch_size: int = 0,
        n_epochs: int = 0,
        n_particles: int = 0,
        ...
        **kwargs,
        ...
        predictive = weights is not None
        ...
        if predictive:
            self._setup_model_guide_predict(weights, scale_elbo)
        else:
        ...
        training_data = self._setup_training_data(observations, covariates)
...
class MuVIPredictiveModel(MuVIModel):
    def __init__(
        self,
        n_samples: int,
        n_features: list[int],
    ...
    def forward(
        ...
        for m in range(self.n_views):
            with feature_plates[m]:
                output_dict[f"pred_sigma_{m}"] = pyro.sample(
                    f"pred_sigma_{m}",
                    dist.LogNormal(self._zeros((1,)), self._ones((1,))),
                )

        with sample_plate:
            output_dict["pred_z"] = pyro.sample(
                "pred_z",
                dist.Normal(
                    self._zeros((self.n_factors,)),
                    self._ones((self.n_factors,)),
            ...
            if any(self.nmf):
                output_dict["pred_z"] = self.pos_transform(output_dict["pred_z"])
            for m in range(self.n_views):
                with feature_plates[m]:
                    y_loc = torch.matmul(output_dict["pred_z"], self.weights[m])

                    if self.likelihoods[m] == "normal":
                        y_dist = dist.Normal(
                            y_loc,
                            output_dict[f"pred_sigma_{m}"],
                        )
                    else:
                        y_dist = dist.Bernoulli(logits=y_loc)
                    ...
                    with (
                        ...
                        output_dict[f"pred_y_{m}"] = pyro.sample(
                            f"pred_y_{m}",
                            y_dist,
                            obs=obs[..., feature_idx],
                            infer={"is_auxiliary": True},
...
class MuVIPredictiveGuide(MuVIGuide):
    def __init__(
        self,
        model,
        init_loc: float = 0.0,
    ...
    def mode(self, name: str):
        ...
        if self.site_to_dist[name] == "LogNormal":
            mode = (loc - scale.square()).exp()
        if (name == "pred_z") and any(self.model.nmf):
            mode = self.model.pos_transform(mode)
        return mode.clone()
    ...
    def forward(
        ...
        for m in range(self.model.n_views):
            with feature_plates[m]:
                output_dict[f"pred_sigma_{m}"] = self._sample(f"pred_sigma_{m}")

        with sample_plate as indices:
            output_dict["pred_z"] = self._sample("pred_z", indices)
        return output_dict
