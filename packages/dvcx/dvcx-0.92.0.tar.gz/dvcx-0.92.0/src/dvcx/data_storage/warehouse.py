...
if TYPE_CHECKING:
    ...
    from dvcx.data_storage.schema import DatasetRow as DatasetRowSchema
...
class AbstractWarehouse(ABC, Serializable):
    ...
    @property
    def dataset_row_cls(self) -> type["DatasetRowSchema"]:
        return self.schema.dataset_row_cls
    ...
    def prepare_entries(
        ...
        def _prepare_entry(entry: Entry):
            ...
            return {
                f.name: d.get(f.name)
                for f in self.dataset_row_cls.file_columns()
                if f.name != "id"
            }
    ...
    def _get_nodes_by_glob_path_pattern(
        self, dataset_rows: "DatasetRowSchema", path_list: list[str], glob_name: str
    ) -> Iterator[Node]:
    ...
    def _get_node_by_path_list(
        self, dataset_rows: "DatasetRowSchema", path_list: list[str], name: str
    ) -> Node:
        """
    ...
    def _populate_nodes_by_path(
        self, dataset_rows: "DatasetRowSchema", path_list: list[str]
    ) -> list[Node]:
        """
    ...
    @staticmethod
    def expand_query(dir_expanded_query, dataset_rows: "DatasetRowSchema"):
        dr = dataset_rows
    ...
    def get_node_by_path(self, dataset_rows: "DatasetRowSchema", path: str) -> Node:
    ...
    def expand_path(self, dataset_rows: "DatasetRowSchema", path: str) -> list[Node]:
    ...
    def select_node_fields_by_parent_path_tar(
        self, dataset_rows: "DatasetRowSchema", parent_path: str, fields: Iterable[str]
    ) -> Iterator[tuple[Any, ...]]:
        """
        Gets latest-version file nodes from the provided parent path
        """
