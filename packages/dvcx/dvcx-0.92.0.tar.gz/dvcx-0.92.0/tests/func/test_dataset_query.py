...
from dvcx.dataset import DatasetDependencyType, DatasetStatus
...
def test_udf(cloud_test_catalog):
    ...
    default_schema = {
        ...
        for c in catalog.warehouse.dataset_row_cls.file_columns()
...
def test_row_generator(cloud_test_catalog, dogs_dataset):
    ...
    assert issubclass(schema.pop("id"), Int)
    assert schema == DatasetRow.schema
...
def test_row_generator_with_new_columns(cloud_test_catalog, dogs_dataset):
    ...
    schema = dataset.schema
    id_type = schema.pop("id")
    assert issubclass(id_type, Int)
...
def test_row_generators_sequence_with_new_columns(cloud_test_catalog, dogs_dataset):
    ...
    for res in DatasetQuery(name="upper", catalog=catalog).to_records():
    ...
    for res in DatasetQuery(name="lower", catalog=catalog).to_records():
...
def test_simple_dataset_query(cloud_test_catalog):
    ...
    columns = [c.name for c in warehouse.dataset_row_cls.file_columns()]
    ds1, ds2 = (
        [
            {k: v for k, v in zip(columns, r) if k != "id"}
