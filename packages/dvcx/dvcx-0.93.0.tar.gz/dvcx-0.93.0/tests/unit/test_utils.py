...
DVCX_TEST_PATHS = ["/file1", "file2", "/dir/file3", "dir/file4"]
DVCX_EX_ROOT = ["/file1", "/file2", "/dir/file3", "/dir/file4"]
DVCX_EX_SUBDIR = [
    "subdir/file1",
    "subdir/file2",
    "subdir/dir/file3",
...
@pytest.mark.parametrize(
    "src,paths,expected",
    (
        ("", DVCX_TEST_PATHS, DVCX_EX_ROOT),
        ("/", DVCX_TEST_PATHS, DVCX_EX_ROOT),
        ("/*", DVCX_TEST_PATHS, DVCX_EX_ROOT),
        ("/file*", DVCX_TEST_PATHS, DVCX_EX_ROOT),
        ("subdir", DVCX_TEST_PATHS, DVCX_EX_SUBDIR),
        ("subdir/", DVCX_TEST_PATHS, DVCX_EX_SUBDIR),
        ("subdir/*", DVCX_TEST_PATHS, DVCX_EX_SUBDIR),
        ("subdir/file*", DVCX_TEST_PATHS, DVCX_EX_SUBDIR),
        ("subdir/double", DVCX_TEST_PATHS, DVCX_EX_DOUBLE_SUBDIR),
        ("subdir/double/", DVCX_TEST_PATHS, DVCX_EX_DOUBLE_SUBDIR),
        ("subdir/double/*", DVCX_TEST_PATHS, DVCX_EX_DOUBLE_SUBDIR),
        ("subdir/double/file*", DVCX_TEST_PATHS, DVCX_EX_DOUBLE_SUBDIR),
    ),
)
def test_dvcx_paths_join(src, paths, expected):
    assert list(dvcx_paths_join(src, paths)) == expected
...
def test_determine_processes(parallel, settings, expected):
    if settings is not None:
        os.environ["DVCX_SETTINGS_PARALLEL"] = settings
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (pa.date32(), DateTime),
        (pa.string(), String),
        (pa.struct({"x": pa.int32(), "y": pa.string()}), JSON),
        (pa.map_(pa.string(), pa.int32()), JSON),
        (pa.dictionary(pa.int64(), pa.string()), String),
    ),
)
...
def test_arrow_type_mapper_list():
    col_type = pa.list_(pa.string())
    assert arrow_type_mapper(col_type).to_dict() == {
        "type": "Array",
