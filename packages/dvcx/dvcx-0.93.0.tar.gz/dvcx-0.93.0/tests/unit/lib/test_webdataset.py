...
def test_webdataset_basic():
    ...
    assert laion01.file_info.name == "01.jpg"
    assert laion01.file_info.parent == tar_file.name
    assert laion01.file_info.location is not None
    assert isinstance(laion01.file_info.location, list)
    assert len(laion01.file_info.location) > 0

    parent = laion01.file_info.location[0].get("parent", None)
    assert parent is not None
    parent_file = File(**parent)
    assert parent_file == tar_file

    assert laion64.file_info.name == "64.jpg"
    assert laion64.file_info.parent == tar_file.name
