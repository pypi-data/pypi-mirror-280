...
class BlindSourceSeparation(
    ...
    optional_input_names=["n_comp", "save", "processing_order"],
    output_names=["dataset", "comp", "W"],
):
    def run(self):
        if not isinstance(self.inputs.dataset, dtypes.OWDataset):
            raise TypeError(
                f"'dataset' input should be an instance of OWDataset. Get {type(self.inputs.dataset)}"
            )
