    ...
    : GroptOperator(N, Naxis, dt, 1, Naxis*N, false)
{
    ...
    // Proper SAFE model is not stable in Krlov optimization, Setting to false removes the abs(), which
...
void Op_SAFE::set_params(double _tau1, double _tau2, double _tau3, double _a1, double _a2, double _a3, double _stim_limit, double _g_scale)
{
                           ...
                           int apply_weight, int norm, bool repeat_balance)
{
    ...
    stim1.setZero();
    for (int j = 0; j < Naxis; j++) {
        stim1(j*N+N-1) = alpha1(j) * X(j*N+N-1);
        for (int i = N-2; i >= 0; i--) {
            stim1(j*N+i) = alpha1(j) * X(j*N+i) + (1-alpha1(j)) * stim1(j*N+i+1);
        }
    }
    if (true_safe) {
        for (int i = 0; i < X.size(); i++) {
    ...
    stim2.setZero();
    for (int j = 0; j < Naxis; j++) {
        stim2(j*N+N-1) = alpha2(j) * abs(X(j*N+N-1));
        for (int i = N-2; i >= 0; i--) {
            stim2(j*N+i) = alpha2(j) * abs(X(j*N+i)) + (1-alpha2(j)) * stim2(j*N+i+1);
        }
    }
    if (!true_safe) {
        for (int i = 0; i < X.size(); i++) {
    ...
    stim3.setZero();
    for (int j = 0; j < Naxis; j++) {
        stim3(j*N+N-1) = alpha3(j) * X(j*N+N-1);
        for (int i = N-2; i >= 0; i--) {
            stim3(j*N+i) = alpha3(j) * X(j*N+i) + (1-alpha3(j)) * stim3(j*N+i+1);
        }
    }
    if (true_safe) {
        for (int i = 0; i < X.size(); i++) {
