    ...
    def OnQuoteDataSyn(self, SymbolType, DataType, QuoteData, bstrModifiedFields):
        ...
        if DataType==1 and SymbolType!=8: #实时
            ...
            if atm_1C and atm_1P and optlist:
                self.onATM(DataType,quote.GetStringData("Symbol"),{"Symbol":quote.GetStringData("Symbol"),"ATM":atm_0,"OTM-1C":atm_1C,"OTM-1P":atm_1P,"OPTLIST":(optlist.strip("|").split("|"))})
    ...
    def subquote(self,datatype:int,symbol:str,startime:int,endtime:int):#"-1" :超過上限 "-10" 無報價權限
        ...
        try:
            ...
            if (datatype==9 or datatype==10 or datatype==19 or datatype==820 or datatype==800) and ("TC.F" in symbol or "TC.S" in symbol):
                ...
                if re.find("-")==0 and self.debuglevel:
                    self.writelog("subquote:{0},{1},{2},{3}".format(datatype,symbol,startime,endtime,re))
                time.sleep(0.5)
