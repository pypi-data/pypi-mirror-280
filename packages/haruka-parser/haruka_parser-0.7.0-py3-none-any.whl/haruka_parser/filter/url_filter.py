...
from ._import_utils import ASSETS_PATH
...
def get_list(abs_path: str, file_name: str, extra: set, do_normalize: bool = True):
    if file_name.endswith(".gz"):
        with gzip.open(os.path.join(abs_path, file_name), "rt") as f:
...
class URLFilter(BaseFilter):
    ...
    def download_data(self):
        ...
        self.block_listed_domains = get_list(
            ASSETS_PATH, "adult/domains.gz", self.block_listed_domains, do_normalize=False
        )
        self.block_listed_url = get_list(ASSETS_PATH, "adult/urls", self.block_listed_url, do_normalize=False)
