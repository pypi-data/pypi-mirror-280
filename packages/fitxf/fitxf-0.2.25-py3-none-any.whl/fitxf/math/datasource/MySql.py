...
from nwae.math.datasource.DatastoreInterface import DatastoreInterface, DbParams
...
class MySql(DatastoreInterface):
    ...
    def __connect(
        ...
        self.con = mysql.connector.connect(
            ...
            password = self.db_params.db_username,
    ...
    def get(
        ...
        query = match_phrase
        self.logger.debug(
            'Get/Query from table "' + str(tablename) + '", match condition = ' + str(match_condition)
            + ', for query ' + str(query)
        )

        colnames = self.get_colnames(tablename=tablename)

        sql = 'SELECT * FROM `' + str(tablename) + '` WHERE '
        for i, (k, v) in enumerate(match_phrase.items()):
            if i > 0:
                sql += ' ' + str(match_condition) + ' '
            sql += '`' + str(k) + '`' + " = '" + str(v) + "'"
...
if __name__ == '__main__':
    Env.set_env_vars_from_file(env_filepath=EnvRepo().REPO_DIR + '/.env.nwae.math.ut.mysql')
