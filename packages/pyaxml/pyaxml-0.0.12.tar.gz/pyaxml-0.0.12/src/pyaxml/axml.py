...
class StringBlocks:
    ...
    def compute(self, update_size=False):
        ...
        self.proto.hnd.styleoffset = self.proto.hnd.stringoffset + len(b"".join(StringBlock(proto=elt, utf8=self.proto.hnd.flag & axml_pb2.UTF8_FLAG == axml_pb2.UTF8_FLAG).pack() for elt in self.proto.stringblocks))
    ...
    def from_axml(buff : bytes, proto=None):
        ...
        if len(str_blocks.proto.stringoffsets) > 0:
            ...
            last_str = StringBlock.from_axml(temp_buff[str_blocks.proto.stringoffsets[-1]:], str_blocks.proto.hnd.flag & axml_pb2.UTF8_FLAG == axml_pb2.UTF8_FLAG)[0]
            for i in range(str_blocks.proto.hnd.len_stringblocks-1):
                stbl , _ = StringBlock.from_axml(temp_buff[str_blocks.proto.stringoffsets[i]:str_blocks.proto.stringoffsets[i+1]], str_blocks.proto.hnd.flag & axml_pb2.UTF8_FLAG == axml_pb2.UTF8_FLAG)
                str_blocks.proto.stringblocks.append(stbl.proto)
            str_blocks.proto.stringblocks.append(last_str.proto)
    ...
    def from_axml(buff : bytes, proto=None):
        ...
        while len(buff) > 0:
            ...
            if hnd.proto.type == axml_pb2.RES_XML_START_ELEMENT_TYPE:
                _, buff = RES_XML_START_ELEMENT.from_axml(buff, proto=elt.start_elt)
            elif hnd.proto.type == axml_pb2.RES_XML_END_ELEMENT_TYPE:
                _, buff = RES_XML_END_ELEMENT.from_axml(buff, proto=elt.end_elt)
            elif hnd.proto.type == axml_pb2.RES_XML_START_NAMESPACE_TYPE:
                _, buff = RES_XML_START_NAMESPACE.from_axml(buff, proto=elt.start_ns)
            elif hnd.proto.type == axml_pb2.RES_XML_END_NAMESPACE_TYPE:
                _, buff = RES_XML_END_NAMESPACE.from_axml(buff, proto=elt.end_ns)
            else:
