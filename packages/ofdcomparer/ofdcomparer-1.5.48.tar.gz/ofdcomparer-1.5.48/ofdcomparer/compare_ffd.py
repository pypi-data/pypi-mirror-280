...
class ComparerOfd:
    ...
    def compare_fn_ofd(self, etalon: dict, changes: dict = None,
        ...
        comparable = self.OfdHelper.get_fd_from_ofd(fd=fd if fd is not None else self.DTO10.get_last_fd_number(),
                                                    ofd=ofd, rnm=rnm, fn=fn)
        if comparable is None:
            raise GetFdOfdError
    ...
    def __cast_bytes(self, key, etalon, comparable):
        ...
        """
        if self.__is_base64(etalon):
            self.etalon_value = base64.b64decode(etalon).hex()
        if self.__is_base64(comparable):
            self.comparable_value = base64.b64decode(comparable).hex()
        return True
