...
class RuntimeContext(AbstractTypeContext):
    ...
    @staticmethod
    def get_instance(runtime_name: RuntimeName, connection_type: ConnectionType, tcp_connection_data: TcpConnectionData = None):
        if connection_type == ConnectionType.InMemory:
            if runtime_name in RuntimeContext.__memory_runtime_contexts:
                ...
                runtime_ctx = RuntimeContext(runtime_name, connection_type, "")
                RuntimeContext.__memory_runtime_contexts[runtime_name] = runtime_ctx
                return runtime_ctx
        if connection_type == ConnectionType.Tcp:
            if (runtime_name, tcp_connection_data) in RuntimeContext.__network_runtime_contexts:
                runtime_ctx = RuntimeContext.__network_runtime_contexts.get((runtime_name, tcp_connection_data))
                runtime_ctx.current_command = None
                return runtime_ctx
            else:
                runtime_ctx = RuntimeContext(runtime_name, connection_type, tcp_connection_data)
                RuntimeContext.__network_runtime_contexts[(runtime_name, tcp_connection_data)] = runtime_ctx
                return runtime_ctx

    def __init__(self, runtime_name: RuntimeName, connection_type: ConnectionType, tcp_connection_data: TcpConnectionData):
        self.__isExecuted = False
        self.__runtime_name = runtime_name
        self.__connection_type = connection_type
        self.__tcp_connection_data = tcp_connection_data
    ...
    def execute(self):
        ...
        self.__response_command = self.__python_interpreter.execute(self.__current_command, self.__connection_type,
                                                                    self.__tcp_connection_data)
        self.__current_command = None
    ...
    def get_type(self, type_name: str, *args):
        ...
        self.__current_command = None
        return InvocationContext(self.__runtime_name, self.__connection_type, self.__tcp_connection_data,
