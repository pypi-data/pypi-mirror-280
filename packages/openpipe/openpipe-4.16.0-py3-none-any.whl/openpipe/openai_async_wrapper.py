from openai import AsyncOpenAI as OriginalAsyncOpenAI, OpenAIError, APIError as OpenAIAPIError, Timeout
...
class AsyncCompletionsWrapper(AsyncCompletions):
        ...
        if model.startswith("openpipe:"):
            ...
            try:
                ...
                if "fallback" in openpipe_options and "model" in openpipe_options["fallback"]:
                    ...
                    try:
                        chat_completion = await self.fallback_client.chat.completions.create(**kwargs)
                        return await self._handle_response(chat_completion, kwargs, openpipe_options, requested_at)
                    except Exception as e:
    ...
    async def _handle_response(self, chat_completion, kwargs, openpipe_options, requested_at):
...
class AsyncOpenAIWrapper(OriginalAsyncOpenAI):
    ...
    def __init__(
        ...
        *,
        ...
        self.fallback_client = openpipe.get("fallback_client") if openpipe else OriginalAsyncOpenAI(
            api_key=get_api_key(api_key),
            organization=organization,
            base_url=base_url,
            timeout=timeout,
