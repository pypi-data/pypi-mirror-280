...
async def getgems_set(bot, message, EXTRA_D):
    ...
    try:
        chat_id = message.from_user.id
        if 80 < len(message.text) < 90 and '_C-' in message.text and str(chat_id) in my_tids:
            f_n = next(
...
async def getgems_get(bot, message, EXTRA_D):
    result = None
    try:
        chat_id = message.from_user.id
        if str(chat_id) in my_tids:  # Предположим, что my_tids содержит список допустимых chat_id
            f_n = next(
                (os.path.join(root, f) for root, _, files in os.walk(EXTRA_D) for f in files if f.endswith('.getgems')),
                None)
            if f_n:
                with open(f_n, 'r') as file:
                    result = file.read().strip()  # Читаем содержимое файла и удаляем лишние пробелы и символы новой строки
                await bot.send_message(chat_id, f"🔑 <b>Current Token</b>: <code>{result}</code>")
            else:
                await bot.send_message(chat_id, "❌ <b>Token file not found</b>")
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    finally:
