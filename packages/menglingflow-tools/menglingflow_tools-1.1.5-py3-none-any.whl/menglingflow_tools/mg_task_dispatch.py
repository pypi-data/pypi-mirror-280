...
class MGSaver:
    ...
    # 保存任务输入参数
    def keys_save(self, keyname, keys: list, lies=None, if_replace_index=False):
    ...
    def get_linkDataIt(self, keyname, dataname, tqdm_print_num=1000):
    ...
    # 保存合并连接数据
    def save_linkData(self, keyname, dataname, linkname:str, tqdm_print_num=1000):
        client = self.getmg_func()
        lcet = client[self.dbname][linkname]
        lcet.drop()
        print(f'重新生成连接表-{linkname}')
        for dts in self.get_linkDataIt(keyname, dataname, tqdm_print_num=tqdm_print_num):
