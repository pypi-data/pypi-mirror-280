...
class VehiclePIDController():
    ...
    def __init__(self, vehicle, dt=0.2, args_lateral=None, args_longitudinal=None, offset=0, max_throttle=0.75, max_brake=0.3,
                 max_steering=0.8):
        """
        Constructor method.
        ...
        args_lateral = _args_lateral_dict if args_lateral is None else args_lateral
        args_longitudinal = _args_longitudinal_dict if args_longitudinal is None else args_lateral
    ...
    def run_step(self, target_speed, target_transform):
        ...
        current_steering = self._lat_controller.run_step(target_transform)
    ...
    def get_fallback_control(self, brake=None):
        ...
        :return: control
        '''
        brake = self.max_brake if brake is None else brake
        control = carla.VehicleControl()
        control.steer = 0.0
        control.throttle = 0.0
        control.brake = brake
    ...
    def get_constant_control(self, acceleration, steer):
        control = carla.VehicleControl()
        if acceleration >= 0.0:
            control.throttle = min(acceleration, self.max_throt)
            control.brake = 0.0
