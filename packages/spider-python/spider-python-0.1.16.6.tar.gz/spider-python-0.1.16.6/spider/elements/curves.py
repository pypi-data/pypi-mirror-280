...
class BasePolynomial(ExplicitCurve):
    def __init__(self, coef=None, valid_x_range=None):
        ...
        if coef is None:
            ...
            # for index in range(1,4): # 先求1-3阶导数
            #     self.derivative_coef(order=index)
...
class myCubicSpline(InterpolationCurve):
    ...
    def interpolate(self, x, order):
        ...
        if order == 0:
            ...
            val = 0.
        # if order == 0:
        #     val = self.a[index] + self.b[index] * dx + self.c[index] * dx ** 2.0 + self.d[index] * dx ** 3.0
        # elif order == 1:
        #     val = self.b[index] + 2.0 * self.c[index] * dx + 3.0 * self.d[index] * dx ** 2.0
        # elif order == 2:
        #     val = 2.0 * self.c[index] + 6.0 * self.d[index] * dx
        # elif order == 3:
        #     val = 6.0 * self.d[index]
        # else:
        #     val = 0.
        return val
...
#             tb = (self.a[index + 1] - self.a[index]) / h[index] - h[index] * \
#                  (self.c[index + 1] + 2.0 * self.c[index]) / 3.0
#             self.b.append(tb)
#
#     def calc(self, t):
#         """
...
#         elif t > self.x[-1]:
#             return None
#         index = self._search_index(t)
#         dx = t - self.x[index]
#         result = self.a[index] + self.b[index] * dx + self.c[index] * dx ** 2.0 + self.d[index] * dx ** 3.0
#
#         return result
#
#     def calcd(self, t):
...
#         index = self._search_index(t)
#         dx = t - self.x[index]
#         result = self.b[index] + 2.0 * self.c[index] * dx + 3.0 * self.d[index] * dx ** 2.0
#         return result
#
#     def calcdd(self, t):
#         """
...
#         index = self._search_index(t)
#         dx = t - self.x[index]
#         result = 2.0 * self.c[index] + 6.0 * self.d[index] * dx
#         return result
#
#     def _search_index(self, x):
#         """
...
#         for index in range(self.nx - 1):
#             if index != (self.nx - 2):
#                 A[index + 1, index + 1] = 2.0 * (h[index] + h[index + 1])
#             A[index + 1, index] = h[index]
#             A[index, index + 1] = h[index]
#
#         A[0, 1] = 0.0
#         A[self.nx - 1, self.nx - 2] = 0.0
#         A[self.nx - 1, self.nx - 1] = 1.0
...
#             B[index + 1] = 3.0 * (self.a[index + 2] - self.a[index + 1]) / \
#                        h[index + 1] - 3.0 * (self.a[index + 1] - self.a[index]) / h[index]
#         #  print(B)
#
#         return B
...
class BezierCurve(ParametricCurve):
    ...
    def __init__(self, control_points, pre_calculation=True):
        ...
        #         self._binom_coeff_dict[(self.n, index)] = val
        #     # 提前计算
