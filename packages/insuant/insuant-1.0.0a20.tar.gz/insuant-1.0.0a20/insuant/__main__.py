...
from insuant.main import setup_app
from insuant.services.database.utils import session_getter
from insuant.services.deps import get_db_service
from insuant.services.utils import initialize_services
from insuant.utils.logger import configure, logger
from insuant.utils.util import update_settings
...
@app.command()
def run(
    host: str = typer.Option("127.0.0.1", help="Host to bind the server to.", envvar="INSUANT_HOST"),
    ...
    log_file: Path = typer.Option("logs/insuant.log", help="Path to the log file.", envvar="LANGFLOW_LOG_FILE"),
...
def print_banner(host: str, port: int):
    ...
    try:
        from insuant.version import __version__ as version  # type: ignore

        is_pre_release |= is_prerelease(version)  # Update pre-release status
        notice = build_version_notice(version, "insuant")
        notice = stylize_text(notice, "insuant", is_pre_release)
        if notice:
            notices.append(notice)
        package_names.append("insuant")
        package_name = "insuant"
    except ImportError:
        version = None

    # Attempt to handle langflow-base similarly
    if version is None:  # This means insuant.version was not imported
        try:
            from importlib import metadata

            langflow_base_version = metadata.version("insuant")
            is_pre_release |= is_prerelease(langflow_base_version)  # Update pre-release status
            notice = build_version_notice(langflow_base_version, "insuant")
            notice = stylize_text(notice, "insuant", is_pre_release)
            if notice:
                notices.append(notice)
            package_names.append("insuant")
...
def run_langflow(host, port, log_level, options, app):
    ...
    try:
        if platform.system() in ["Windows"]:
            ...
            from insuant.server import InsuantApplication

            InsuantApplication(app, options).run()
    except KeyboardInterrupt:
        pass
...
def superuser(
    ...
    with session_getter(db_service) as session:
        from insuant.services.auth.utils import create_super_user
...
presto_catalog = os.getenv('PRESTO_CATALOG')
presto_schema = os.getenv('PRESTO_SCHEMA')
presto_timezone = os.getenv('PRESTO_TIMEZONE')

mongodb_uri = os.getenv('mongodb://localhost:27017/bank')
