...
from insuant.field_typing import Text
...
class RunnableExecComponent(CustomComponent):
    ...
    def build(
        ...
        questions = AIModel.prompt(questionPrompt.format(question=input_value))
        print(questions)
        arrOfQuestions = ast.literal_eval(questions.text())
        print(arrOfQuestions)
        for question in arrOfQuestions:
            prompt = "Please make sure your SQL query includes only schema column names; avoid using any illusionary column names. Additionally, refrain from using semicolons in your query. Strictly Don't use semicolons. Answer the user Question: " + question
            print(prompt)
            input_dict, status = self.get_input_dict(runnable, input_key, prompt)
            result = runnable.invoke(input_dict)
            result_value, _status = self.get_output(result, input_key, output_key)
            status += _status
            status += f"\n\nOutput: {result_value}\n\nRaw Output: {result}"
            self.status = status
            answers[question] = str(result_value)
        json_string = json.dumps(answers, indent=4)
        json_obj = json.loads(json_string)
        for index, (key, value) in enumerate(json_obj.items(), start=1):
            finalAnswer += f"{index}. {value} \n"
        return finalAnswer
