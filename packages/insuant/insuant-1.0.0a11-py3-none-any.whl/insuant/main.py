...
from insuant.database import get_db, get_presto_db
...
def setup_app(static_files_dir: Optional[Path] = None, backend_only: bool = False) -> FastAPI:
    ...
    return app
def main():
    # Example usage of PostgreSQL session
    with get_db() as db:
        results = db.query.all()
        print(results)

    # Example usage of Presto session
    with get_presto_db() as presto_db:
        presto_results = presto_db.execute().fetchall()
