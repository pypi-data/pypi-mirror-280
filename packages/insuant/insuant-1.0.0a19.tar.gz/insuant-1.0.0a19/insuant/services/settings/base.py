...
from insuant.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
...
class Settings(BaseSettings):
    ...
    STORE_URL: Optional[str] = "https://api.insuant.store"
    DOWNLOAD_WEBHOOK_URL: Optional[str] = (
        "https://api.insuant.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    )
    LIKE_WEBHOOK_URL: Optional[str] = "https://api.insuant.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"
    ...
    def set_database_url(cls, value, values):
        if not value:
            ...
            if langflow_database_url := os.getenv("LANGFLOW_DATABASE_URL"):
                ...
                # Originally, we used sqlite:///./insuant.db
                # so we need to migrate to the new format
                # if there is a database in that location
                if not values["CONFIG_DIR"]:
                    raise ValueError("CONFIG_DIR not set, please set it or provide a DATABASE_URL")
                from insuant.version import is_pre_release  # type: ignore

                pre_db_file_name = "langflow-pre.db"
                db_file_name = os.getenv('POSTGRES_URL')
