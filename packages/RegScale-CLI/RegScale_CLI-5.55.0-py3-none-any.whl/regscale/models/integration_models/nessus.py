...
from regscale.models.integration_models.tenable import Plugin, TenableBasicAsset, TenablePort, TenableScan
...
class NessusReport(BaseModel, IntegrationIssue):
    ...
    @staticmethod
    def process_files(file_collection, regscale_ssp_id):
    ...
    def create_regscale_vuln(**kwargs: dict) -> Tuple[List["NessusReport"], List[ScanHistory]]:
        ...
        asset = None
        res = [asset for asset in existing_assets if asset["otherTrackingNumber"] == report.asset.uuid]
        if res:
            asset = res[0]
        ...
        if scans := [
            ...
            regscale_vuln = Vulnerability(
                ...
                daysOpen=None,
                dns=report.asset.hostnames,
                ipAddress=report.asset.ipv4s,
                mitigated=None,
                operatingSystem=(report.asset.operating_systems[0] if report.asset.operating_systems else None),
                ...
                title=f"{report.output} on asset {report.asset.hostnames}",
        ...
        for file in filelst:
            with open(file, "rb") as fp:
                try:
                    ...
                    continue
                for host in {vuln.asset.hostnames for vuln in vulns if vuln.asset}:
                    if host not in res:
                        res[host] = []
                    # Filter out the info vulns
                    res[host].extend([vuln.dict() for vuln in vulns if vuln.asset.hostnames == host])
                del vulns
                gc.collect()
        # tmp_dir.close()
        log_memory()
