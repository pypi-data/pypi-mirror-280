...
from cifkit.utils.cif_parser import (
    check_atom_site_label_error,
)


class CifEnsemble:
    def __init__(
        self, cif_dir_path: str, preprocess=True, add_nested=False
    ) -> None:

        if preprocess:
            # Process each file, handling exceptions that may occur
            # Here, each CIF is initialized but not used.
            for file_path in get_file_paths(
                cif_dir_path, add_nested_files=add_nested
            ):
                try:
                    remove_author_loop(file_path)
                    preprocess_label_element_loop_values(file_path)
                    check_atom_site_label_error(file_path)
                except Exception as e:
                    print(f"Error processing {file_path}: {e}")

            # Move ill-formatted files after processing, decide to add nested files
            move_files_based_on_errors(cif_dir_path, add_nested)
        ...
        self.cifs: list[Cif] = [
            Cif(file_path) for file_path in self.file_paths
        ]
