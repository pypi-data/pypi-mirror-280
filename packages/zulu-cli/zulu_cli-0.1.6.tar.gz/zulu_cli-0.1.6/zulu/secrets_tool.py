...
def derive_key_and_iv(password: str, salt: bytes, key_length: int, iv_length: int) -> (bytes, bytes):
...
def escape_special_characters(text: str) -> str:
    """Escape special characters in a string for YAML compatibility."""
...
def decrypt_yaml_values(data: Union[dict, list], password: str) -> Union[dict, list]:
    if isinstance(data, dict):
        decrypted = {key: decrypt_yaml_values(value, password) for key, value in data.items()}
        return decrypted
    elif isinstance(data, list):
        return [decrypt_yaml_values(item, password) for item in data]
    elif isinstance(data, str):
        try:
            return decrypt_value(data, password).strip()
        except Exception:
            return data
    else:
        return data
...
def encrypt_yaml_values(data: Union[dict, list], password: str) -> Union[dict, list]:
    if isinstance(data, dict):
        return {key: encrypt_yaml_values(value, password) for key, value in data.items()}
    elif isinstance(data, list):
