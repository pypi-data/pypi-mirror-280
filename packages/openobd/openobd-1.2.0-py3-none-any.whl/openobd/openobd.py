...
from openobd_protocol.Session.Messages import BusConfiguration_pb2 as grpcBusConfiguration
from openobd_protocol.Messages import Status_pb2 as grpcStatus
...
from openobd_protocol.SessionController.Messages import Session_pb2 as grpcSession
...
from openobd_protocol.ConnectionMonitor.Messages import ConnectionInformation_pb2 as grpcConnectionInformation
...
class OpenOBDSession(GrpcChannel):
    ...
    def __init__(self, session_info: grpcSession.SessionInfo, grpc_port=443):
        ...
        self.rds = grpcService.sessionStub(self.channel)
    ...
    def open_stream(self, user_interface_messages: Iterator[grpcUserInterface.Control]) -> Iterator[grpcUserInterface.Control]:
        """
        ...
        """
        return self.ui.openStream(user_interface_messages, metadata=self._metadata())
    ...
    def get_connector_information(self) -> grpcConnectionInformation.ConnectorInformation:
        ...
        """
        return self.connector_monitor.getConnectorInformation(request=grpcEmpty.EmptyMessage(), metadata=self._metadata())
    ...
    def stream_connector_information(self) -> Iterator[grpcConnectionInformation.ConnectorInformation]:
        ...
        """
        return self.connector_monitor.streamConnectorInformation(request=grpcEmpty.EmptyMessage(), metadata=self._metadata())

    def finish(self, service_result: grpcServiceResult.ServiceResult) -> grpcStatus.Status:
        """
...
class OpenOBD(object):
    ...
    def start_session_on_connector(self, connector_id: str) -> OpenOBDSession:
        response = self.session_controller.start_session_on_connector(grpcSession.ConnectorId(value=connector_id))
        return OpenOBDSession(response)

    def start_session_on_ticket(self, ticket_id: str) -> OpenOBDSession:
        response = self.session_controller.start_session_on_ticket(grpcSession.TicketId(value=ticket_id))
        return OpenOBDSession(response)
...
class OpenOBDSessionController(GrpcChannel):
    ...
    def start_session_on_ticket(self, ticket_id: grpcSession.TicketId) -> grpcSession.SessionInfo:

        return self.session_controller.startSessionOnTicket(ticket_id, metadata=self._metadata())

    def start_session_on_connector(self, connector_id: grpcSession.ConnectorId) -> grpcSession.SessionInfo:
