...
class FilterConverter(Protocol[TExpr]):
    ...
    def greatest(self, a: TExpr, b: TExpr) -> TExpr:
        """Evaluates to max(a, b) unless a or b is null, then evaluates to null."""
        ...

    def least(self, a: TExpr, b: TExpr) -> TExpr:
        """Evaluates to min(a, b) unless a or b is null, then evaluates to null."""
        ...

    def coalesce(self, a: TExpr, b: TExpr) -> TExpr:
        """
        Evaluates to `a if a is not None else b`, optimized for vectorized execution.
        """
        ...
...
class PolarsFilterConverter(FilterConverter["pl.Expr"]):
    ...
    def greatest(self, a: pl.Expr, b: pl.Expr) -> pl.Expr:
        import polars as pl

        return pl.max_horizontal(a, b)

    def least(self, a: pl.Expr, b: pl.Expr) -> pl.Expr:
        import polars as pl

        return pl.min_horizontal(a, b)

    def coalesce(self, a: pl.Expr, b: pl.Expr) -> pl.Expr:
        import polars as pl
