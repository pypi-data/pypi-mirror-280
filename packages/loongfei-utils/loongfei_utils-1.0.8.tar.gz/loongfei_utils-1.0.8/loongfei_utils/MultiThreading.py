...
class ExecuteList:
    ...
    def __init__(self, func:FunctionType, need_execute_list:list, thread_num:int, logger:object):
        ...
        """
        self.__function__ = func  # 处理序列数据的方法
        self.__need_execute_list__ = need_execute_list
        self.__thread_num__ = thread_num
        self.logger = logger

        self.__executing_list__ = []  # 正在处理中的数据序列
        self.__execute_error_list__ = []  # 处理错误的数据序列
        self.__thread_list__ = []  # 正在执行的线程列表
        self.lock = threading.Lock()

    def start_thread(self):
        if len(self.__need_execute_list__) < self.__thread_num__:
            self.__thread_num__ = len(self.__need_execute_list__)
            ...
            if execute_list_item != "":
                ...
                try:
                    ...
                    if not execute_result:
                        self.__getting_executing_data__("3", execute_list_item)
                        self.logger.error("---> 线程：“" + thread_name + "” ，处理数据 “" + str(execute_list_item) + "” 后，处理方法返回错误，请排查处理方法~~~")
                    else:
