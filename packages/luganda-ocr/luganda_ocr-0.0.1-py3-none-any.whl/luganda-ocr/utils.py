...
def single_sample_prediction(model, image_path):
...
def load_image_files(folder_path):
    image_files = [os.path.join(folder_path, filename) for filename in os.listdir(folder_path) if filename.endswith(".png")]
...
def batch_prediction(model, folder_path):
    model_path = "luganda_ocr/models/sentenceModel.h5"
