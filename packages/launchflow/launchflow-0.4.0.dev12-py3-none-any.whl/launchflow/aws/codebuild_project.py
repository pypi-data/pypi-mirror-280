...
class Source(Inputs):
    ...
    def __post_init__(self):
        if self.buildspec_path is not None and type == "NO_SOURCE":
            raise ValueError("buildspec_path must be None when type is NO_SOURCE")
...
@dataclasses.dataclass
class CodeBuildProjectInputs(Inputs):
...
class CodeBuildProject(Resource[CodeBuildProjectInputs]):
    def __init__(
        self,
        name: str,
        *,
        ...
        super().__init__(name=name, product=ResourceProduct.AWS_CODEBUILD_PROJECT)
    ...
    def inputs(self, environment_type: EnvironmentType) -> CodeBuildProjectInputs:
    ...
    def outputs(self) -> CodeBuildProjectOutputs:
        return CodeBuildProjectOutputs()

    async def outputs_async(self) -> CodeBuildProjectOutputs:
        return CodeBuildProjectOutputs()
