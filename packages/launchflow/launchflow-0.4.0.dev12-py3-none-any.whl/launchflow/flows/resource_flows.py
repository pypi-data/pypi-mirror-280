...
async def _destroy_resource(
    ...
    async with lock as lock_info:
        ...
        logs_file = None
        task_description = (
            f"Destroying [light_goldenrod3]{resource_ref}[/light_goldenrod3]..."
        )
        ...
        try:
            ...
            if verbose:
                _dump_verbose_logs(logs_file, f"Destroy {resource_ref} logs")
            progress.console.print(
                f"[green]✓[/green] [light_goldenrod3]{resource_ref}[/light_goldenrod3] successfully destroyed"
            )
            success = True
            ...
            if verbose:
                _dump_verbose_logs(logs_file, f"Destroy {resource_ref} logs")
            progress.console.print(
                f"[red]✗[/red] [light_goldenrod3]{resource_ref}[/light_goldenrod3] failed to delete"
            )
            await plan.resource_manager.save_resource(plan.resource, lock_info.lock_id)
            success = False
            ...
            while plans_to_start:
                ...
                async def execute_plan_wrapper(node):
                    result = await _destroy_resource(
                        node.lock, node.plan, environment, progress, verbose
                    )
...
async def stop_local_containers(
    ...
    if prompt:
        ...
        for answer in answers:
            rich.print(
                f"[pink1]>[/pink1] Stop: [light_goldenrod3]{to_stop_options[answer]}[/light_goldenrod3]"
            )
    ...
    for container in to_stop:
        task = progress.add_task(
            f"Stopping [light_goldenrod3]{container}[/light_goldenrod3]...", total=1
        )
        ...
        while stop_queue:
            ...
            try:
                ...
                if container.container.status == "exited":
                    progress.console.print(
                        f"[green]✓[/green] Stop successful for [light_goldenrod3]{container}[/light_goldenrod3]"
                    )
                    successes += 1
            except Exception as e:
                progress.remove_task(task)
...
async def _run_import_plans(
    ...
    os.makedirs(base_logging_dir, exist_ok=True)
    logs_file = f"{base_logging_dir}/{plan.resource.name}-{int(time.time())}.log"
    task_description = f"Importing [light_goldenrod3]{plan.resource.__class__.__name__}({plan.resource.name})[/light_goldenrod3]..."
    task_description += (
        f"\n  > View detailed logs with: [bold]tail -f {logs_file}[/bold]"
    )
    task = progress.add_task(task_description)
    ...
    try:
        ...
        message = f"[green]✓[/green] Successfully imported [light_goldenrod3]{plan.resource.__class__.__name__}({plan.resource.name})[/light_goldenrod3]"
        ...
        message = f"[red]✗[/red] Failed to import [light_goldenrod3]{plan.resource.__class__.__name__}({plan.resource.name})[/light_goldenrod3]"
        message += f"\n  > {e}"
