...
async def delete_gcp_environment(inputs: GCPEnvironmentDeletionInputs):
    ...
    with Progress(
        ...
        delete_task = progress.add_task(
            f"Deleting GCP project {inputs.environment.gcp_config.project_id}...",
            total=None,
        )
        projects_client = resourcemanager_v3.ProjectsAsyncClient()
        try:
            operation = await projects_client.delete_project(
                name="projects/{}".format(inputs.environment.gcp_config.project_id)
            )
        ...
        progress.console.print(
            f"[green]✓ GCP Project: `{inputs.environment.gcp_config.project_id}` successfully delete[/green]"
        )
