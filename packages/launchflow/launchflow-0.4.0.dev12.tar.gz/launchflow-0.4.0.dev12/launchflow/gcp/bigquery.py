...
@dataclasses.dataclass
class BigQueryDatasetInputs(Inputs):
    location: str
...
class BigQueryDataset(GCPResource[BigQueryDatasetOutputs]):
    ...
    dataset = lf.gcp.BigQueryDataset("my-dataset", location="EU")
    ```
    ...
    def __init__(
        ...
        **Args**:
        ...
        - `allow_nonempty_delete`: If True, the dataset can be deleted even if it is not empty. Defaults to False.
        """
        super().__init__(
            name=name,
    ...
    def inputs(self, environment_type: EnvironmentType) -> BigQueryDatasetInputs:
