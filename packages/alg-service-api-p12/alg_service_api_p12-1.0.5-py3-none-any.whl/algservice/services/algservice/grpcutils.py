...
from algservice.services.algservice.models import AlgorithmInfos, Algorithm, Mission, Rect, Point, AlgRequest, \
    PreprocessResponse, \
    DetectResponse, MissionResponse, AlgResponse, MissionPreprocessResponse, PreprocessData, AlgorithmProp
from .pb2utils import value_or_empty, bytes_or_empty

from algservice.util.exceptions import ErrorException, ErrorCode
from ..grpc.alg_service_pb2 import AlgorithmsResponse_, AlgorithmProp_, MissionPreprocessResponse_, PreprocessResponse_, \
    Shape_, MissionResponse_, DetectResponse_, AlgRequest_, Mission_, DetectRequest_, AlgorithmInfo_, Point_, \
    PreprocessData_, AlgResponse_
...
class pb2_to_local:
    ...
    def to_AlgorithmProp(prop: AlgorithmProp_) -> AlgorithmProp:
        out = AlgorithmProp(prop.name, prop.fieldType, prop.defaultValue, prop.label, prop.required, prop.extValue)
        return out
...
class local_to_pb2:
    ...
    def to_AlgorithmProp(prop: AlgorithmProp) -> AlgorithmProp_:
        out = AlgorithmProp_(name=value_or_empty(prop.name), fieldType=value_or_empty(prop.field_type),
                             ...
                             label=value_or_empty(prop.label), required=prop.required, extValue=prop.ext_value)
        return out
