...
@app.command()
def crop(image_folder: Path = typer.Option(folder_config.get("images_folder_unpatched"), help="Path to the train raw(unpatched) folder images")):
    processor = DataPipelineSetup()
    processor.crop(str(image_folder))


@app.command() def img_patchify(image_folder_unpatched: Path=typer.Option(folder_config.get("images_folder_unpatched"), help="Path to the
                 train raw(unpatched) folder images"), image_folder_patched:
                 Path=typer.Option(folder_config.get("images_folder_patched"),
...
(5, help="Numbers
of Epochs"), images_folder: Path=typer.Option(os.path.dirname(folder_config.get("images_folder_patched")),
                    help ="Path to the images patches folder, format -
                    'image_folder/class''"), masks_folder: Path=                    typer.Option(os.path.dirname(folder_config.get("root_folder_patched")),
                    help ="Path to the images patches folder, format -
                    'mask_folder/class''"), model_folder: Path=                    typer.Option(folder_config.get("models_folder"),
                    help = "Path to the model folder"), model_name: str =                    typer.Option("your_model", help="Name your model without
                    extension")):
    modelling= ModelTraining()
...
@ app.command() def predict_image(image_path: Path=typer.Option(os.path.join(folder_config.get("test_folder"),
                  "038_43-18-ROOT1-2023-08-08_pvd_OD01_Col0_05-Fish Eye
                  Corrected.png"), help="Path to an image"), ouput_folder:
                  Path=typer.Option(folder_config.get("external_data_folder"),
                  help="Path to where the prediction should be stored"),
                  model_folder: Path=typer.Option(folder_config.get("models_folder"), help="Path
                  to the model folder"), model_name: str=                  typer.Option("best_model_root_masks", help="Model name only
                  without extension")):
...
typer.Option(folder_config.get("test_folder"), help="Path to the images
                   folder"), output_folder: Path=                   typer.Option(folder_config.get("data_predictions"), help="Path
                   to where the predicted masks should be stored"),
                   model_folder: Path=typer.Option(folder_config.get("models_folder"), help="Path to the model
...
typer.Option(os.path.join(folder_config.get("test_folder"), "035_43-17-ROOT1-2023-08-08_mock_pH5_+Fe_Col0_04-Fish
                       ...
                       output_folder: Path=typer.Option(folder_config.get("external_data_folder"),
    help="Path to where the prediction should be stored"), model_folder: Path=typer.Option(folder_config.get("models_folder"), help="Path to the model
    folder"), model_name: str = typer.Option("best_model_root_masks",
    help="Model name only without extension")): segmentation=InstanceSegmentation() segmentation.test_overlaying(str(image_path),
...
typer.Option(folder_config.get("data_predictions_clean"), help="Path to the
                           ...
                           Path=typer.Option(folder_config.get("test_folder"), help="Path to the raw test
