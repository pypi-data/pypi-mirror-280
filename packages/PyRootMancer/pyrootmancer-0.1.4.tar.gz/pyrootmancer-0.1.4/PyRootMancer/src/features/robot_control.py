...
class RobotControl:
    ...
    def control_robot(self, env, rl_model, obs, skel_data_sorted, skel_data_unique, conversion_factor):
        for skeleton in skel_data_unique:
            ...
            G = nx.from_pandas_edgelist(skel_data_current, source='node-id-src', target='node-id-dst',
                                        edge_attr='branch-distance')

            if nx.number_of_nodes(G) > 0:
                longest_array = max(list(nx.connected_components(G)), key=len)
                root_data = skel_data_current[(skel_data_current['node-id-src'].isin(longest_array) | skel_data_current[
                    'node-id-dst'].isin(longest_array)) & (skel_data_current['branch-type'] == 1)]
