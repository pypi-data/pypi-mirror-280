...
class RootLengthCalculator:
    ...
    def analyze_image(self, file_name: str, img: np.ndarray, predicted_mask: np.ndarray) -> None:
        ...
        sorted_components = sorted(
            range(1, retval), key=lambda x: stats[x, cv2.CC_STAT_AREA], reverse=True
        )
        min_area = 200
        min_top = 300
        max_top = 1000
        ...
        if not selected_components:
            ...
            cropped_roots = [
                filtered_image[y: y + h, x: x + w]
                for x, y, w, h in part_boxes.values()
            ]
    ...
    def calculate_length(self, skeleton: np.ndarray, summary: pd.DataFrame) -> float:
        ...
        length = (
                summary.loc[main_max, "euclidean-distance"]
                + summary.loc[main_min, "euclidean-distance"]
        )
        return length
