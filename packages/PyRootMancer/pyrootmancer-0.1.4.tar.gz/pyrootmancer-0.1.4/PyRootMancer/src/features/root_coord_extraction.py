...
class LandmarkExtraction:
    ...
    def get_bottom_coordinates(self, input_folder, num_img: int, threshold: int = 50) -> Tuple[List[int], List[int]]:
        ...
        try:
            ...
            predicted_clean_paths = [os.path.join(input_folder, file)
            ...
            filtered_df = df[(df['image-coord-dst-0'] < 2415) &
                             (df['branch-type'] != 0) &
                             (df['euclidean-distance'] > 20)]

            image_coord_dst_0, image_coord_dst_1 = (
                filtered_df
                .sort_values(by='image-coord-dst-0', ascending=False)
            ...
            filtered_image_coord_dst_0 = [val for idx, val in enumerate(sorted_image_coord_dst_0) if
                                          idx not in indices_to_remove]
            filtered_image_coord_dst_1 = [val for idx, val in enumerate(sorted_image_coord_dst_1) if
                                          idx not in indices_to_remove]
            ...
            print(f"No roots detected")
            return [], []

    def display_root_landmarks(self, input_folder, test_folder, num_img):

        predicted_clean_paths = [os.path.join(input_folder, file)
                                 for file in os.listdir(input_folder)]

        original_image_paths = [os.path.join(test_folder, file)
        ...
        if num_labels == 1:
            ...
            for i in range(len(image_coord_dst_0)):
                cv2.circle(image, (image_coord_dst_1[i], image_coord_dst_0[i]), 25, (255, 255, 0),
                           2)  # Green color circles
...
if __name__ == "__main__":
    ...
    y, x = landmarks.get_bottom_coordinates(folder_config.get("data_predictions_clean"),
                                            image_num)
