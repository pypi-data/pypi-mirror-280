...
from blette.utils import ConfigType, SampleList, get_classes, get_palette
from blette.visualization import EdgeLocalVisualizer

InputType = Union[str, np.ndarray]
InputsType = Union[InputType, Sequence[InputType]]
PredType = Union[BasicEdgeDataSample, SampleList]


class EdgeInferencer(BaseInferencer):
    """Edge inferencer, provides inference and visualization
    interfaces. Note: MMEngine >= 0.5.0 is required.

    Args:
        model (str, optional): Path to the config file or the model name
            defined in metafile. Example: "fcn_r50-d8_4xb2-40k_cityscapes-512x1024"
            and the weights of model will be download automatically. If use config
            file, like "configs/fcn/fcn_r50-d8_4xb2-40k_cityscapes-512x1024.py",
            the `weights` should be defined.
        weights (str, optional): Path to the checkpoint. If it is not specified
            and model is a model name of metafile, the weights will be loaded
            from metafile. Defaults to None.
        classes (list, optional): Input classes for result rendering, as the
            prediction of edge detector model is an edge map with label
        ...
        scope (str, optional): The scope of the model. Defaults to 'blette'.
    """  # noqa
        ...
        **kwargs,
        ...
        Args:
            ...
            return_datasamples (bool): Whether to return results as
                :obj:`MultiEdgeDataSample`. Defaults to False.
            batch_size (int): Batch size. Defaults to 1.
            show (bool): Whether to display the rendering color edge
            ...
            img_out_dir (str): Subdirectory of `out_dir`, used to save
                rendering color edge mask, so `out_dir` must be defined
    ...
    def visualize(
        ...
        Args:
            ...
            img_out_dir (str): Output directory of rendering prediction i.e.
                color edge mask. Defaults: ''
            opacity (int, float): The transparency of edge mask.
                Defaults to 0.8.
    ...
    def postprocess(
        ...
        Returns:
            ...
            - ``predictions`` (List[np.ndarray], np.ndarray): Returned by
                ...
                If ``return_datasample=False``, it will be the edge mask
                with label indice.
        """
        ...
        for i, pred in enumerate(preds):
            pred_data = pred.pred_edge.numpy().data[0]
            ...
            if pred_out_dir != "":
                ...
                if len(classes) > 1:
                    # NOTE: we need to save "per-class" prediction since
                    # the prediction is a multi-label edge map
                    for i, class_name in enumerate(classes):
                        pd = pred_data[i].astype(np.uint8)

                        # name the save directory
                        if save_as_class_name:
                            class_dir = osp.join(pred_out_dir, class_name)
                        else:
                            class_dir = osp.join(
                                pred_out_dir, f"class_{str(i + 1).zfill(3)}"
                            )

                        mmengine.mkdir_or_exist(class_dir)
                        img_name = str(self.num_pred_imgs).zfill(8) + "_pred.png"
                        img_path = osp.join(class_dir, img_name)
                        output = Image.fromarray(pd)
