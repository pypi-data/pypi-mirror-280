...
"""General evaluation protocols for BDD100k Semantic Boundary Benchmark

- should work because bdd100k is based on cityscapes labels
...
class BDD100kEvaluator(CityscapesEvaluator):
    GT_DIR = "gtEval"
    ...
    def set_sample_names(self):
        ...
        sample_names = []
        for fp in glob.glob(
            osp.join(self.dataset_root, "images/10k", self.split, "*.jpg")
        ):
            sample_name = osp.splitext(osp.basename(fp))[0]
...
def parse_args():
    parser = argparse.ArgumentParser("BDD100k evaluation parser")
    parser.add_argument(
        "bdd100k_path",
        type=str,
        help="the root path of the bdd100k dataset",
    )
    parser.add_argument(
        "pred_path",
        type=str,
...
def evaluate(
    ...
    if categories is None:
        ...
        categories = list(range(1, len(BDD100kEvaluator.CLASSES) + 1))
    else:
    ...
    logger.info("Running BDD100k Evaluation")
