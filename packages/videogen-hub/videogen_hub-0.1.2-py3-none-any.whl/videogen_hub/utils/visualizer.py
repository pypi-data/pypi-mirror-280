...
def gather_outputs(result_folder_path: Union[str, os.PathLike], look_up_videos: List = None, input_folder_name: Union[str, list] = 'input'):
    """
    ...
    Args:
        ...
        look_up_videos (List, optional): List of images to look up. If not provided, defaults to None.
    ...
    if look_up_videos == None:
        look_up_videos = get_list_in_folder(
            os.path.join(result_folder_path, input_folder_name))
        # filter out anything that is not image
        look_up_videos = filter_images(look_up_videos, parent_path=os.path.join(
            result_folder_path, input_folder_name))

    gathered_outputs = []
    for look_up_image in look_up_videos:
        ims, txts = [], []
