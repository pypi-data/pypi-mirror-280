...
class HTML:
    ...
    def add_videos(self, vids: Union[str, List], txts: Union[str, List], links: Union[str, List], width: int = 512):
        """
        Add a row of videos to the HTML document.

        Args:
            vids (Union[str, List[str]]): List of video paths or a single video path.
            txts (Union[str, List[str]]): List of video captions or a single caption.
            links (Union[str, List[str]]): List of hyperlinks or a single hyperlink associated with the videos.
            width (int, optional): Display width of the videos in pixels. Defaults to 512.
        """
        self.t = table(
            border=1, style="table-layout: fixed;")  # Insert a table
        self.doc.add(self.t)
            ...
            with tr():
                for vid, txt, link in zip(vids, txts, links):
                    with td(style="word-wrap: break-word;", halign="center", valign="top"):
                        with p():
                            with a(href=os.path.join(link)):
                                video(style="width:%dpx" % width, src=os.path.join(vid), controls=True)
                            br()
                            p(txt)
