...
class Script(metaclass=ABCMeta):
    ...
    def run_script(self, args, event_writer, input_stream):
        ...
        try:
            ...
            if str(args[1]).lower() == "--scheme":
                # Khulnasoft has requested XML specifying the scheme for this
    ...
    @property
    def service(self):
        """ Returns a Khulnasoft service object for this script invocation.

        The service object is created from the Khulnasoftd URI and session key
        ...
        :return: :class:`khulnasoftlib.client.Service`. A value of None is returned,
        ...
        khulnasoftd_uri = self._input_definition.metadata["server_uri"]
        session_key = self._input_definition.metadata["session_key"]

        khulnasoftd = urlsplit(khulnasoftd_uri, allow_fragments=False)

        self._service = Service(
            scheme=khulnasoftd.scheme,
            host=khulnasoftd.hostname,
            port=khulnasoftd.port,
            token=session_key,
        )
    ...
    def validate_input(self, definition):
        ...
        When Khulnasoft calls a modular input script in validation mode, it will
        pass in an XML document giving information about the Khulnasoft instance (so
        ...
        logged back to Khulnasoft.

        The default implementation always passes.

        :param definition: The parameters for the proposed input passed by khulnasoftd.
        """

    @abstractmethod
    def stream_events(self, inputs, ew):
        """The method called to stream events into Khulnasoft. It should do all of its output via
        EventWriter rather than assuming that there is a console attached.

        :param inputs: An ``InputDefinition`` object.
        :param ew: An object with methods to write events and log messages to Khulnasoft.
        """
