...
class EventWriter:
    """``EventWriter`` writes events and error messages to Khulnasoft from a modular input.
    Its two important methods are ``writeEvent``, which takes an ``Event`` object,
    and ``log``, which takes a severity and an error message.
    """

    # Severities that Khulnasoft understands for log messages from modular inputs.
    # Do not change these
    DEBUG = "DEBUG"
    INFO = "INFO"
    WARN = "WARN"
    ...
    def write_event(self, event):
        """Writes an ``Event`` object to Khulnasoft.
    ...
    def log(self, severity, message):
        """Logs messages about the state of this modular input to Khulnasoft.
        These messages will show up in Khulnasoft's internal logs.
