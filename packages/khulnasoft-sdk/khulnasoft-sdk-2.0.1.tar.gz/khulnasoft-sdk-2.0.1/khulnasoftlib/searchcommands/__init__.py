...
.. topic:: Design Notes
  ...
  1. Commands are constrained to this ABNF grammar::
     ...
     It does not show that :code:`field-name` values may be comma-separated. This is because Khulnasoft strips commas from
     the command line. A search command will never see them.

  2. Search commands targeting versions of Khulnasoft prior to 6.3 must be statically configured as follows:
  ...
  3. Commands support dynamic probing for settings.
     ...
     Khulnasoft probes for settings dynamically when :code:`supports_getinfo=true`.
  ...
  4. Commands do not support parsed arguments on the command line.
     ...
     Khulnasoft parses arguments when :code:`supports_rawargs=false`. The
     :code:`SearchCommand` class sets this value unconditionally. You cannot
     override it.

     **Rationale**

     Khulnasoft parses arguments by stripping quotes, nothing more. This may be useful
     in some cases, but doesn't work well with our chosen grammar.

  5. Commands consume input headers.

     An input header is provided by Khulnasoft when :code:`enableheader=true`. The
     :class:`SearchCommand` class sets this value unconditionally. You cannot
     override it.

  6. Commands produce an output messages header.

     Khulnasoft expects a command to produce an output messages header when
  ...
  7. Commands support multi-value fields.
     ...
     Multi-value fields are provided and consumed by Khulnasoft when
  ...
  8. This module represents all fields on the output stream in multi-value
     ...
     Khulnasoft recognizes two kinds of data: :code:`value` and :code:`list(value)`.
  ...
  9. A :class:`ReportingCommand` must override :meth:`~ReportingCommand.reduce`
     ...
     Khulnasoft processing pipeline are distinguished as this example illustrates.
...
.. topic:: References
  ...
  1. `Custom Search Command manual: <https://dev.khulnasoft.com/enterprise/docs/devtools/customsearchcommands>`__

  2. `Create Custom Search Commands with commands.conf.spec <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Admin/Commandsconf>`_

  3. `Configure seach assistant with searchbnf.conf <https://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Admin/Searchbnfconf>`_

  4. `Control search distribution with distsearch.conf <https://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Admin/Distsearchconf>`_
