...
from khulnasoftlib.utils import ensure_str


# Relative imports
import khulnasoftlib
...
class SearchCommand:
    ...
    record = Option(doc='''
        ...
        **Description:** When `true`, records the interaction between the command and khulnasoftd. Defaults to `false`.
    ...
    @property
    def service(self):
        """ Returns a Khulnasoft service object for this command invocation or None.

        The service object is created from the Khulnasoftd URI and authentication token passed to the command invocation in
        ...
        :return: :class:`khulnasoftlib.client.Service`, if :code:`enableheader` and :code:`requires_srinfo` are both
        ...
        khulnasoftd_uri = searchinfo.khulnasoftd_uri

        if khulnasoftd_uri is None:
            return None

        uri = urlsplit(khulnasoftd_uri, allow_fragments=False)
    ...
    def _map_input_header(self):
        ...
        self._input_header.update(
            ...
            sid=searchinfo.sid,
            khulnasoftVersion=searchinfo.khulnasoft_version,
            truncated=None)
    ...
    _metadata_map = {
        ...
        'searchinfo': {
            ...
            'khulnasoft_version':
                (None, lambda s: s.input_header.get('khulnasoftVersion')),
            'khulnasoftd_uri':
                (lambda v: v.khulnasoftd_uri, lambda s: s.search_results_info),
            'username':
    ...
    def _prepare_protocol_v1(self, argv, ifile, ofile):
        ...
        debug = environment.khulnasoftlib_logger.debug
    ...
    def _prepare_recording(self, argv, ifile, ofile):
        ...
        recordings = os.path.join(environment.khulnasoft_home, 'var', 'run', 'khulnasoftlib.searchcommands', 'recordings')
        ...
        # Save a khulnasoft command line because it is useful for developing tests

        with open(recording + '.khulnasoft_cmd', 'wb') as f:
            f.write('khulnasoft cmd python '.encode())
        ...
        try:
            if argv[1] == '__GETINFO__':
                ...
                message = (
                    ...
                    'configuration is unsupported by khulnasoftlib.searchcommands. Please ensure that '
    ...
    def _process_protocol_v2(self, argv, ifile, ofile):
        ...
        # Read search command metadata from khulnasoftd
        # noinspection PyBroadException
        try:
        ...
        # Write search command configuration for consumption by khulnasoftd
        # noinspection PyBroadException
        try:
    ...
    def _report_unexpected_error(self):
        ...
        environment.khulnasoftlib_logger.error(message + '\nTraceback:\n' + ''.join(traceback.format_tb(tb)))
...
def dispatch(command_class, argv=sys.argv, input_file=sys.stdin, output_file=sys.stdout, module_name=None,
    ...
    **Example**
    ...
    ..  code-block:: python
        ...
        #!/usr/bin/env python
        from khulnasoftlib.searchcommands import dispatch, StreamingCommand, Configuration, Option, validators
        @Configuration()
