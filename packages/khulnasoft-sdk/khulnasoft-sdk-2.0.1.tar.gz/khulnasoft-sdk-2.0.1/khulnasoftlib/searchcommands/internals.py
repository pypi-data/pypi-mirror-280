...
class CommandLineParser:
    ...
    **Note:**
    ...
    because Khulnasoft strips commas from the command line. A custom search command will never see them.
    ...
    **Example:**
    ...
    def unquote(cls, string):
        ...
        Khulnasoft search command quote rules are applied. The enclosing double-quotes, if present, are removed. Escaped
        double-quotes ('\"' or '""') are replaced by a single double-quote ('"').

        **NOTE**

        We are not using a json.JSONDecoder because Khulnasoft quote rules are different than JSON quote rules. A
...
class CsvDialect(csv.Dialect):
    """ Describes the properties of Khulnasoft CSV streams """
    delimiter = ','
    quotechar = '"'
    doublequote = True
...
class InputHeader(dict):
    """ Represents a Khulnasoft input header as a collection of name/value pairs.
    ...
    def flush(self, finished=None, partial=None):
        ...
        if self.pending_record_count > 0 or (self._chunk_count == 0 and 'messages' in self._inspector):
            ...
            if self._chunk_count == 0:
                ...
                # Guarantee: These messages are displayed by khulnasoftweb and the job inspector
                ...
                # Guarantee: These messages are displayed by khulnasoftweb and the job inspector, if and only if the
...
class RecordWriterV2(RecordWriter):
    ...
    def write_chunk(self, finished=None):
        ...
        # TODO: DVPL-6448: khulnasoftlib.searchcommands | Add support for partial: true when it is implemented in
