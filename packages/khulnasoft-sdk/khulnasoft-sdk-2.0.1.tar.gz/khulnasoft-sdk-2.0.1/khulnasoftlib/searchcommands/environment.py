...
def configure_logging(logger_name, filename=None):
    ...
    This function expects a Khulnasoft app directory structure::
    ...
    if filename is not None:
        ...
        if filename != _current_logging_configuration_file:
            ...
            chdir(app_root)
            try:
                fileConfig(filename, {'KHULNASOFT_HOME': khulnasoft_home})
            finally:
...
khulnasoft_home = path.abspath(path.join(getcwd(), environ.get('KHULNASOFT_HOME', '')))
app_file = getattr(sys.modules['__main__'], '__file__', sys.executable)
app_root = path.dirname(path.abspath(path.dirname(app_file)))

khulnasoftlib_logger, logging_configuration = configure_logging('khulnasoftlib')


__all__ = ['app_file', 'app_root', 'logging_configuration', 'khulnasoft_home', 'khulnasoftlib_logger']
