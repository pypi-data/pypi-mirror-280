...
# various Khulnasoft endpoints. The approach here is to leverage the binding
...
"""The **khulnasoftlib.client** module provides a Pythonic interface to the
`Khulnasoft REST API <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTcontents>`_,
allowing you programmatically access Khulnasoft's resources.

**khulnasoftlib.client** wraps a Pythonic layer around the wire-level
binding of the **khulnasoftlib.binding** module. The core of the library is the
:class:`Service` class, which encapsulates a connection to the server, and
provides access to the various aspects of Khulnasoft's functionality, which are
exposed via the REST API. Typically you connect to a running Khulnasoft instance
...
:class:`Service` objects have fields for the various Khulnasoft resources (such as apps,
...
from khulnasoftlib import data
from khulnasoftlib.data import record
from khulnasoftlib.binding import (AuthenticationError, Context, HTTPError, UrlEncoded,
...
class IllegalOperationException(Exception):
    """Thrown when an operation is not possible on the Khulnasoft instance that a
...
def connect(**kwargs):
    """This function connects and logs in to a Khulnasoft instance.
    ...
    :param cookie: A session cookie. When provided, you don't need to call :meth:`login`.
        This parameter is only supported for Khulnasoft 6.2+.
    :type cookie: ``string``
    :param autologin: When ``True``, automatically tries to log in again if the
        session terminates.
    :type autologin: ``boolean``
    :param `username`: The Khulnasoft account username, which is used to
                       authenticate the Khulnasoft instance.
    :type username: ``string``
    :param `password`: The password for the Khulnasoft account.
    :type password: ``string``
    :param retires: Number of retries for each HTTP connection (optional, the default is 0).
                    NOTE THAT THIS MAY INCREASE THE NUMBER OF ROUND TRIP CONNECTIONS TO THE KHULNASOFT SERVER.
    :type retries: ``int``
...
# API is not going to be the same as enterprise Khulnasoft's
# API, so we will derive both Service (for enterprise Khulnasoft)
# and StormService for (Khulnasoft Storm) from _BaseService, and
...
class Service(_BaseService):
    """A Pythonic binding to Khulnasoft instances.

    A :class:`Service` represents a binding to a Khulnasoft instance on an
    ...
    :class:`khulnasoftlib.binding.Context` are also available in case you need
    to do something beyond what is provided by this class.

    After creating a ``Service`` object, you must call its :meth:`login`
    method before you can issue requests to Khulnasoft.
    ...
    :param `username`: The Khulnasoft account username, which is used to
                       authenticate the Khulnasoft instance.
    :type username: ``string``
    :param `password`: The password, which is used to authenticate the Khulnasoft
                       instance.
    :type password: ``string``
    :param retires: Number of retries for each HTTP connection (optional, the default is 0).
                    NOTE THAT THIS MAY INCREASE THE NUMBER OF ROUND TRIP CONNECTIONS TO THE KHULNASOFT SERVER.
    ...
    **Example**::
        ...
        s = client.Service(cookie="khulnasoftd_8089=...")
    """

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._khulnasoft_version = None
        self._kvstore_owner = None
        self._instance_type = None

    @property
    def apps(self):
        """Returns the collection of applications that are installed on this instance of Khulnasoft.
    ...
    @property
    def confs(self):
        """Returns the collection of configuration files for this Khulnasoft instance.
    ...
    def event_types(self):
        """Returns the collection of event types defined in this Khulnasoft instance.
    ...
    def fired_alerts(self):
        """Returns the collection of alerts that have been fired on the Khulnasoft
    ...
    @property
    def indexes(self):
        """Returns the collection of indexes for this Khulnasoft instance.
    ...
    @property
    def info(self):
        """Returns the information about this instance of Khulnasoft.
    ...
    @property
    def inputs(self):
        """Returns the collection of inputs configured on this Khulnasoft instance.
    ...
    def modular_input_kinds(self):
        """Returns the collection of the modular input kinds on this Khulnasoft instance.

        :return: A :class:`ReadOnlyCollection` of :class:`ModularInputKind` entities.
        """
        if self.khulnasoft_version >= (5,):
            return ReadOnlyCollection(self, PATH_MODULAR_INPUTS, item=ModularInputKind)
        raise IllegalOperationException("Modular inputs are not supported before Khulnasoft version 5.")

    @property
    def storage_passwords(self):
        """Returns the collection of the storage passwords on this Khulnasoft instance.
    ...
    def restart(self, timeout=None):
        """Restarts this Khulnasoft instance.
        ...
        """
        msg = {"value": "Restart requested by " + self.username + "via the Khulnasoft SDK for Python"}
    ...
    def restart_required(self):
        """Indicates whether khulnasoftd is in a state that requires a restart.
    ...
    @property
    def settings(self):
        """Returns the configuration settings for this instance of Khulnasoft.
    ...
    @property
    def khulnasoft_version(self):
        """Returns the version of the khulnasoftd instance this object is attached
        to.
        ...
        """
        if self._khulnasoft_version is None:
            self._khulnasoft_version = tuple(int(p) for p in self.info['version'].split('.'))
        return self._khulnasoft_version

    @property
    def khulnasoft_instance(self):
        if self._instance_type is None :
            khulnasoft_info = self.info
            if hasattr(khulnasoft_info, 'instance_type') :
                self._instance_type = khulnasoft_info['instance_type']
            else:
                self._instance_type = ''
    ...
    def disable_v2_api(self):
        if self.khulnasoft_instance.lower() == 'cloud':
            return self.khulnasoft_version < (9,0,2209)
        return self.khulnasoft_version < (9,0,2)

    @property
    def kvstore_owner(self):
        """Returns the KVStore owner for this instance of Khulnasoft.
...
class Endpoint:
    """This class represents individual Khulnasoft resources in the Khulnasoft REST API.
    ...
    def get(self, path_segment="", owner=None, app=None, sharing=None, **query):
        ...
        **Example**::
        ...
        #   - Fallback from v2+ to v1 if Khulnasoft Version is < 9.
        # if api_version >= 2 and ('search' in query and path.endswith(tuple(["results_preview", "events", "results"])) or self.service.khulnasoft_version < (9,)):
...
class Entity(Endpoint):
    """This class is a base class for Khulnasoft entities in the REST API, such as
    ...
    def access(self):
        ...
        :return: A :class:`khulnasoftlib.data.Record` object with three keys:
    ...
    def acl_update(self, **kwargs):
        ...
        :param kwargs: Additional entity-specific arguments (required).
            ...
            - "owner" (``string``): The Khulnasoft username, such as "admin". A value of "nobody" means no specific user (required).
...
class ReadOnlyCollection(Endpoint):
    """This class represents a read-only collection of entities in the Khulnasoft
    instance.
    """
    ...
    def _load_list(self, response):
        ...
        *response* is assumed to be a :class:`Record` containing an
        ...
        # khulnasoftd returns something that doesn't match
        # <feed><entry></entry><feed>.
    ...
    def itemmeta(self):
        ...
        :return: A :class:`khulnasoftlib.data.Record` object containing the metadata.
...
class Collection(ReadOnlyCollection):
    ...
    Khulnasoft provides a number of different collections of distinct
    entity types: applications, saved searches, fired alerts, and a
    number of others. Each particular type is available separately
    from the Khulnasoft instance, and the entities of that type are
    ...
    **Example**::
    ...
    def create(self, name, **params):
        ...
        :type name: ``string``
        :param namespace: A namespace, as created by the :func:`khulnasoftlib.binding.namespace`
            function (optional).  You can also set ``owner``, ``app``, and
            ``sharing`` in ``params``.
        :type namespace: A :class:`khulnasoftlib.data.Record` object with keys ``owner``, ``app``,
            and ``sharing``.
...
class Configurations(Collection):
    """This class provides access to the configuration files from this Khulnasoft
    instance. Retrieve this collection using :meth:`Service.confs`.

    Khulnasoft's configuration is divided into files, and each file into
...
class StoragePasswords(Collection):
    """This class provides access to the storage passwords from this Khulnasoft
...
class Indexes(Collection):
    """This class contains the collection of indexes in this Khulnasoft instance.
    ...
    def delete(self, name):
        ...
        **Note**: This method is only supported in Khulnasoft 5.0 and later.

        :param name: The name of the index to delete.
        :type name: ``string``
        """
        if self.service.khulnasoft_version >= (5,):
            Collection.delete(self, name)
        else:
            raise IllegalOperationException("Deleting indexes via the REST API is "
                                            "not supported before Khulnasoft version 5.")
...
class Index(Entity):
    ...
    def attach(self, host=None, source=None, sourcetype=None):
        ...
        cookie_header = self.service.token if self.service.token is _NoAuthenticationToken else self.service.token.replace("Khulnasoft ", "")
        cookie_or_auth_header = f"Authorization: Khulnasoft {cookie_header}\r\n"
        ...
        # the connection open and use the Khulnasoft extension headers to note
        # the input mode
        ...
        headers = [f"POST {str(self.service._abspath(path))} HTTP/1.1\r\n".encode('utf-8'),
                   ...
                   b"X-Khulnasoft-Input-Mode: Streaming\r\n",
                   b"\r\n"]
    ...
    def attached_socket(self, *args, **kwargs):
        """Opens a raw socket in a ``with`` block to write data to Khulnasoft.
    ...
    def clean(self, timeout=60):
        ...
        try:
            if not was_disabled_initially and self.service.khulnasoft_version < (5,):
                # Need to disable the index first on Khulnasoft 4.x,
    ...
    def upload(self, filename, **kwargs):
        ...
        **Note**: The file must be locally accessible from the server.
        ...
        :param kwargs: Additional arguments (optional). For more about the
            available parameters, see `Index parameters <http://dev.khulnasoft.com/view/SP-CAAAEE6#indexparams>`_ on Khulnasoft Developer Portal.
        :type kwargs: ``dict``
...
class Input(Entity):
    """This class represents a Khulnasoft input. This class is the base for all
    ...
    def __init__(self, service, path, kind=None, **kwargs):
        ...
        # and "khulnasofttcp" (which is "tcp/cooked").
        Entity.__init__(self, service, path, **kwargs)
        if kind is None:
    ...
    def update(self, **kwargs):
        ...
        :param kwargs: Additional arguments (optional). For more about the
            available parameters, see `Input parameters <http://dev.khulnasoft.com/view/SP-CAAAEE6#inputparams>`_ on Khulnasoft Developer Portal.
        :type kwargs: ``dict``
        ...
        if self.kind not in ['tcp', 'khulnasofttcp', 'tcp/raw', 'tcp/cooked', 'udp']:
            return super().update(**kwargs)
        else:
            # The behavior of restrictToHost is inconsistent across input kinds and versions of Khulnasoft.
            # In Khulnasoft 4.x, the name of the entity is only the port, independent of the value of
            # restrictToHost. In Khulnasoft 5.0 this changed so the name will be of the form <restrictToHost>:<port>.
...
class Inputs(Collection):
    ...
    def __getitem__(self, key):
        ...
        # <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTinput>
        if isinstance(key, tuple) and len(key) == 2:
            # Fetch a single kind
            key, kind = key
            key = UrlEncoded(key, encode_slash=True)
    ...
    def itemmeta(self, kind):
        ...
        :return: The metadata.
        :rtype: class:``khulnasoftlib.data.Record``
        """
    ...
    @property
    def kinds(self):
        """Returns the input kinds on this Khulnasoft instance.
...
class Job(Entity):
    ...
    def __init__(self, service, sid, **kwargs):
        # Default to v2 in Khulnasoft Version 9+
        path = "{path}{sid}"
        # Formatting path based on the Khulnasoft Version
    ...
    def events(self, **kwargs):
        ...
        :param kwargs: Additional parameters (optional). For a list of valid
            ...
            <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#GET_search.2Fjobs.2F.7Bsearch_id.7D.2Fevents>`_
    ...
    def results(self, **query_params):
        ...
        to :class:`khulnasoftlib.results.JSONResultsReader` along with the query param "output_mode='json'", as in::

            import khulnasoftlib.client as client
            import khulnasoftlib.results as results
            from time import sleep
            service = client.connect(...)
        ...
        :param query_params: Additional parameters (optional). For a list of valid
            ...
            <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#GET_search.2Fjobs.2F.7Bsearch_id.7D.2Fresults>`_.
    ...
    def preview(self, **query_params):
        ...
        Unlike :class:`khulnasoftlib.results.JSONResultsReader`along with the query param "output_mode='json'",
        ...
        from the server. Pass the handle returned to :class:`khulnasoftlib.results.JSONResultsReader` to get a nice,
        ...
        :param query_params: Additional parameters (optional). For a list of valid
            ...
            <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#GET_search.2Fjobs.2F.7Bsearch_id.7D.2Fresults_preview>`_
            in the REST API documentation.
        :type query_params: ``dict``
    ...
    def searchlog(self, **kwargs):
        ...
        :param `kwargs`: Additional parameters (optional). For a list of valid
            ...
            <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#GET_search.2Fjobs.2F.7Bsearch_id.7D.2Fsearch.log>`_
            in the REST API documentation.
        :type kwargs: ``dict``
    ...
    def set_priority(self, value):
        ...
        Higher numbers indicate higher priority. Unless khulnasoftd is
    ...
    def summary(self, **kwargs):
        ...
        :param `kwargs`: Additional parameters (optional). For a list of valid
            ...
            <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#GET_search.2Fjobs.2F.7Bsearch_id.7D.2Fsummary>`_
            in the REST API documentation.
        :type kwargs: ``dict``
    ...
    def timeline(self, **kwargs):
        ...
        :param `kwargs`: Additional timeline arguments (optional). For a list of valid
            ...
            <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#GET_search.2Fjobs.2F.7Bsearch_id.7D.2Ftimeline>`_
            in the REST API documentation.
        :type kwargs: ``dict``
...
class Jobs(Collection):
    ...
    def __init__(self, service):
        # Khulnasoft 9 introduces the v2 endpoint
    ...
    def create(self, query, **kwargs):
        ...
        :param kwargs: Additiona parameters (optional). For a list of available
            ...
            <http://dev.khulnasoft.com/view/SP-CAAAEE5#searchjobparams>`_
            on Khulnasoft Developer Portal.
        :type kwargs: ``dict``
    ...
    def export(self, query, **params):
        ...
        pass the handle to :class:`khulnasoftlib.results.JSONResultsReader` along with the query param
        "output_mode='json'"::
        ...
        :param params: Additional arguments (optional). For a list of valid
            ...
            <http://docs/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#search.2Fjobs.2Fexport>`_
            in the REST API documentation.
        :type params: ``dict``
    ...
    def oneshot(self, query, **params):
        ...
        objects, pass the handle to :class:`khulnasoftlib.results.JSONResultsReader` along with the query param
        "output_mode='json'" ::
...
class SavedSearch(Entity):
    ...
    def dispatch(self, **kwargs):
        ...
        :param `kwargs`: Additional dispatch arguments (optional). For details,
                         ...
                         <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsearch#POST_saved.2Fsearches.2F.7Bname.7D.2Fdispatch>`_
    ...
    def update(self, search=None, **kwargs):
        ...
        :param `kwargs`: Additional arguments (optional). For a list of available
            ...
            <http://dev.khulnasoft.com/view/SP-CAAAEE5#savedsearchparams>`_
            on Khulnasoft Developer Portal.
        :type kwargs: ``dict``
...
class Settings(Entity):
    """This class represents configuration settings for a Khulnasoft service.
    ...
    def update(self, **kwargs):
        ...
        :param kwargs: Additional arguments. For a list of valid arguments, see
            ...
            <http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/RESTAPI/RESTsystem#POST_server.2Fsettings.2F.7Bname.7D>`_
            in the REST API documentation.
        :type kwargs: ``dict``
...
class User(Entity):
    """This class represents a Khulnasoft user.
    """
...
# Khulnasoft automatically lowercases new user names so we need to match that
# behavior here to ensure that the subsequent member lookup works correctly.
class Users(Collection):
    """This class represents the collection of Khulnasoft users for this instance of
    Khulnasoft. Retrieve this collection using :meth:`Service.users`.
    """
    ...
    def create(self, username, password, roles, **params):
        ...
        :param params: Additional arguments (optional). For a list of available
            ...
            <http://dev.khulnasoft.com/view/SP-CAAAEJ6#userauthparams>`_
            on Khulnasoft Developer Portal.
        :type params: ``dict``
        ...
        **Example**::
        ...
        # khulnasoftd doesn't return the user in the POST response body,
...
class Role(Entity):
    ...
    def grant(self, *capabilities_to_grant):
        ...
        :param capabilities_to_grant: Zero or more capabilities to grant this
            ...
            `Capabilities <http://dev.khulnasoft.com/view/SP-CAAAEJ6#capabilities>`_
            on Khulnasoft Developer Portal.
...
class Roles(Collection):
    """This class represents the collection of roles in the Khulnasoft instance.
    ...
    def create(self, name, **params):
        ...
        :param params: Additional arguments (optional). For a list of available
            ...
            <http://dev.khulnasoft.com/view/SP-CAAAEJ6#rolesparams>`_
            on Khulnasoft Developer Portal.
        :type params: ``dict``
...
class Application(Entity):
    """Represents a locally-installed Khulnasoft app."""
