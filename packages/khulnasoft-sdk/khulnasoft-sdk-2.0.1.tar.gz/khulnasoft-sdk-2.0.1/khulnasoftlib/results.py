...
"""The **khulnasoftlib.results** module provides a streaming XML reader for Khulnasoft
search results.

Khulnasoft search results can be returned in a variety of formats including XML,
...
class Message:
    """This class represents informational messages that Khulnasoft interleaves in the results stream.
...
class ResultsReader:
    """This class returns dictionaries and Khulnasoft messages from an XML results
    stream.

    ``ResultsReader`` is iterable, and returns a ``dict`` for results, or a
    :class:`Message` object for Khulnasoft messages. This class has one field,
    ...
    **Example**::
    ...
    def _parse_results(self, stream):
        ...
        try:
            ...
            # khulnasoft that is described in __init__.
...
class JSONResultsReader:
    """This class returns dictionaries and Khulnasoft messages from a JSON results
    stream.
