...
"""The **khulnasoftlib.binding** module provides a low-level binding interface to the
...
If you want a friendlier interface to the Khulnasoft REST API, use the
:mod:`khulnasoftlib.client` module.
"""
...
from khulnasoftlib import __version__
...
class _NoAuthenticationToken:
    """The value stored in a :class:`Context` or :class:`khulnasoftlib.client.Service`
...
def _authentication(request_fun):
    ...
    *request_fun* is a function taking no arguments that needs to
    be run with this ``Context`` logged into Khulnasoft.
    ...
    **Example**::
        ...
        import khulnasoftlib.binding as binding
...
def _authority(scheme=DEFAULT_SCHEME, host=DEFAULT_HOST, port=DEFAULT_PORT):
    ...
    **Example**::
        ...
        _authority(host="khulnasoft.utopia.net") == "https://khulnasoft.utopia.net:8089"

        _authority(host="2001:0db8:85a3:0000:0000:8a2e:0370:7334") == \
            "https://[2001:0db8:85a3:0000:0000:8a2e:0370:7334]:8089"

        _authority(scheme="http", host="khulnasoft.utopia.net", port="471") == \
            "http://khulnasoft.utopia.net:471"
...
def namespace(sharing=None, owner=None, app=None, **kwargs):
    """This function constructs a Khulnasoft namespace.

    Every Khulnasoft resource belongs to a namespace. The namespace is specified by
    ...
    Khulnasoft resources equivelent to using ``owner={currentUser}`` and
    ``app={defaultApp}``.
    ...
    :type app: ``string``
    :returns: A :class:`khulnasoftlib.data.Record` containing the reconciled
        namespace.
...
class Context:
    """This class represents a context that encapsulates a khulnasoftd connection.
    ...
    method before you can issue requests to khulnasoftd. Or, use the :func:`connect`
    ...
    :param khulnasoftToken: Khulnasoft authentication token
    :type khulnasoftToken: ``string``
    :param headers: List of extra HTTP headers to send (optional).
    :type headers: ``list`` of 2-tuples.
    :param retires: Number of retries for each HTTP connection (optional, the default is 0).
                    NOTE THAT THIS MAY INCREASE THE NUMBER OF ROUND TRIP CONNECTIONS TO THE KHULNASOFT SERVER AND BLOCK THE
    ...
    **Example**::
        ...
        c = binding.Context(cookie="khulnasoftd_8089=...")
    """
    ...
    def __init__(self, handler=None, **kwargs):
        ...
        self.bearerToken = kwargs.get("khulnasoftToken", "")
    ...
    def _auth_headers(self):
        ...
        if self.has_cookies():
            ...
            if self.token.startswith('Khulnasoft '):
                token = self.token
            else:
                token = f'Khulnasoft {self.token}'
        if token:
    ...
    def connect(self):
        """Returns an open connection (socket) to the Khulnasoft instance.
        ...
        **Example**::
            ...
            socket.write("X-Khulnasoft-Input-Mode: Streaming\\r\\n")
            socket.write("\\r\\n")
        """
    ...
    def post(self, path_segment, owner=None, app=None, sharing=None, headers=None, **query):
        ...
        Some of Khulnasoft's endpoints, such as ``receivers/simple`` and
    ...
    def login(self):
        """Logs into the Khulnasoft instance referred to by the :class:`Context`
        object.
        ...
        **Example**::
        ...
        try:
            response = self.http.post(
                ...
                cookie="1")  # In Khulnasoft 6.2+, passing "cookie=1" will return the "set-cookie" header

            body = response.body.read()
            session = XML(body).findtext("./sessionKey")
            self.token = f"Khulnasoft {session}"
            return self
        except HTTPError as he:
            if he.status == 401:
...
class AuthenticationError(HTTPError):
    """Raised when a login request to Khulnasoft fails.

    If your username was unknown or you provided an incorrect password
    in a call to :meth:`Context.login` or :meth:`khulnasoftlib.client.Service.login`,
...
#
# The HTTP interface used by the Khulnasoft binding layer abstracts the underlying
...
class HttpLib:
    ...
    :meth:`post`, and :meth:`get` methods for the three HTTP methods that Khulnasoft
    uses.
...
def handler(key_file=None, cert_file=None, timeout=None, verify=False, context=None):
    ...
    def request(url, message, **kwargs):
        ...
        head = {
            ...
            "Host": host,
            "User-Agent": "khulnasoft-sdk-python/%s" % __version__,
            "Accept": "*/*",
            "Connection": "Close",
        }  # defaults
