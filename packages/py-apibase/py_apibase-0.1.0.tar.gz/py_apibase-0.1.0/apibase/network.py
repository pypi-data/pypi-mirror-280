...
class APINetwork():
    ...
    def __call__(self, method, url, headers=None, params=None, data_str=None, data_json=None, files=None):
        ...
        try:
            ...
            raise APINetworkError('network request timeout error: %s' % (e)) from None
        except requests_ConnectionError as e:
            raise APINetworkError('network request connection error: %s' % (e)) from None
        except RequestException as e:
            raise APINetworkError('network request exception error: %s' % (e)) from None
    ...
    def _do_network(self, method, url, headers, params, data_str, data_json, files):
        ...
        if method == 'GET':
            ...
            # should never happen
            raise APINetworkError('internal error - http method invalid: %s' % (method))
        # success!
        return r
