...
def recognize(
    ...
    det_model_name, det_model_backend = det_model_name.split('::')
    if det_model_name == 'naive_det':
        out_texts = visualize_naive_result(img, det_model_name, ocr_out, box_score_thresh)
        return [gr.update(visible=False), gr.update(visible=True), gr.update(value=out_texts, visible=True)]
    else:
        out_img_path = visualize_result(img, ocr_out)
        return [gr.update(value=out_img_path, visible=True), gr.update(visible=False), gr.update(visible=False)]
...
def main():
    ...
    desc = '<p style="text-align: center">详细说明参见：<a href="https://github.com/breezedeus/CnOCR" target="_blank">Github</a>；' \
    ...
    with gr.Blocks() as demo:
        gr.Markdown(f'<h1 style="text-align: center; margin-bottom: 1rem;">{title} <a href="https://github.com/breezedeus/cnocr" target="_blank">CnOCR</a></h1>')
        gr.Markdown(desc)
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=3, variant='compact'):
                ...
                naive_warn = gr.Markdown('**⚠️ Warning**: "naive_det" 检测模型不返回文本框位置！', visible=False)
                out_texts = gr.Dataframe(headers=['得分', '文本'], label='识别结果', interactive=False, visible=False)
                sub_btn.click(
                    recognize,
                    inputs=[
                        det_model_name,
