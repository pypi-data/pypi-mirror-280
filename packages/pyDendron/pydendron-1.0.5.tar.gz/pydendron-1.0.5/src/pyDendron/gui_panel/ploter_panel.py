...
class PloterPanel(Viewer):
    ...
    def _sync_data(self, event):
        ...
        try:
            print('ploter panel sync data', event)
            if comp():
                self._layout.loading = True
                lst = self.dataset_package.dt_data[KEYCODE].to_list() if self.dataset_package.dt_data is not None else []
    ...
    def on_active_tab(self, event):
        print('on_active_tab:', self.tabs.active, self.dataset_package.dt_data is not None, self.ploter.data is not None, self.light_ploter.data is not None)
    ...
    def light_plot(self):
        sliding_keycode = self.sliding_select.value if self.sliding_select.value is not None else 'None'
        fixed_keycode = self.fixed_select.value if self.fixed_select.value is not None else 'None'
        df = self.dataset_package.dt_data
        print('light_plot:', sliding_keycode, fixed_keycode)
        if (sliding_keycode != 'None') and (fixed_keycode != 'None') and (df is not None):
