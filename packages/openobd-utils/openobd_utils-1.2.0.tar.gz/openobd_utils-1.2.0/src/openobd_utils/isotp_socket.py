...
from openobd import *
from .stream_manager import StreamManager, TimeoutException
from .response_exceptions import *


class IsotpSocket(StreamManager):

    def __init__(self, client: OpenOBDSession, isotp_channel: IsotpChannel, timeout: float | None = 10):
        """
        Handles sending messages to the given channel and parsing incoming responses.

        :param client: an authenticated OpenOBDClient with which to start the stream.
        ...
        """
        super().__init__(client.open_channel)
        self.channel = isotp_channel
    ...
    def request(self, payload: str, timeout: float | None = None, silent: bool = False, tries: int = 1) -> str | None:
        ...
        message = IsotpMessage(channel=self.channel, payload=payload)
        timeout = timeout if timeout is not None else self.timeout

        for current_try in range(1, tries + 1):
            self.send(message, clear_received_messages=True)
    ...
    def _receive_and_handle_response(self, request_payload: str, timeout: float | None) -> str:
        ...
        try:
            response = self.receive(timeout=timeout)    # type: IsotpMessage
        except TimeoutException:
            raise NoResponseException(request=request_payload, response=None, request_id=self.channel.request_id, response_id=self.channel.response_id)
        ...
        try:
            ...
            e.set_request(request_payload)
            e.set_response(response_payload, overwrite=False)
            e.set_request_id(self.channel.request_id)
            e.set_response_id(self.channel.response_id)
            raise e
