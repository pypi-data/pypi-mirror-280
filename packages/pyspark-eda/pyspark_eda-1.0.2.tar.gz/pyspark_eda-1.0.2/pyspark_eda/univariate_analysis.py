...
def get_univariate_analysis(df, print_graphs=0, id_list=None):
    ...
    for column in numerical_columns:
        ...
        mean_value = float(df.filter(~df[column].isNull()).select(round(F.mean(column), 3)).first()[0])  # Updated line
        mode_value = float(df.filter(~df[column].isNull()).groupBy(column).count().orderBy(F.desc('count')).select(column).first()[0])  # Updated line
        null_count = df.filter(df[column].isNull()).count()
        null_percentage = round_off((null_count / total_count) * 100)
        skewness_value = float(df.filter(~df[column].isNull()).select(round(F.skewness(column), 3)).first()[0])  # Updated line
        kurtosis_value = float(df.filter(~df[column].isNull()).select(round(F.kurtosis(column), 3)).first()[0])  # Updated line
        stddev_value = float(df.filter(~df[column].isNull()).select(round(F.stddev(column), 3)).first()[0])  # Updated line
