...
def get_univariate_analysis(df, print_graphs=0, id_list=None):
    ...
    for column in numerical_columns:
        ...
        mean_value = df.filter( ~(df[column].isNull()) ).select( round(F.mean(column) , 3)   ).first()[0]
        ...
        skewness_value = df.filter( ~(df[column].isNull()) ).select( round(F.skewness(column) , 3 ) ).first()[0] 
        kurtosis_value = df.filter( ~(df[column].isNull()) ).select(  round(F.kurtosis(column) ,3 )  ).first()[0]
        stddev_value = df.filter( ~(df[column].isNull()) ).select(   round(F.stddev(column)  , 3 ) ).first()[0]
