    ...
    if correlation_analysis and numerical_columns:
        ...
        for i in range(len(numerical_columns)):
            for j in range(i + 1, len(numerical_columns)):
                ...
                correlations.append((col1, col2, round_off(corr,int = 4)))
        ...
        for i in range(len(categorical_columns)):
            for j in range(i + 1, len(categorical_columns)):
                ...
                if counts.size > 0:  # Check if the matrix is non-empty
                    v = cramers_v(counts)
                    cramer_results.append((col1, col2, round_off(float(v),int = 4)))
    ...
    if anova_analysis and numerical_columns and categorical_columns:
        ...
        for num_col in numerical_columns:
            for cat_col in categorical_columns:
                ...
                anova_results.append((num_col, cat_col, round_off(float(f_val),int = 4), round_off(float(p_val), int = 4)))
