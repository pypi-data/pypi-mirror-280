...
class SensitivityAnalysisReader(Reader):
    ...
    def _read_exploration_strategy_list(cls,
        ...
        for row in sensitivity_analysis_dictionary[SensitivityAnalysisSheet.SHEET].values():
            ...
            current_line_strategy = xlsx_utils.clean_str_cell_else_raise(row[SensitivityAnalysisSheet.EXPLORATION_STRATEGY],
                                                  exception_message_hint=exception_message_hint)
            current_line_parameter_name = xlsx_utils.clean_str_cell_else_raise(row[SensitivityAnalysisSheet.PARAMETER_NAME],
                                                        exception_message_hint=exception_message_hint)
            current_line_values = row[SensitivityAnalysisSheet.VALUES]
            current_line_values = [current_line_values] if isinstance(current_line_values,
                                                                      (int, float)) \
                else xlsx_utils.clean_list_cell(current_line_values)
