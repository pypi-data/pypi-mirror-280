...
class StatThresholdAnomaliser(BaseSeriesAnnotator):
    """Anomaly detection based on thresholding the values of segment statistics.

    Parameters
    ----------
    change_detector : BaseSeriesAnnotator
    ...
    def _predict(self, X: Union[pd.DataFrame, pd.Series]) -> pd.Series:
        ...
        self.change_detector.fmt = "dense"
        self.change_detector.labels = "int_label"
        self.segments = self.change_detector.predict(X)

        df = pd.concat([X, self.segments], axis=1)
        self.anomalies = []
        for _, segment in df.reset_index(drop=True).groupby("int_label"):
            segment_stat = self.stat(segment.iloc[:, 0].values)
            if (segment_stat < self.stat_lower) | (segment_stat > self.stat_upper):
                self.anomalies.append((int(segment.index[0]), int(segment.index[-1])))
        ...
        return format_anomaly_output(
            ...
            X.index,
            self.anomalies,
            scores=self.change_detector.scores,
        )
    ...
    def get_test_params(cls, parameter_set="default"):
        ...
        params = [
            {
                "change_detector": Moscore(bandwidth=10),
                "stat": np.mean,
                "stat_lower": -1.0,
                "stat_upper": 1.0,
            },
