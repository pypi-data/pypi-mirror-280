...
class RCModel(ConfMeta):
    ...
    @classmethod
    def set_conf(cls, conf, db_model=None):
        cls.conf = conf
        if db_model:
    ...
    @classmethod
    async def cache_by_pk(cls, pk_val: Union[bytes, int, str], split: Union[str, int, float, datetime, date] = None):
        async def from_db():
            ...
            if db_info:
                if cls.expired_mode:
                    ...
                    await cls.conf.rds.set_hash(tb_name, pk_val, json_encode(db_info))
    ...
    #             info = json_parse(item, log_fun=cls.log_err)
    ...
    #             for item in all_items if (info := json_parse(item, cls.log_err))]

    @classmethod
    async def get_name(cls, pk_id: int or str, key_field='name'):
        if not pk_id:
            return '-'
    ...
    @classmethod
    async def get_map(cls, pk_id: int or str, key_list: list or tuple = None):
        if not pk_id:
            return {}
        info = await cls.cache_by_pk(pk_id)
