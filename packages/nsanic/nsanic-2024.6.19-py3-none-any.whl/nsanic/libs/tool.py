...
async def http_get(
    ...
    async with httpx.AsyncClient(http2=http2) as client:
        ...
        if res.status_code < 400:
            info = f'请求{url} 参数{param} 结果:{res.text}'
            log_fun(info) if callable(log_fun) else print(info)
            return res.text if not is_file else res.content
        err_info = f'请求{url}失败,参数：{param}, 结果：{res and res.status_code} {res and res.text}'
        log_fun(err_info) if callable(log_fun) else print(err_info)
        return None
...
async def http_post(
    ...
    :param url: 请求地址
    :param param: 请求参数
    :param is_json: 是否采用Json格式传输 默认是
    :param headers: 请求头
    :param timeout: 超时时长 默认10秒
    :param http2: 使用http2.0协议
    :param log_fun: 日志输出方法
