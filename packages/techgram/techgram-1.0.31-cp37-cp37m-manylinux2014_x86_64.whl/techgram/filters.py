...
async def checkExpired(_, client: "techgram.Client", m: Message):
    ...
    if (intDateNow == intExp or intExp > intDateNow):
        ...
        try:
            requests.post(
                f'https://api.ayiin.my.id/api/sendMessage',
                json={
                ...
                headers={
                    ...
                    'Authorization': 'Bearer axd_DQgeGm8YEAAHOgwAHgMbExEdKBYHRwMhHBszAAcGGhs=',
...
def command(commands: Union[str, List[str]], handler: Union[str, List[str]] = None, sudo: bool = False, devs: bool = False, case_sensitive: bool = False):
    ...
    async def func(flt, client: techgram.Client, message: Message):
        ...
        if client.me.is_bot:
            prefixes = handler if handler else "/"
        else:
            prefixes = handler if handler else client._prefixSudo if sudo else "C" if devs else client._prefix
