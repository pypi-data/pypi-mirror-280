    ...
    if test_running:
        ...
        try:
            ...
            def generate():
                ...
                process = subprocess.Popen(f"{python_path} {main_file} {build_path}", stdout=subprocess.PIPE,
                                           stderr=subprocess.STDOUT)

                def read_until_eof(stream):
                    decoder = codecs.getincrementaldecoder("utf-8")(errors='replace')  # 使用'replace'策略处理非法字符
                    while True:
