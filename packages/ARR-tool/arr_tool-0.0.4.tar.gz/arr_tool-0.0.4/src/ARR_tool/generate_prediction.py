...
def plot_confusion_matrix(cm, path, classes, normalize = False,
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        ...
        # Save the plot
        plt.savefig(path['plots'] + 'arr_confusion_matrix.png')
...
( data['AdvConsequences'] == adv)]
    ...
    aware_score = awareness_date(pd.to_datetime(pred_date), 
                                 pd.to_datetime(match['Latest AwareDate'].values[0]))    
...
def predict_decisions(pred_date,config_path):
    ...
    data_on_pred_date[col_].to_excel(path['prediction'] + 'Prediction.xlsx',index=None)

    cm = confusion_matrix(data_on_pred_date['ReportabilityDecision'], data_on_pred_date['Prediction'])
    plot_confusion_matrix(cm, path, classes=['0 - Not Reportable', '1 - Reportable'],date=pred_date)
