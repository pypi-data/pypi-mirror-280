...
def update(config_path):
    ...
    last_update_date = list(pd.to_datetime(train_data['Latest AwareData']).sort_values().values)[-1]
    ...
    case1_data =pd.DataFrame(columns=['PatientInvolvement', 'ProductLine', 'ProductFamily', 'HazardLevel1',
        ...
        'AdvConsequences', 'Last 5 Prediction', 'Latest AwareData',
    ...
    for i in range(len(data_to_update)):
        try:
            ...
            if(len(df_temp)>0):
                train_data.loc[df_temp.index,'Latest AwareData'] = data_to_update[['AwarenessDate']].iloc[i,:].values
                ...
                new_entry = pd.DataFrame({
                        ...
                        'Latest AwareData': data_to_update[['AwarenessDate']].iloc[i,:].values[0],       
                        'Decision Type': 'A', 
