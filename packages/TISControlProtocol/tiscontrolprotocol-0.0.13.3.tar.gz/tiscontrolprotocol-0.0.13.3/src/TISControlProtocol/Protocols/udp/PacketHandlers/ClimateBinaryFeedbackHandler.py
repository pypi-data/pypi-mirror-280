...
async def handle_climate_binary_feedback(hass: HomeAssistant, info: dict):
    ac_number = AC_NUMBER_MAP.get(info["additional_bytes"], None)
    ...
    try:
        hass.bus.async_fire(f"tis_ac_{ac_number}", event_data)
        logging.error(f"ac binary feedback event: {event_data}")
