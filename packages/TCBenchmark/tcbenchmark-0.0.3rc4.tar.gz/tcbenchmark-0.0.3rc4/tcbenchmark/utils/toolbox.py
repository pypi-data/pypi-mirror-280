...
def get_timeseries_sets(splits: dict, season_dict: dict, **kwargs):
    ...
    if test_strategy in ["last", "random", "first"]:
        ...
        if test_strategy == "last":
            ...
            if "validation" in splits.keys():
                val_size = int(np.ceil(splits["validation"] * len(available_seasons)))
                val_set = random_generator(other_seasons, val_size, replace=False)
            ...
            if data is not None and (len(candidate_timestamps) > timeseries_steps):
                ...
                if data_series.shape[-1] != len(
                    ...
                    if setdata is None:
                        setdata = {
                            "data": data_series,
                            "intensity_deltas": rolling_delta,
                            "base_intensities": rolling_base,
                        }
                    else:
                        setdata["timeseries_data"] = np.vstack(
                            (setdata["data"], data_series)
                        )
                        setdata["intensity_deltas"] = np.vstack(
                            (setdata["intensity_deltas"], rolling_delta)
                        )
                        setdata["base_intensities"] = np.vstack(
                            (setdata["base_intensities"], rolling_base)
                        )
