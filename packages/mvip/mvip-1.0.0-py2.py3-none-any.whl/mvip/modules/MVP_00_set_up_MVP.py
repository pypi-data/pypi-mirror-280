...
def fetch_arguments(parser):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Install geNomad and CheckV databases in the respective repositories. (default = True).",
    )
    parser.add_argument(
        '--genomad_db_path', 
        type=str,
...
def create_output_folders(args):
    ...
    print(f"\n\033[1m{print_step(create_output_folders)}: subset test (needs to be removed)...\033[0m")

def install_databases(args):
    if args["install_databases"] is False:
        ...
        if os.path.exists(genomad_db_path):
            ...
            subprocess.run(['genomad', 'download-database', genomad_db_path])
        ...
        if os.path.exists(checkv_db_path):
            ...
            subprocess.run(['checkv', 'download_database', checkv_db_path])
...
"""
    ...
    with open(summary_report_path, 'w') as summary_report:
        ...
        # For each tool, find it in conda list and get its version
        for tool in tools:
            try:
                result = subprocess.run(
                    ["conda", "list", tool], capture_output=True, text=True, check=True
                )
                lines = result.stdout.splitlines()
                if len(lines) > 3:
                    version_line = lines[3]  # Assuming the version info is in the 4th line
