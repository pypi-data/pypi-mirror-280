...
def _upgrade_root(profiles_root: str):
    call_gui = _path(profiles_root, _file_call_gui)
...
def _cmdline():
    if argv:
        if len(argv) == 1:
            _load_profile(argv[0])
        else:
            print(f"[ CMDLINE ]/parse:", argv)
            _config_key = argv.pop(0)
            if _config_key == "/":
                _load_profile(argv.pop(0))
            while argv:
                if _config_key.endswith("]"):
                    _config_key, _idx = _config_key[:-1].split("[")
                    _attr = getattr(rconfig, _config_key)
                    _attr.__setitem__(int(_idx), type(_attr[0])(argv.pop(0)))
                    ...
                    _globals[_config_key] = _attrt(argv.pop(0))
                try:
                    _config_key = argv.pop(0)
                except IndexError:
                    break
...
try:
    if argv[0] == "demo":
        ...
        if argv and argv[0] == "all":
            ...
            _profile_folder = _path(__profiles_home__, _profile_prefix + argv[1])
        else:
        ...
        if argv:
            ...
            try:
                ...
                pass
            try:
                colors = argv.remove("colors")
            except ValueError:
                pass
            try:
                plugins = argv.remove("plugins")
            except ValueError:
                pass
            try:
                things = argv.remove("things")
            except ValueError:
                pass
        else:
