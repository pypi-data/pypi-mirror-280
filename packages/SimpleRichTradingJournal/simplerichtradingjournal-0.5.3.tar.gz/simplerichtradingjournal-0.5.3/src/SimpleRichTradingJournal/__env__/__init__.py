...
from __ini__ import FLAGS, DIRECTIVES
...
PONG_URL = _path(URL, _folder_profile_assets, _file_pong)


def make_pong_file(server_pid):
    with open(_path(__profile_folder__, _folder_profile_assets, _file_pong), "w") as f:
        f.write(f"[{PROFILE}]\n"
                f"main    : {getpid()}\n"
                f"server  : {server_pid}")


def _ping() -> bytes | bool:
    try:
...
if FLAGS.ping:
    if ping := _ping():
        ...
        print("\x1b[31m[ 404 ]\x1b[m", PONG_URL)
        exit(1)

elif FLAGS.kill:
    if ping := _kill():
        print("\x1b[31m[ KILL ]\x1b[m", ping.decode())
        exit()
    else:
        print("\x1b[31m[ 404 ]\x1b[m", PONG_URL)
        exit(1)
