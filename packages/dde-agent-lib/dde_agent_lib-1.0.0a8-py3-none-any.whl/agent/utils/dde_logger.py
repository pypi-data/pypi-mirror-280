...
class DdeLogger:
    ...
    def _init_logger(cls):
        ...
        # 创建一个处理器，用于写入日志文件
        file_handler = NbLogFileHandler(file_name=log_file, log_path=log_directory, max_bytes=20*1000*1000, back_count=10)
        ...
        sls_handler = QueuedLogHandler(end_point='cn-hongkong.log.aliyuncs.com',
                                       access_key_id='LTAI5tKZ4iBRyMhmKSTxQ4DX',
                                       access_key='ZUL6joK4VBZEQvGH8KbTaJjZNBf38V', project='dde-engine-log',
                                       log_store=log_store)
        ...
        # nacos_logger.propagate = False

env = os.environ.get("ENVIRONMENT")


def format_log(level=LogTypeEnum.WARNING, _type=None, subtype=TaskSubtypeEnum.DEFAULT.value, environment=env, content=None, status=0, errorCode="0",
               timecost=0, extra=None):
    kwargs = locals()
    kwargs.pop("level")
    if timecost != 0:
