...
def get_app_config():
    ...
    if _app_config is not None and environment == _app_environment:
        return _app_config
    if environment == "dev":
        with open('apps/agentcore/config/app_dev.yaml') as f:
            _app_config = yaml.safe_load(f)
    elif environment == "test":
        with open('apps/agentcore/config/app_test.yaml') as f:
            _app_config = yaml.safe_load(f)
    elif environment == "prod":
        with open('apps/agentcore/config/app_prod.yaml') as f:
            _app_config = yaml.safe_load(f)
    elif environment == "pre_prod":
        with open('apps/agentcore/config/app_pre_prod.yaml') as f:
            _app_config = yaml.safe_load(f)
    else:
