...
def get_ths_concept_detail_by_app(symbol: str = "886019") -> pd.DataFrame:
    ...
    try:
        ...
        if 'change' in items_df.columns:
            items_df['change'] = 0
        if 'exchange' in items_df.columns:
            items_df['exchange'] = 0
        if 'q_ratio' in items_df.columns:
            items_df['q_ratio'] = 0
        if 'pct_chg' in items_df.columns:
            items_df['pct_chg'] = 0
        if 'pe' in items_df.columns:
            items_df['pe'] = 0
        if 'flow_mv' in items_df.columns:
            items_df['flow_mv'] = 0
        if 'total_mv' in items_df.columns:
            items_df['total_mv'] = 0
