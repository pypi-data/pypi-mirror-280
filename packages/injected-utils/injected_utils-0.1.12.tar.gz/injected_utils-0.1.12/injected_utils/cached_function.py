...
class AsyncCachedFunction:
    ...
    value_invalidator: Callable[[Any], Union[bool, Awaitable[bool]]] = field(default=lambda x: False) _concurrent_validation_count: int = field(default=0)
    ...
    async def __call__(self, *args, **kwargs):
        ...
        async with self._lock_map.get(key):
            ...
            if cause := await self._invalidate_value(result):
                raise RuntimeError(
                    f"the calculated result for cache is invalid! :{self.name}! {str(args)[:100]}...,{str(kwargs)[:100]}... \n cause:{cause}")
            if invalidated:
                logger.info(
