                ...
                while n_images < new_n_images:
                    try:
                        ...
                        if len(profile_link):
                            ...
                            upload_time_link = await page.locator(".user-area time").all()
                            if len(upload_time_link):
                                dt = await upload_time_link[0].get_attribute("datetime")
                                upload_datetime = int(
                                    (
                                        datetime.strptime(dt, "%Y-%m-%d %H:%M") - datetime(1970, 1, 1, 0, 0, 0)
                                    ).total_seconds()
                                )
                                logger.info(f"{upload_datetime=}")
                        ...
                        yield CrawlerContent(
                            ...
                            priority_timestamp=upload_datetime,
                            url=full_local_url,
                        )
