...
import pandas as pd
from fudstop._markets.list_sets.dicts import healthcare,energy,etfs,real_estate,financial_services,communication_services,consumer_cyclical,consumer_defensive,utilities,industrials,basic_materials,technology
...
class MasterSDK:
    def __init__(self):
        ...
        self.markets = WebullMarkets(host='localhost', database='markets', password='fud', port=5433, user='chuck')
        self.occ = occSDK(host='localhost', database='markets', password='fud', port=5433, user='chuck')
        self.fed = FedNewyork()
        self.fedprint = FedPrint()
        self.client = OpenAI(api_key=os.environ.get('OPENAI_KEY'))
        self.ew = EarningsWhisper() #sync
        self.treas = Treasury(host='localhost', database='fudstop', password='fud', port=5433, user='chuck')
        self.poly = Polygon(host='localhost', database='fudstop', password='fud', port=5433, user='chuck')
        self.poly_opts =PolygonOptions(host='localhost', database='fudstop', password='fud', port=5433, user='chuck')
        self.db= PolygonDatabase(host='localhost', database='fudstop', password='fud', port=5433, user='chuck')
    ...
    async def ew_sentiment(self, limit:int=20, insert:bool=False):
        ...
        data = self.ew.get_top_sentiment()
