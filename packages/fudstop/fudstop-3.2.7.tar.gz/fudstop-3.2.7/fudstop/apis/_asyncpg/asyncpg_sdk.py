...
class AsyncpgSDK:
    ...
    async def connect(self):
        ...
        self.pool = await create_pool(
            host=self.host, user=self.user, database=self.database, port=5433, password=self.password, min_size=1, max_size=40
        )
    ...
    async def fetch_table_data(self, table_name: str):
        ...
        query += " ORDER BY insertion_timestamp DESC LIMIT 50" if 'history' not in table_name else ";"
        ...
        if rows:
            ...
            data_dict = {col: [] for col in columns}  # Initialize a dictionary with column names as keys
        
            for row in rows:
                for col in columns:
                    data_dict[col].append(row[col])  # Append each value to the appropriate list in the dictionary
