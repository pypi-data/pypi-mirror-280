...
def transform_id(transform: st.Transform) -> str:
    ...
    elif spec_type == "custom_code":
        digest = str(spec.custom_code.digest)
        return f"{spec_type}.{digest}"
    else:
        return spec_type
...
def custom_code(image_name: str, digest: str) -> Transform:
    return Transform(
        sp.Transform(
            name="custom_code",
            spec=sp.Transform.Spec(
                custom_code=sp.Transform.CustomCode(
                    image_name=image_name, digest=digest
                )
            ),
            inversible=True,
            schema_preserving=True,
