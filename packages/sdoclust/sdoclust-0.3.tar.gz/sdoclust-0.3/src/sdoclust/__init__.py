...
def keep_label_consistency(O,ol,O_old,ol_old):

    dist = distance.cdist(O, O_old)
    dist_sorted = np.argsort(dist, axis=1)
    closest = dist_sorted[:,0].flatten()
    ol_closest = ol_old[closest]

    ol_vect = np.empty(len(ol),dtype=complex)
    ol_vect.real = ol_closest 
    ol_vect.imag = ol
    lost_clusters = np.setdiff1d(np.unique(ol_old), np.unique(ol_closest))

    u,c = np.unique(ol_vect, return_counts=True)
    i = np.argsort(c)[::-1]
    u,c = u[i],c[i]
    labels = list(np.arange(len(u)+len(lost_clusters)))
    new_labels = list(set(labels) - set(np.unique(ol_old)))

    ol_new = np.zeros(len(ol)).astype(int)
    for e in u:
        if e.real in labels:
            ol_new[ol_vect==e] = e.real
            labels.remove(e.real)
        else:
            ol_new[ol_vect==e] = new_labels[0]
            new_labels.remove(new_labels[0])
...
class SDOclust:
    ...
    def __init__(self, x=5, qv=0.3, hbs=False, smooth=False, smooth_f=0.25, rseed=5, zeta=0.6, chi_min=8, chi_prop=0.05, e=3, chi=None, cb_outlierness=False, xc=None, k=None, q=None, chunksize=None):
        self.x = x
        self.qv = qv
        self.hbs = hbs
        self.smooth = smooth
    ...
    def update(self, X):
        ...
        ol = keep_label_consistency(O,ol,old_O,old_ol)
