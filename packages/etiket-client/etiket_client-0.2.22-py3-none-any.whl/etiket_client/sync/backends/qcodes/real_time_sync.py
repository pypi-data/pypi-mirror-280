...
import sqlite3, logging, time, io
...
def QCoDeS_live_sync(run_id : int, database : str, datasetUUID : UUID):
    ...
    try: 
        while not (check_complete(c, run_id) and sync_idx == getMaxId(c, table_name)):            
            ...
            data = c.fetchall()
            if len(data) == 0:
                if t_last_fetch + 120 < time.time():
                    logger.warning("No new data found in the last two minutes. Exiting real time sync.")
                    break
                time.sleep(0.1)
                continue
            else:
        ...
        logging.exception("Real time sync failed with error :: %s", e)
    finally:
        dc.complete()
