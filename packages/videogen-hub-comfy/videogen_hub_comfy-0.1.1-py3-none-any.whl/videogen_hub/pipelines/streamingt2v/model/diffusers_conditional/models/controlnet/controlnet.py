...
class Merger(nn.Module):
    def __init__(
        ...
        if merge_mode.startswith("attention"):
            self.attention = ConditionalModel(
        ...
        if x.shape[1] - condition_signal.shape[1] > 0:
            if self.frame_expansion == "last_frame":
                fillup_latent = repeat(
                    condition_signal[:, -1],
                    "B C H W -> B F C H W",
                    F=x.shape[1] - condition_signal.shape[1],
                )
            elif self.frame_expansion == "zero":
                fillup_latent = torch.zeros(
                    (
                        x.shape[0],
                        self.n_frames_sample - self.n_frames_condition,
                        *x.shape[2:],
                    ),
                    device=x.device,
                    dtype=x.dtype,
            ...
            if self.frame_expansion != "none":
                condition_signal = torch.cat([condition_signal, fillup_latent], dim=1)
...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(block_out_channels) - 1):
            ...
            self.blocks.append(
                nn.Conv2d(
                    channel_in, channel_out, kernel_size=3, padding=1, stride=stride
                )
            )
            if use_normalization:
                res = res // 2
        ...
        if not final_3d_conv:
            self.conv_out = zero_module(
                nn.Conv2d(
                    block_out_channels[-1] + int(use_controlnet_mask),
...
class ControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # conv_in
        self.merger = Merger(
            n_frames_sample=num_frames,
            n_frames_condition=num_frame_conditioning,
            merge_mode=merging_mode,
        ...
        self.controlnet_down_blocks.append(
            ZeroConv(
                channels=output_channel, mode=zero_conv_mode, num_frames=num_frames
            )
        )
    ...
    def from_unet(
        ...
        **kwargs,
        ...
        if load_weights_from_unet:
            ...
            controlnet.transformer_in.load_state_dict(unet.transformer_in.state_dict())
            ...
            )  # can be that the controlnet model does not use image clip encoding
    ...
    def forward(
        ...
        if encoder_hidden_states.shape[1] > 77:
            ...
            context_text, context_img = (
                encoder_hidden_states[:, :77, :],
                encoder_hidden_states[:, 77:, :],
            )
            context_text = context_text.repeat_interleave(repeats=num_frames, dim=0)

            if self.image_encoder_name == "FrozenOpenCLIPImageEmbedder":
                context_img = context_img.repeat_interleave(repeats=num_frames, dim=0)
            else:
                context_img = rearrange(
                    context_img, "b (t l) c -> (b t) l c", t=num_frames
                )

            encoder_hidden_states = torch.cat([context_text, context_img], dim=1)
        else:
        ...
        if num_frames > 1:
            if self.gradient_checkpointing:
                ...
                sample = self.transformer_in(
                    sample, num_frames=num_frames, attention_mask=attention_mask
                ).sample

        sample = self.merger(
            sample * weight_control_sample, weight_control * controlnet_cond
        )
