...
class OptimizerParams:
    def __init__(
        ...
        self.load_trained_base_model_and_resampler_from_ckpt = (
...
class InferenceParams:
    def __init__(
        ...
        self.height = height
        self.video_length = (
            video_length if isinstance(video_length, int) else int(video_length)
        )
    ...
    def to_dict(self):
        ...
        keys = [
            ...
            if not callable(getattr(self, entry)) and not entry.startswith("__")
        ]
...
class AttentionMaskParams:
    ...
    def __init__(
        ...
        self.temporal_self_attention_only_on_conditioning = (
...
class UNetParams:
    ...
    def __init__(
        ...
        assert (
            not temporal_self_attention_only_on_conditioning
        ), "This parameter is only here for backward compatibility. Set AttentionMaskParams instead."
        assert (
            not temporal_self_attention_mask_included_itself
        ), "This parameter is only here for backward compatibility. Set AttentionMaskParams instead."
        ...
        if spatial_latent_input:
            assert (
                unet_from_diffusers
            ), "Spatial latent input only implemented by original diffusers model. Set 'model.unet_params.unet_from_diffusers=True'."
