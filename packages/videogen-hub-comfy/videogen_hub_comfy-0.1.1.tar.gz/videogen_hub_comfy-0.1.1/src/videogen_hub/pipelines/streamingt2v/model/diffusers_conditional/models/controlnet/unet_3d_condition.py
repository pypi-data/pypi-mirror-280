...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.conv_in = Conv2D_ExtendedChannels(
            ...
            in_channel_extension=5 if self.concat else 0,
        )
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            if self.merging_mode.startswith("attention"):
                for idx in range(3):
                    self.cross_attention_merger_down_blocks.append(
                        ConditionalModel(
                            input_channels=(
                                input_channel if idx == 0 else output_channel
                            ),
                            conditional_model=self.merging_mode.split("attention_")[1],
                        )
                    )
        ...
        self.conv_out = Conv2D_ExtendedChannels(
            ...
            out_channel_extension=2 if channel_expansion else 0,
        )
        ...
        if down_block_additional_residuals is not None:
            ...
            if self.merging_mode == "addition":
                ...
                for (
                    ...
                    self.cross_attention_merger_down_blocks,
                ):
        ...
        if mid_block_additional_residual is not None:
            if self.merging_mode == "addition":
                ...
                sample = self.cross_attention_merger_mid_block(
                    rearrange(sample, "(B F) C H W -> B F C H W", B=batch_size),
                    rearrange(
