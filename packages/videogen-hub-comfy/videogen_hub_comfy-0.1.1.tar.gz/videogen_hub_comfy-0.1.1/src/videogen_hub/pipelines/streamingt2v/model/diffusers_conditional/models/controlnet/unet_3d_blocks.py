...
def transformer_g_c(transformer, sample, num_frames):
    sample = g_c(
        custom_checkpoint(transformer, mode="temp"),
        sample,
        num_frames,
        use_reentrant=False,
    )
...
def cross_attn_g_c(
    ...
    def ordered_g_c(idx):
        ...
        if idx == 0:
            return g_c(
                custom_checkpoint(attn, mode="attn"),
                hidden_states,
                encoder_hidden_states,
                cross_attention_kwargs,
                attention_mask,
        ...
        if idx == 1:
            return g_c(
                custom_checkpoint(temp_attn, mode="temp"),
                hidden_states,
                num_frames,
                use_reentrant=False,
            )
        ...
        if idx == 2:
            return g_c(
                custom_checkpoint(resnet, mode="resnet"),
                hidden_states,
                temb,
                use_reentrant=False,
            )
        ...
        if idx == 3:
            return g_c(
                custom_checkpoint(temp_conv, mode="temp"),
                hidden_states,
                num_frames,
                use_reentrant=False,
            )
    ...
    def forward(
        ...
        if self.gradient_checkpointing:
            hidden_states = up_down_g_c(
                self.resnets[0], self.temp_convs[0], hidden_states, temb, num_frames
            )
        else:
