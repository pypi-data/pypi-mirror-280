...
def parse_args(arg_list):
    ...
    parser.add_argument("--save_img_path", type=str,
                        default="./src/videogen_hub/pipelines/opensora_plan/sample_videos/t2v")
...
class OpenSoraPlanPipeline():
    def __init__(self, arg_list, device):
        self.args = parse_args(arg_list)
        self.device = device

    def inference(self, save_output):
        # torch.manual_seed(args.seed)
        args = self.args
        torch.set_grad_enabled(False)
        device = self.device

        vae = getae_wrapper(args.ae)(args.model_path, subfolder="vae", cache_dir=args.cache_dir).to(device,
                                                                                                    dtype=torch.float16)
        ...
        # Load model:
        transformer_model = LatteT2V.from_pretrained(args.model_path, subfolder=args.version, cache_dir=args.cache_dir,
        ...
        if args.force_images:
            save_image(video_grids / 255.0, os.path.join(args.save_img_path,
            ...
            imageio.mimwrite(os.path.join(args.save_img_path,
                                          f'{args.sample_method}_gs{args.guidance_scale}_s{args.num_sampling_steps}.{ext}'),
                             video_grids, fps=args.fps, quality=9)
            print('save path {}'.format(args.save_img_path))
        else:
            return video_grids
