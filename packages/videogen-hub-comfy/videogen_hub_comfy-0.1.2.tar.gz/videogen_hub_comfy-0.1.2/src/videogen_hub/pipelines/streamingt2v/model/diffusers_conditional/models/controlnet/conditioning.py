...
class CrossAttention(nn.Module):
    ...
    def __init__(self, input_channels, attention_head_dim, norm_num_groups=32):
        ...
        self.attention = Attention(
            query_dim=input_channels,
            cross_attention_dim=input_channels,
            heads=input_channels // attention_head_dim,
    ...
    def forward(self, hidden_state, encoder_hidden_states, num_frames):
        ...
        hidden_state_norm = rearrange(hidden_state_norm, "B C F H W -> (B H W) F C")
        ...
        residual = rearrange(residual, "(B H W) F C  -> (B F) C H W", H=h, W=w)
...
class ConditionalModel(nn.Module):
    ...
    def __init__(self, input_channels, conditional_model: str, attention_head_dim=64):
        ...
        if conditional_model == "self_cross_transformer":
            ...
            self.temporal_transformer = TransformerTemporalModelCrossAttn(
                num_attention_heads=input_channels // attention_head_dim,
                attention_head_dim=attention_head_dim,
                in_channels=input_channels,
                double_self_attention=False,
            ...
            raise NotImplementedError(f"mode {conditional_model} not implemented")
    ...
    def forward(self, sample, conditioning, num_frames=None):
        ...
        if self.conditional_model != "test_conv":
            ...
            sample = self.temporal_transformer(
                sample, encoder_hidden_states=conditioning, num_frames=num_frames
            ).sample
