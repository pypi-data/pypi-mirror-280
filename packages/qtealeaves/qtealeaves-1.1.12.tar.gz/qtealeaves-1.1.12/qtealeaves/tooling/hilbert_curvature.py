    ...
    **Attributes**
    ...
    def __init__(self, dim, size):
        ...
        if dim == 1:
            ...
            raise Exception("No Hilbert curve beyond 3d supported.")
    ...
    def init_1d(self, size):
        ...
        **Arguments**
        ...
        """
        raise Exception(str(self) + ": no mapping needed for 1d case.")
    ...
    def init_2d(self, size):
        ...
        **Arguments**
        ...
        if len(size) != 2:
            raise Exception("Dimension is 2d, but size is not of length 2.")
    ...
    def init_3d(self, size):
        ...
        **Arguments**
        ...
        if len(size) != 3:
            raise Exception("Dimension is 3d, but size is not of length 3.")
    ...
    def __call__(self, coord):
        ...
        **Arguments**
        ...
        if not isinstance(coord, int):
            raise Exception("Call method expects integer since v0.2.22.")
    ...
    def calculate_size(self):
        ...
        raise Exception("Dimensionality not implemented.")
    ...
    def init_3d(self, size):
        ...
        **Arguments**
        ...
        if size[0] >= size[1] >= size[2]:
            ...
            raise Exception("Case not covered; please report as a bug.")
...
def map_selector(dim, size, map_type):
    ...
    **Arguments**
    ...
    elif map_type == "SnakeMap":
        return SnakeMap(dim, size)
    elif map_type == "ZigZagMap":
        return ZigZagMap(dim, size)
    elif isinstance(map_type, HilbertCurveMap):
        return map_type
    elif map_type is None:
        raise Exception("Map type was not set by QuantumModel; report as bug.")

    raise Exception('Unknown map_type "%s".' % (map_type))
...
def hilbert_curve_2d(nx, ny):
    ...
    **Arguments**
    ...
    if abs(np.log2(nx) - int(np.log2(nx))) > 1e-14:
        raise Exception("Lenght nx must be a power of 2.")
    if abs(np.log2(ny) - int(np.log2(ny))) > 1e-14:
        raise Exception("Lenght ny must be a power of 2.")
    ...
    for ii in range(nx):
        for jj in range(ny):
            ...
            if top and down:
                ...
                raise Exception("Case not printed.")
...
def hilbert_curve_3d(nx, ny, nz):
    ...
    **Results**
    ...
    if abs(np.log2(nz) - int(np.log2(nz))) > 1e-14:
        raise Exception("Lenght nz must be a power of 2.")
...
def main(*args):
    ...
    if len(args) == 2:
        ...
        raise Exception("Number of arguments not valid.")
