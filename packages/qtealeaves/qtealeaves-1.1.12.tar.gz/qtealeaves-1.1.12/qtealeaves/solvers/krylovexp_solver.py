...
class KrylovSolverH:
    ...
    **Arguments**
    ...
    vec0 : vector to apply epxonential matrix to.
    ...
    def solve(self, verbose=0):
        ...
        for ii in range(self.nn_max):
            ...
            precision_fom = 2.0 * abs(self.sdiag[ii]) * abs(krylov_exp_mat[ii, 0])

            if precision_fom < self.tolerance:
                nn = ii + 1
                if verbose > 0:
                    print(
                        f"Krylov solver converged in {nn} steps with {precision_fom}."
                    )
                break

            if ii + 1 == self.nn_max:
                if verbose > 0:
                    print(f"Krylov solver stopped at max_iter with {precision_fom}.")
                break
...
class KrylovSolverNH:
    ...
    **Arguments**
    ...
    def init_basis(self):
        ...
        self.hessenberg = np.zeros((dim, dim), dtype=dtype)
    ...
    def solve(self, verbose=0):
        ...
        for ii in range(self.nn_max):
            ...
            precision_fom = 2.0 * norm * np.abs(krylov_exp_mat[0, 0])

            if precision_fom < self.tolerance:
                nn = ii + 1
                if verbose > 0:
                    print(
                        f"KrylovSolverNH convergence in {nn} steps with {precision_fom}."
                    )
                break

            if ii + 1 == self.nn_max:
                if verbose > 0:
                    print(f"KrylovSolverNH stopped at max_iter with {precision_fom}.")
                break
