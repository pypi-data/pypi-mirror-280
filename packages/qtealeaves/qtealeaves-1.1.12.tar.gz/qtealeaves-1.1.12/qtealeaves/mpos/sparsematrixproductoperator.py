...
class SparseMPOSites(_RestrictedList):
    ...
    def __init__(self, num_sites, operators, do_vecs=True):
        super().__init__()

        for ii in range(num_sites):
            operator_eye = operators[(ii, "id")]
            site = SparseMatrixOperatorPy(
                ii == 0, ii + 1 == num_sites, do_vecs, operator_eye
            )
            self.append(site)
...
class SparseMPO(_AbstractEffectiveOperators):
    ...
    def __init__(self, num_sites, operators, do_vecs=True):
        super().__init__()
        self.site_terms = SparseMPOSites(num_sites, operators, do_vecs=do_vecs)
    ...
    def contract_tensor_lists(self, tensor, pos, pos_links, custom_ops=None):
        ...
        if shape[0] != shape[1]:
            print("Error information", shape)
            raise Exception("Failed to contract to square sparse MPO.")
        ...
        if shape[0] == 1:
            ...
            if tmp is None:
                ...
                raise Exception("Diagonal empty in SPO.")
            ctens = tmp
    ...
    def print_summary(self, log_file=None):
        ...
        myprint("=" * 20 + f" Contraction summary SPO {mode_str} " + "=" * 20)
