...
class _TNObsBase(_ParameterizedClass):
        ...
        for name in self.name:
            if name not in all_results:
                ...
                if isinstance(new[name], dict):
                    raise Exception("Dictionary addition not implemented.")
            else:
                all_results[name] += new[name]
        return all_results
    ...
    def write_results(self, fh, is_measured, **kwargs):
        ...
        **Arguments**
        ...
        raise Exception("Must be implemented by actual class for len > 0.")
...
def deb_print(*args, verbosity_level=1):
    """
    Utility function for print messages during debugging.

    **Arguments**

    All positional arguments will be forwarded to the
    print function.

    verbosity_level : int, optional
        Sets the verbosity level, which can then
        be adapted here during debugging.
    """
