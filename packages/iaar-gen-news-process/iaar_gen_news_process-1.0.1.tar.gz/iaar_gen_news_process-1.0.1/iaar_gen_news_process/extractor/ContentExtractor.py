...
from Utils.gne.utils import iter_node, pad_host_for_images, config, get_high_weight_keyword_pattern
...
class ContentExtractor:
    ...
    def extract(self, selector, host='', body_xpath='', with_body_html=False, use_visiable_info=False):
        ...
        for node in iter_node(body):
            ...
            images_list_desc = [img for img in images_list if 'data:image' not in img]
            host = host or config.get('host', '')
            if host:
                images_list = [pad_host_for_images(host, url) for url in images_list_desc]
