...
class TestGenericRepository(unittest.TestCase):
    ...
    def test_6_create_table_by_dataframe(self):
        with self.db_connection.session_scope() as session:
            session.execute(text(f"DROP TABLE IF EXISTS {self.table_name}"))
        data = pd.DataFrame(
            [
                {"id": 1, "name": "John Doe", "age": 30,
                    "predict": "Positive", "score": 0.932},
                {"id": 2, "name": "Jane Smith", "age": 25,
                    "predict": "Negative", "score": 0.1332},
                {"id": 3, "name": "Bob Johnson", "age": 35,
                    "predict": "Positive", "score": 0.53},
            ]
        )
    ...
    def test_7_update_table_by_dataframe_with_versioning(self):
        with self.repo.db.session_scope() as session:
            session.execute(text(f"DROP TABLE {self.table_name}"))
        data = pd.DataFrame(
            [
                {"id": 1, "name": "John Doe", "age": 30, "predict": "",
                    "created_at": "2021-01-01", "version": 1},
                {"id": 2, "name": "Jane Smith", "age": 25, "predict": "",
                    "created_at": "2021-01-01", "version": 1},
                {"id": 3, "name": "Bob Johnson", "age": 35,
                    "created_at": "2021-01-01", "version": 1, "predict": ""},
            ]
        )
        ...
        df = self.repo.to_dataframe(self.table_name)
        self.assertEqual(len(df), 3)
        self.assertListEqual(list(df["name"]), ["John Doe", "Jane Smith", "Bob Johnson"])
        self.assertListEqual(list(df["predict"]), ["Positive", "Positive", "Negative"])
