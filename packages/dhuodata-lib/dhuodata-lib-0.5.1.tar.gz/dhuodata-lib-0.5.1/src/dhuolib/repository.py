...
class GenericRepository:
        ...
        for col, dtype in df.dtypes.items():
            if pd.api.types.is_integer_dtype(dtype):
                ...
                if engine.dialect.name == 'oracle':
                    dtype_mapping[col] = Float(precision=53).with_variant(OracleFLOAT(binary_precision=53), 'oracle')
                else:
    ...
    def create_table_by_dataframe(self, table_name: str, df: pd.DataFrame):
        with self.db.session_scope() as session:
            ...
            num_rows_inserted = df.to_sql(
                name=table_name, con=session.bind, if_exists="replace", index=False, dtype=dtype_mapping)
    ...
    def update_table_by_dataframe(
        self, table_name: str, df_predict: pd.DataFrame, if_exists: str = "append", is_update_version: bool = False
    ):
        with self.db.session_scope() as session:
