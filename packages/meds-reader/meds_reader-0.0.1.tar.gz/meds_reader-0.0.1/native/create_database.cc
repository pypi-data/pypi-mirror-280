...
std::pair<size_t, std::vector<uint32_t>> write_files(
    ...
    std::ofstream output_file(target_filename, std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
...
void process_string_property(const std::string& property_name, std::filesystem::path temp_path, const std::vector<WorkEntry>& work_entries, int num_threads) {
    ...
    {
        std::ofstream zdict(string_path / std::string("zdict"), std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
    ...
    std::ofstream data_file((string_path / "data"), std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
    ...
    for (const auto& entry : work_entries) {
        ...
        std::ifstream entry_file(entry_path, std::ios_base::in | std::ios_base::binary);
...
std::tuple<uint32_t, std::vector<int64_t>, std::vector<uint32_t>, std::vector<uint32_t>> get_patient_ids(
    ...
    std::ofstream output_file(target_path, std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
...
void process_patient_id_and_time(std::filesystem::path temp_path, const std::vector<WorkEntry>& work_entries, int num_threads) {
    ...
    {
        std::ofstream pids_file(temp_path / "patient_id", std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
        std::ofstream lengths_file(temp_path / "length", std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);

        std::ofstream data_file((temp_path / "time"), std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
...
void create_database(const char* source, const char* destination, int num_threads) {
    ...
    std::filesystem::copy(source_path / "metadata.json", destination_path / "metadata.json");
    ...
    {
        std::ofstream property_file(destination_path / "properties", std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
    ...
    for (const auto& property : properties) {
        if (property.second->Equals(std::make_shared<arrow::StringType>())) {
            process_string_property(property.first, destination_path, work_entries, num_threads);
        }
    }
