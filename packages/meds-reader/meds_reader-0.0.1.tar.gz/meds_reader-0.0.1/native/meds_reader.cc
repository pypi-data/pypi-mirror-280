...
public:
    ...
    absl::Span<const T> data() const {
        return absl::Span<const T>((const T*) data_pointer, data_size / sizeof(T));
    }
...
public:
    PerfectHashMap(std::vector<T*> items) {
        ...
        if (values.size() != items.size()) {
            throw std::runtime_error("Very serious bug in perfect hash function, bad input?");
        }
        ...
        while (true) {
            ...
            if (num_tries > 1000) {
                throw std::runtime_error("Serious bug in perfect hash map, tried 1000 times");
            }
...
struct PropertyReader {
    virtual std::vector<PyObjectWrapper> get_property_data(int32_t patient_offset, int32_t length) = 0;
    virtual ~PropertyReader() {};
};

std::unique_ptr<PropertyReader> create_property_reader(std::filesystem::path root_directory, const std::string& property_name, arrow::Type::type property_type);

struct PatientDatabaseState : std::enable_shared_from_this<PatientDatabaseState> {
    ...
    std::vector<PyObjectWrapper> get_property_data(size_t index, int32_t patient_offset, int32_t length);
    ...
    std::vector<std::pair<std::string, arrow::Type::type>> properties;
...
PyObject* create_event(std::shared_ptr<PatientData> patient, int32_t event_offset);
...
struct PatientData {
    ...
    PatientData(std::shared_ptr<PatientDatabaseState> pd, int32_t po) : patient_database(pd), patient_offset(po), patient_length(pd->get_patient_length(po)), saved_properties(pd->get_num_properties()) {

    }

    PyObject* get_property(PyObject* property_name, size_t event_index) {
        // Needs to get the property
        std::optional<size_t> index = patient_database->get_property_index(property_name);

        if (!index) {
            // Does not contain
            return PyErr_Format(PyExc_AttributeError, "Could not find key %U in meds_reader.Event", property_name);
        }
        ...
        if (!val) {
            // Force init it
            val.emplace(patient_database->get_property_data(*index, patient_offset, patient_length));
        }
...
struct Patient {
    ...
    alignas(std::shared_ptr<PatientData>) char state_data[sizeof(std::shared_ptr<PatientData>)];
...
PyMemberDef PatientTypeMembers[] = {
    {.name = "patient_id", .type=T_OBJECT_EX, .offset=offsetof(Patient, patient_id), .flags=READONLY},
    {.name = "events", .type=T_OBJECT_EX, .offset=offsetof(Patient, events), .flags=READONLY},
    {nullptr},
};

PyTypeObject PatientType = {
    .ob_base = PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "meds_reader.Patient",
...
PyObject* create_patient(std::shared_ptr<PatientDatabaseState> patient_database, PyObject* patient_id) {
    std::optional<int32_t> patient_offset = patient_database->get_patient_offset(PyLong_AsLongLong(patient_id));

    if (!patient_offset) {
        return PyErr_Format(PyExc_KeyError, "Could not find patient_id %S in meds_reader.PatientDatabase", patient_id);
    }
    ...
    return wrap([&](){
        actual_self->state = new (actual_self->state_data) std::shared_ptr<PatientData>(std::make_shared<PatientData>(patient_database, *patient_offset));
        ...
        for (int32_t i = 0; i < length; i++) {
            PyTuple_SET_ITEM(actual_self->events, i, create_event(*(actual_self->state), i));
        }
...
struct EventData {
    ...
    EventData(std::shared_ptr<PatientData> pd, int32_t ei) : patient_data(pd), event_index(ei) {};
...
PyObject* event_getattro(PyObject* self, PyObject* key) {
    ...
    return wrap([&](){
        return actual_self->state->get_property(key);
    });
}

PyTypeObject EventType = {
    .ob_base = PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "meds_reader.Event",
...
PyObject* create_event(std::shared_ptr<PatientData> patient, int32_t event_index) {
    PyObjectWrapper self{EventType.tp_alloc(&EventType, 0)};

    Event* actual_self = (Event*) self.borrow();

    return wrap([&]() {
        actual_self->state = new (actual_self->state_data) EventData(patient, event_index);
...
struct StringPropertyReader : PropertyReader {
    StringPropertyReader(std::filesystem::path property_path): zdict_file(property_path / "zdict"), data_file(property_path / "data"), context(ZSTD_createDCtx(), context_deleter) {
        {
            ...
            while (true) {
                ...
                PyObject* value_string = PyUnicode_FromStringAndSize(val.data(), val.size());
        ...
        size_t ret = ZSTD_DCtx_loadDictionary_byReference(context.get(), (void*)zdict_file.bytes().data(), zdict_file.bytes().size());
        if (ZSTD_isError(ret)) {
    ...
    std::vector<PyObjectWrapper> get_property_data(int32_t patient_offset, int32_t length) {
        uint32_t offset = data_file.data<uint32_t>()[patient_offset];
        uint32_t num_bytes = data_file.data<uint32_t>()[patient_offset + 1] - offset;

        uint32_t* length_pointer = (uint32_t*)(data_file.bytes().data() + offset);
        ...
        size_t ret = ZSTD_decompressDCtx(context.get(), decompressed.data(), decompressed.size(), data_file.bytes().data() + offset + sizeof(uint32_t), num_bytes - sizeof(uint32_t));
        ...
        size_t num_read = streamvbyte_decode((const uint8_t*)decompressed.data() + sizeof(uint32_t), values.data(), value_size);

        if (num_read + sizeof(uint32_t) > decompressed_size) {
            throw std::runtime_error("Decoded too much? " + std::to_string(num_read) + " " + std::to_string(decompressed_size));
        }
        ...
        while (result.size() < (size_t) length) {
            ...
            for (size_t j = 0; j < bitset.size() && result.size() < (size_t) length; j++) {
                if (bitset[j]) {
                    ...
                    if (next_code % 2 == 0) {
                        ...
                        PyObject* value_string = PyUnicode_FromStringAndSize(view.data(), view.size());
                        if (value_string == nullptr) {
                            throw std::runtime_error("Could not create unicode string");
                        }
...
struct TimePropertyReader : PropertyReader {
    TimePropertyReader(std::filesystem::path property_path): data_file(property_path) {}
    ...
    std::vector<PyObjectWrapper> get_property_data(int32_t patient_offset, int32_t length) {
        ...
        size_t num_read = streamvbyte_decode_0124((const uint8_t*)data_file.bytes().data() + offset + sizeof(uint32_t), values.data(), num_values);

        if (num_read + sizeof(uint32_t) != num_bytes) {
            throw std::runtime_error("Decoded too much? " + std::to_string(num_read) + " " + std::to_string(num_bytes));
        }
        ...
        auto add_times = [&](uint32_t copies) {
            PyObjectWrapper dt{PyDateTime_FromDateAndTime(day.year(), day.month(), day.day(), day.hour(), day.minute(), day.second(), microseconds)};
...
std::unique_ptr<PropertyReader> create_property_reader(std::filesystem::path root_directory, const std::string& property_name, arrow::Type::type property_type) {
    if (property_type == arrow::Type::STRING) {
        return std::make_unique<StringPropertyReader>(root_directory / property_name);
    } else if (property_name == "time") {
        if (property_type != arrow::Type::TIMESTAMP) {
            throw std::runtime_error("Time type must be a timestamp");
        }
        return std::make_unique<TimePropertyReader>(root_directory / property_name);
    } else {
        throw std::runtime_error("Unsupported property " + property_name + " " + arrow::internal::ToString(property_type));
    }
}

PyObjectWrapper create_pyarrow_dtype(PyObject* pyarrow, arrow::Type::type type) {
    const char* name;
    ...
    switch (type) {
        ...
        case arrow::Type::INT64:
            ...
            break;
        default:
            throw std::runtime_error("Unhandled type " + std::to_string(type) + " " + arrow::internal::ToString(type));        
    }
    ...
    if (arg == nullptr) {
        ...
        if (py_arg.borrow() == nullptr) {
            throw std::runtime_error("Could not create string for arrow type argument?");
        }

        result = PyObject_CallMethodOneArg(pyarrow, py_name.borrow(), py_arg.borrow());
    }
...
PatientDatabaseState::PatientDatabaseState(std::string_view dir) : root_directory(dir), patient_id_file(root_directory / "patient_id"), length_file(root_directory / "length") {
    {
        ...
        while (current.size() != 0) {
            ...
            static_assert(sizeof(arrow::Type::type) <= sizeof(int64_t));
            int64_t casted_type = *((int64_t*) current.data());
            current = current.substr(sizeof(casted_type));
            arrow::Type::type type = static_cast<arrow::Type::type>(casted_type);

            PyObjectWrapper property_name{PyUnicode_FromStringAndSize(next_name.data(), next_name.size())};
            if (property_name.borrow() == nullptr) {
                throw std::runtime_error("Could not construct a property name string?");
            }

            PyUnicode_InternInPlace(&property_name.ref);
            properties.emplace_back(std::move(next_name), type);

            PyObjectWrapper dtype = create_pyarrow_dtype(pyarrow.borrow(), type);

            property_names.emplace_back(property_name.borrow());

            if (PyDict_SetItem(py_properties.borrow(), property_name.steal(), dtype.borrow()) == -1) {
                throw std::runtime_error("Could not insert properties in map ...");
            }
...
std::optional<size_t> PatientDatabaseState::get_property_index(PyObject* property_name) {
    PyUnicode_InternInPlace(&property_name);
    return property_map->get_index(property_name);
}

std::vector<PyObjectWrapper> PatientDatabaseState::get_property_data(size_t index, int32_t patient_offset, int32_t length) {
    if (property_accessors[index] == nullptr) {
        property_accessors[index] = create_property_reader(root_directory, properties[index].first, properties[index].second);
    }
    return property_accessors[index]->get_property_data(patient_offset, length);
}

int64_t PatientDatabaseState::get_patient_id(int32_t patient_offset) const {return patient_id_file.data<int64_t>()[patient_offset];}

uint32_t PatientDatabaseState::get_patient_length(int32_t patient_offset) const {return length_file.data<uint32_t>()[patient_offset];}


std::optional<int32_t> PatientDatabaseState::get_patient_offset(int64_t patient_id) {
...
PyObjectWrapper PatientDatabaseState::get_metadata() {
    if (py_metadata.borrow() == nullptr) {
        ...
        PyObjectWrapper metadata_string{PyUnicode_FromStringAndSize(metadata_file.bytes().data(), metadata_file.bytes().size())};
        py_metadata = PyObject_CallMethodOneArg(json.borrow(), loads.borrow(), metadata_string.borrow());
    }
...
struct PatientDatabase {
    ...
    alignas(std::shared_ptr<PatientDatabaseState>) char state_data[sizeof(std::shared_ptr<PatientDatabaseState>)];
...
int patient_database_init(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    auto result = wrap([&]() {
        actual_self->state = new (actual_self->state_data) std::shared_ptr<PatientDatabaseState>(std::make_shared<PatientDatabaseState>(std::string_view(path_str, path_size)));
        return str;
    });
...
PyObject* patient_database_reduce(PyObject* self, PyObject* Py_UNUSED(unused)) {
    ...
    PyObject* path_string = PyUnicode_FromStringAndSize(path.data(), path.size());

    if (path_string == nullptr) {
        return PyErr_Format(PyExc_RuntimeError, "Could not convert the database path to a string ...");
    }
...
PyMethodDef patient_database_methods[] = { 
    {"__reduce__", patient_database_reduce, METH_NOARGS, "Save the patient database"},
    {nullptr}
};
...
PyObject* patient_database_iterator_next(PyObject *self) {
    PatientDatabaseIterator* actual_self = (PatientDatabaseIterator*) self;
    PatientDatabase* patient_database = (PatientDatabase*) actual_self->patient_database;

    if (actual_self->index >= (*(patient_database->state))->num_patients) {
        return PyErr_Format(PyExc_StopIteration, "Exceeded the size of the PatientDatabase");
    }
    
    static_assert (sizeof(int64_t) == sizeof(long long));

    return wrap([&](){
         int64_t patient_id = (*patient_database->state)->get_patient_id(actual_self->index++);
...
PyTypeObject PatientDatabaseIteratorType = {
    .ob_base = PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "meds_reader.PatientDatabaseIterator",
...
PyObject* patient_database_iter(PyObject* self) {
    PyObject* iter = PatientDatabaseIteratorType.tp_alloc(&PatientDatabaseIteratorType, 0);
...
PyTypeObject PatientDatabaseType = {
    .ob_base = PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "meds_reader.PatientDatabase",
...
PyObject* convert_to_patient_database(PyObject* self, PyObject* args) {
    const char* source;
    const char* destination;
    Py_ssize_t num_threads = 1;

    if (!PyArg_ParseTuple(args, "ss|n:convert_to_patient_database", &source, &destination, &num_threads)) {
        return nullptr;
    }

    return wrap([&]() {
        create_database(source, destination, num_threads);
        Py_RETURN_NONE;
    });
}

PyMethodDef meds_reader_methods[] = { 
    {"convert_to_patient_database", convert_to_patient_database, METH_VARARGS, "Save the patient database"},
    {nullptr}
};
