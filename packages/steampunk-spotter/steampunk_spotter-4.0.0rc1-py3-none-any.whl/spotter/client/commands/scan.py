...
def _parser_callback(args: argparse.Namespace, project_root: Path) -> None:  # pylint: disable=unused-argument
    ...
    with progress:
        ...
        if import_payload:
            parsing_result = ParsingResult(
                tasks=[], playbooks=[], dynamic_inventories=[], roles=[], module_defaults=[], errors=[]
            )
            scan_payload = PayloadV4.from_args(parsing_result, environment, include_metadata, import_payload)
        ...
        if export_payload:
            ...
            try:
                formatted_output = scan_result.format_output(
                    parsing_result.errors, output_format, no_color, no_docs_url, no_scan_url, rewrite
                )
                if output_path:
                ...
                if sarif:
                    sarif_string = scan_result.format_sarif(no_docs_url)
