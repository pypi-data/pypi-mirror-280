...
from ctqkit.exceptions import CtqKitRequestError, CtqKitInputParaError
from ctqkit.utils.laboratory_utils import LaboratoryUtils

logger = logging.getLogger('ctqkit')
...
class BasePlatform:
    ...
    def login(self) -> int:
        ...
        if res.status_code != 200:
            raise CtqKitRequestError('Login failed: request interface failed', res.status_code)

        data = res.json()
        if data.get('code', -1) != 0:
            raise CtqKitRequestError('Login failed')
    ...
    def query_experiment(
        ...
        raise CtqKitRequestError("Failed to query the experimental result.")
    ...
    def re_execute_task(
        ...
        if not lab_id and not query_id:
            raise CtqKitInputParaError("Please provide lab_id or query_id.")
        data = {
            "lab_id": lab_id,
            "query_id": query_id
        }
    ...
    def _send_request(
        ...
        if res.status_code != 200:
            raise CtqKitRequestError(f'Request API failed: {res.text}', res.status_code)
        result = res.json()
        if raise_for_code and result.get('code', -1) != 0:
            raise CtqKitRequestError(f'Request error: {res.text}')
        return result
