...
def _get_computer_data(params):
    ...
    """
    url_manager = URLManager("gd")
    laboratory_url = url_manager.get_url("laboratory_url")
    res = requests.get(f"{laboratory_url}sdk/quantumComputer/config", params=params)
    status_code = res.status_code
    if status_code != 200:
        if status_code == 401:
            print("无法获取拓扑图数据")
            return 0
        raise Exception("无法获取拓扑图数据, status_code:{status_code}")
    result = json.loads(res.text)
    data = result.get("data")
    code = result.get("code", -1)
    msg = result.get("message", "无法获取拓扑图数据")
    if code != 0 or data is None:
        print(
            f"绘图错误，没有获取到拓扑图数据：{msg}(可能原因：1.服务器端原因 2.参数machine_name输入有误)"
        )
        return 0
    return data
