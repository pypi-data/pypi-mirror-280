...
class GoogleContacts(GoogleAccountLocal, metaclass=MetaLogger,
    ...
    def __init__(self):
        ...
        self.user_externals_local: UserExternalsLocal = None    # TODO: Do we still need this here?
    ...
    def pull_people_api(self, email_address: str, username: str, groups_dicts: list[dict] = None) -> str:
        ...
        data_source_instance_id = self.__insert_data_source_instance_id(email=email_address, user_external_id=user_external_id)
    ...
    def pull_contacts_with_stored_token(self, email_address: str, username: str, groups_dicts: list[dict] = None,
        ...
        self.logger.info("Getting token data from DB", object={
            "email": email_address, "profile_id": self.user_context.get_effective_profile_id()})

        token_data = self.user_externals_local.get_auth_details(  # TODO: fix typing
            username=username, system_id=GoogleContactConstants.GOOGLE_SYSTEM_ID, profile_id=self.profile_id)
        if not token_data:
        ...
        self.pull_people_api(email_address=email_address, username=username, groups_dicts=groups_dicts)
    ...
    def __insert_link_contact_groups(self, contact_dict: dict) -> list:
        ...
        for group in self.groups_list:
            groups.append(group)
        if len(groups) > 0:
            linked_groups_results_list = self.contact_group.insert_link_contact_group_with_group_local(
