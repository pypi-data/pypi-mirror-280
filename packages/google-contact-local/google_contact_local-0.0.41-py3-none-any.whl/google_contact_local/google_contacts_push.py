...
class GoogleContactsPush(GoogleContacts):
    ...
    def create_and_push_contacts(
            self, *, username: str = None, data_source_types_ids_list: list = None,
            data_source_instances_ids_list: list = None, order_by: str = None, limit: int = 1) -> list[dict]:
        self.logger.start('create_and_push_contacts',
                          object={'username': username, 'data_source_types_ids_list': data_source_types_ids_list,
                                  'data_source_instances_ids_list': data_source_instances_ids_list, 'order_by': order_by,
                                  'limit': limit})
        username = username or our_get_env('PRODUCT_USER_IDENTIFIER', raise_if_empty=True)
        result_dicts_list = []
        try:
            contacts_ids_list = self.get_contact_ids_with_no_google_people_api_resource_name(
                limit=limit, data_source_types_ids_list=data_source_types_ids_list,
                data_source_instances_ids_list=data_source_instances_ids_list, order_by=order_by)
            for contact_id in contacts_ids_list:
                result_dict = self.create_and_push_contact_by_contact_id(username=username, contact_id=contact_id)
                result_dicts_list.append(result_dict)
        except Exception as exception:
            self.logger.exception('Exception in create_and_push_contacts', object={'exception': exception})
            raise exception
        self.logger.end('create_and_push_contacts', object={'contacts_ids_list': contacts_ids_list})
        return result_dicts_list

    def create_and_push_contact_by_contact_id(self, *, username: str, contact_id: int) -> dict:
        self.logger.start('create_and_push_contact_by_contact_id', object={'contact_id': contact_id})
        result_dict = {}
        try:
            google_contact_dict = self.get_google_contact_dict_by_contact_id(contact_id=contact_id)
            google_contact = self.create_contact_by_google_contact_dict(
                username=username, google_contact_dict=google_contact_dict)
            groups_titles_to_resource_name = self.create_non_existing_google_contact_groups(contact_id=contact_id)
            ...
            result_dict["groups_titles_to_resource_name"] = groups_titles_to_resource_name
        except Exception as exception:
    ...
    def create_contact_by_google_contact_dict(self, *, username: str, google_contact_dict: dict) -> dict:
        ...
        google_contact = self.service.people().createContact(body=google_contact_dict).execute()
        return google_contact

    def get_contact_ids_with_no_google_people_api_resource_name(
            self, *, limit: int = 1, data_source_types_ids_list: list = None,  data_source_instances_ids_list: list = None,
    ...
    def get_google_contact_dict_by_contact_id(self, contact_id: int) -> dict:
        ...
        self.__create_google_contact_dict(
            contact_view_select_result_dict=contact_view_select_result_dict, google_contact_dict=google_contact_dict)
        self.logger.end('get_contact_dict_by_contact_id', object={'google_contact_dict': google_contact_dict})
        return google_contact_dict

    def __create_google_contact_dict(self, *, contact_view_select_result_dict: dict,
                                     google_contact_dict: dict):
        ...
        notes = self.select_one_value_by_column_and_value(
            schema_name='contact_note', view_table_name='contact_note_view',
            select_clause_value='note', column_name='contact_id', column_value=contact_view_select_result_dict['contact_id'])
        ...
        start_timestamp = self.select_one_value_by_where(
            schema_name='profile_profile', view_table_name='profile_profile_view',
            select_clause_value='start_timestamp',
            where='profile_id1 = %s AND profile_id2 = %s',
            params=(contact_view_select_result_dict['owner_profile_id'], contact_view_select_result_dict['main_profile_id']))
        if start_timestamp:
    ...
    def create_non_existing_google_contact_groups(self, *, contact_id: int) -> dict:
        ...
        local_groups_dicts_lists: list[dict] = self.contact_group.get_groups_of_contact_by_contact_id(contact_id=contact_id)
        local_groups_titles_list: list[str] = []
        for local_groups_dict in local_groups_dicts_lists:
            local_groups_titles_list.append(local_groups_dict['title'])
        ...
        for group in google_groups_lists:
            groups_titles_to_resource_name[group['name']] = group['resourceName']
        ...
        for group_title in local_groups_titles_list:
            ...
            groups_titles_to_resource_name_to_link[group_title] = groups_titles_to_resource_name[group_title]
    ...
    def add_contact_to_group(self, *, contact_resource_name: str, group_resource_name: str):
        ...
        self.service.contactGroups().members().modify(
            resourceName=group_resource_name, body=member).execute()

    def link_contact_to_groups(self, *, contact_resource_name: str, groups_titles_to_resource_name: dict):
        for google_group_title, resource_name in groups_titles_to_resource_name.items():
            self.add_contact_to_group(contact_resource_name=contact_resource_name, group_resource_name=resource_name)

    def delete_google_contact(self, contact_resource_name: str):
        try:
            # TODO: add call to api-management
            self.service.people().deleteContact(resourceName=contact_resource_name).execute()
            self.logger.info('Deleted contact', object={'contact_resource_name': contact_resource_name})
        except Exception as exception:
            self.logger.exception('Exception in delete_google_contact', object={'exception': exception})

    def delete_google_contact_group(self, group_resource_name: str):
        try:
            self.service.contactGroups().delete(resourceName=group_resource_name).execute()
            self.logger.info('Deleted group', object={'group_resource_name': group_resource_name})
        except Exception as exception:
            self.logger.exception('Exception in delete_google_contact_group', object={'exception': exception})
    ...
    def delete_multiple_google_contacts(self, contact_resource_names: list):
        for contact_resource_name in contact_resource_names:
            try:
                ...
                self.logger.exception('Exception in delete_multiple_google_contacts', object={'exception': exception})

    def delete_multiple_google_contact_groups(self, groups_titles_to_resource_name: dict):
        for group_resource_name in groups_titles_to_resource_name.values():
            try:
                self.service.contactGroups().delete(resourceName=group_resource_name).execute()
            except Exception as exception:
                self.logger.exception('Exception in delete_multiple_google_contact_groups', object={'exception': exception})
    ...
    def delete_multiple_google_contacts_and_groups(
        ...
        groups_titles_to_resource_name = {}
        for google_contact_and_group_titles_to_resource_name_dict in google_contacts_and_groups_titles_to_resource_name_dict:
            contact_resource_names.append(google_contact_and_group_titles_to_resource_name_dict['contact_resource_name'])
            groups_titles_to_resource_name.update(
                google_contact_and_group_titles_to_resource_name_dict['groups_titles_to_resource_name'])
