...
def monitor_force_energy(directory: str, outdir: str = "."):
    ...
    ...
    # nstep-1 | energy | dE | time
    # nstep | energy | dE | time
    ...
    for i, subfolder in enumerate(subfolders):
        ...
        if i == 0 or i == Nimage - 1 and not os.path.isfile(logfile):
            continue

        folder_names.append(subfolder)
        forces = []
        if os.path.isfile(logfile):
            force_lines = os.popen(f"grep 'Max force:' {logfile}").readlines()
            ...
            with open(logfile, "r") as f:
                pattern = r"""
                    ...
                    """
                energies = re.findall(pattern, f.read(), re.MULTILINE | re.VERBOSE)
