...
def s6_2(language: Literal["EN", "CN"]):
    ...
    projs = ["elements"]
    D["pband"] = get_input(
        Dselect[language][18], projs, completer=WordCompleter(projs), allow_empty=True
    )
    projs2 = ["elements", "orbitals"]
    D["pdos"] = get_input(
        Dselect[language][18], projs2, completer=WordCompleter(projs2), allow_empty=True
    )
    import_thread.join()
    logger.info(Dio[language]["import"])
...
def s8_7(language: Literal["EN", "CN"]):
    ...
    D = {}
    D["menu"] = 87
    D["ind"] = prompt(Dio[language]["neb_unfinished"], completer=pc)
    D["outd"] = prompt(Dio[language]["outd"], completer=pc)
    import_thread.join()
    logger.info(Dio[language]["import"])
    monitor_force_energy(D["ind"], D["outd"])
    yaml.dump(D, yamllog)
