...
def getEmbeddingMLP(poses, embedding, selectorSide):
    ...
    for pose in poses:
        if isSide:
            ...
            isMinus = selectorSide.split(", ")[-1] == "MINUS"
            if isMinus:
                if left < right:
                    embeddings.append(getPoseEmbeddingList(pose, embedding + "LEFT"))
                else:
                    embeddings.append(getPoseEmbeddingList(pose, embedding + "RIGHT"))
            else:
                if left > right:
                    embeddings.append(getPoseEmbeddingList(pose, embedding + "LEFT"))
                else:
            ...
            embeddings.append(getPoseEmbeddingList(pose, embedding))
    return embeddings
