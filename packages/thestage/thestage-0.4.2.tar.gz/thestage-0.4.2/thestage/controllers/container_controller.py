...
@app.command(name='list', help=__("Show list containers"))
def list_items(
        ...
        statuses: List[ContainerStatusEnumDto] = typer.Option(
            ["RUNNING", "BUSY"],
            '--status',
            '-s',
    ...
    if ContainerStatusEnumDto.find_special_status(statuses=statuses):
        statuses = []
...
@app.command(name="item-details", help=__("Show details container"))
def item_details(
    ...
    typer.echo(__("STATUS: %status%", {'status': str(container.status_to_display)}))
    ...
    if container.instance_rented:
        ...
        typer.echo(
            __("INSTANCE STATUS: %instance_status%",
               {'instance_status': str(container.instance_rented.status_to_display)})
        )
    ...
    if container.selfhosted_instance:
        ...
        typer.echo(
            __("SELFHOSTED INSTANCE STATUS: %instance_status%",
               {'instance_status': str(container.selfhosted_instance.status_to_display)})
        )
...
@app.command(name="put-file", help=__("Help copy file to container"))
def put_file(
        container_slug: Optional[str] = typer.Option(
            None,
            '--container-uniqueid',
