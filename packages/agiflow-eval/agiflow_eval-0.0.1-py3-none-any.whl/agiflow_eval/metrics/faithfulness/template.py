...
CLAIMS_TEMPLATE = """Based on the given text, please generate a comphrensive list of FACTUAL claims that can inferred from the provided text.
...
**
IMPORTANT: Please make sure to only return in JSON format, with the "claims" key as value is a list of strings. No words or explanation is needed.
...
TRUTHS_TEMPLATE = """Based on the given text, please generate a comprehensive list of FACTUAL, undisputed truths that can inferred from the provided text.
...
VERDICTS_TEMPLATE = """Based on the given claims, which is a list of strings, generate a list of JSON objects to indicate whether EACH claim contradicts any facts in the retrieval context. The JSON will have 2 fields: 'verdict' and 'reason'.
...
REASON_TEMPLATE = """Below is a list of Contradictions. It is a list of strings explaining why the 'actual output' does not align with the information presented in the 'retrieval context'. Contradictions happen in the 'actual output', NOT the 'retrieval context'.
...
class FaithfulnessTemplate:
    def __init__(
        self,
        claims_template=None,
        truths_template=None,
        verdicts_template=None,
        reason_template=None,
    ):
        self.claims_template = claims_template or CLAIMS_TEMPLATE
        self.truths_template = truths_template or TRUTHS_TEMPLATE
        self.verdicts_template = verdicts_template or VERDICTS_TEMPLATE
        self.reason_template = reason_template or REASON_TEMPLATE

    def generate_claims(self, text=None):
        return Template(self.claims_template).render(text=text)

    def generate_truths(self, text=None):
        return Template(self.truths_template).render(text=text)

    def generate_verdicts(self, claims=None, retrieval_context=None):
        return Template(self.verdicts_template).render(
          claims=claims,
          retrieval_context=retrieval_context,
        )

    def generate_reason(self, score=None, contradictions=None):
