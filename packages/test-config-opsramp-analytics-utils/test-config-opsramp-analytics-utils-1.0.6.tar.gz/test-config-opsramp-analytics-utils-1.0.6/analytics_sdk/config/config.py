...
def init_config():
    ...
    if CONFIG_CONTENT is None:
        ...
        # CONFIGMAP_FILE_PATH = os.getenv('CONFIGMAP_FILE_PATH', '/etc/opsramp/opsramp-config-reportingapps.yml')
        CONFIGMAP_FILE_PATH = '/etc/opsramp/opsramp-config-reportingapps.yml' 
...
def load_value(prop_key, section=None, default_value=None):
    ...
    if _is_key_found_in_env(prop_key):
        print(prop_key, '::: foudn in os env and returning')
        value = _load_env_value(prop_key)
        return value
    print('\n============>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>going to load config map')
