...
class CNE:  
    ...
    _dictionary: dict = {
        ...
        "OUT_CONTRY": "Elector de un Centro de Votación que no está habilitado",
    ...
    def __init__(self, other_url: str = None) -> None:
    ...
    def query(self, nat: str, dni: str|int) -> ResponseData:
        ...
        try:
            r: requests.Response = requests.get(self.URL, params=self.payload)
        except:
           raise ConnectionError(self.err(0))
       
        if r.status_code == 200:
            content: str = self._parse_html(r.content)
            s: int | bool = self._valid_content(content)
            if s:
                self._result = ResponseData(f"{nat}-{str(dni)}", s)
            else:
                self._result: ResponseData = self._parse_text(content)
    ...
    def _valid_content(self, content: str) -> int|bool:
        ...
        elif self._dictionary['OUT_CONTRY'] in content:
            return STATUS.EXTRANGERO
        elif self._dictionary['OBJECTION'] in content:
            matchs: re.Match[str] | None = re.search(self._dictionary['OBJECTION_PATTERS'], content)
            if matchs:
                c: str = matchs.group(1).strip()
                o: str = re.sub(r"\(\d\)+","", c).title().strip()
                if o == "Fallecido":
    ...
    def _parse_text(self, content: str) -> ResponseData:
        patterns: list[str] = ['Cédula:', 'Nombre:', 'Estado:', 'Municipio:', 'Parroquia:', 'Centro:', 'Dirección:', 'Registro Electoral', 'Impresión de Consulta de Datos', 'SERVICIO ELECTORAL', 'Registro ElectoralCorte']
        for pattern in patterns:
            content = content.replace(pattern, '|')
        
        content = content.strip()
        response: list[str] = [re.sub(r'\s+', ' ', val.strip()) for val in content.split('|')]
        return ResponseData(
            response[1],
            STATUS.REGISTERED,
            response[2],
    ...
    def _process(self, data: list, with_tqdm=False) -> None:
        ...
        if with_tqdm:
            ...
            for i in data:
                try:
                    nal, dni = i.split("-")
                    resultconsult: ResponseData = self._cne.query(nal, int(dni))
                    self.results.append(resultconsult)
                except ConnectionError:
                    raise
                except:
