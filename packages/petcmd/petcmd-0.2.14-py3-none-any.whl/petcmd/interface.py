...
try:
	from __main__ import __file__ as tool_name
	print("first import", tool_name)
	if os.path.basename(tool_name) == "__main__.py":
		from __main__ import __package__ as tool_name
		print("second import", tool_name)
except ImportError:
	tool_name = "cli"
	print("replace with cli")
	...
	def command_usage(cls, command: Command):
		...
		def command_name_view() -> str:
			if command.cmds[0] == "__main__":
				return os.path.basename(tool_name).rsplit(".", 1)[0]
