...
class ActAPIMixin(PostAPIMixin):
    ...
    def set_request_act(request):
        request.request_act = request.request_data.pop("act", None)
...
class ModelPutAPIMixin(ModelAPIMixin, ActAPIMixin):
    put_select_params_list = [
        IntParam(name="id", must_has_value=True),
    ]
    put_params_list = []
...
class ModelDeleteAPIMixin(ModelAPIMixin, ActAPIMixin):
    delete_params_list = [
        ListParam(name="id_list", must_has_value=True, list_element=IntParam(name="int"), query_key="id__in"),
    ]
