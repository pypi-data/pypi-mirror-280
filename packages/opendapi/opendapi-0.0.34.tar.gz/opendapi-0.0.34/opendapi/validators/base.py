...
class BaseValidator:
    ...
    def __init__(
        ...
        self.config: OpenDAPIConfig = OpenDAPIConfig(root_dir)
        ...
        def _autoupdate_merge_strategy_for_dict_lists(config, path, base, nxt):
            ...
            for idx, itm in enumerate(base):
                filter_nxt_items = _get_match_using_lookup_keys(
                    nxt, itm, self.AUTOUPDATE_UNIQUE_LOOKUP_KEYS
                )
                if filter_nxt_items:
                    result.append(
            ...
            if self.AUTOUPDATE_UNIQUE_LOOKUP_KEYS:
                # Add only items from nxt
                # ONLY if they are not already merged earlier using AUTOUPDATE_UNIQUE_LOOKUP_KEYS
                for itm in addable_candidates:
                    result_match = _get_match_using_lookup_keys(
                        result, itm, self.AUTOUPDATE_UNIQUE_LOOKUP_KEYS
                    )
                    if result_match:
                        continue
