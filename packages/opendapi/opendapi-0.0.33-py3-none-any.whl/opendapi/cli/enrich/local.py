...
class Enricher:
    ...
    def setup_objects(self):
        ...
        # changes from the most recent commit. allows us to know if a push is missing schema info
        self.changed_files_from_current: OpenDAPIFileContents = (
            self._get_changed_files_from_current()
        )
    ...
    def _get_changed_files_from_current(self) -> OpenDAPIFileContents:
        """Get the changed OpenDAPI files from the current commit"""
        # Add untracked opendapi files created by `opendapi generate` or the user
        add_untracked_opendapi_files(self.root_dir)
    ...
    def validate_and_enrich(self) -> DAPIServerResponse:
        ...
        if self.should_enrich():
            self.all_files.update_dapis_with_suggestions(
