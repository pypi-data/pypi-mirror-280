...
def test_get_bcm_ip_local_ip():
    """
    Test that the function returns "623" for local IP addresses.
    """
    ip = "192.168.10.10"
    vlan_id = "50"
    expected_result = "192.168.50.10"
    assert get_bcm_ip(ip, vlan_id) == expected_result

def test_get_bcm_ip_public_ip():
    """
    Test that the function returns "623" for local IP addresses.
    """
    ip = "203.0.113.1"
    vlan_id = "50"
    expected_result = "203.0.113.1"
    assert get_bcm_ip(ip, vlan_id) == expected_result
