...
from smart_reboot.utils.conversion import get_bcm_ip, get_bcm_port, get_hostname
...
def parse_arguments():
    ...
    parser.add_argument('--bcm_vlan_id', type=str, help="BCM VLAN ID.")
...
def get_configuration(args):
    ...
    bcm_vlan_id_str = args.bcm_vlan_id or os.getenv('BCM_VLAN_ID')
    ...
    if any(not field for field in required_fields):
        raise ValueError(
            "Missing configuration: ensure servers, bcm_pass, bcm_pass, bcm_vlan_id_str, "
            "ssh_key_path, ssh_username, vast_api_key, and smart_reboot_dir_str "
            "are provided."
        )
    ...
    for server_ip, server_port in bcm_servers:
        server_info = {
            ...
            'bcm_ip': get_bcm_ip(server_ip, bcm_vlan_id_str),
