...
In addition to the three required files, a template can optionally contain a large number of other files, including:
...
The full list (at the time of writing) can be found in the file ``/u/sim/cmg/configs/retrieve/jetto`` on the JET
analysis clusters. The majority of these extra files are handled by the ``template`` module in a simple way, whereby the
...
As an example, let's assume that the current working directory contains a JSET, JETTO namelist and lookup file, which
are the minimum set of files required to create a template. We start by creating objects corresponding to each of the
files, using the functionality provided by their respective ``jetto_tools`` modules:
...
The ``Template`` class performs some validation to make sure that the provided files are consistent with each other.
...
their paths to the ``Template`` constructor using the ``extra_files`` keyword argument:

.. highlight:: python

>>> sanco_namelist = jetto_tools.namelist.read('jetto.sin')
>>> template = jetto_tools.template.Template(jset, namelist, lookup, extra_files=['/path/to/jetto.ex', '/path/to/jetto.bnd'])
...
Loading a template from a directory
-----------------------------------

Since it will often be the case that the template files have standard names and are located in the same directory as
each other, the ``template`` module provides a short-cut function which simplifies the process of creating a template
from the contents of a directory, by simply passing the path to the directory to the ``from_directory`` function:
...
Loading a template from files
-----------------------------

If the template files are located in different directories from each other, another function, called ``from_files`` is
provided to handle this situation e.g.:

.. highlight:: python

>>> template = jetto_tools.template.from_files('/path/to/jset/jetto.jset', '/path/to/namelist/jetto.in', '/path/to/lookup/lookup.json')

The ``from_files`` function also supports optional keyword arguments to supply additional files to the template. See the
API reference below for the full description.

Loading a template from the catalogue
-------------------------------------

A template can be loaded from a catalogued JETTO run, if the code is being run on the JET analysis clusters. In a
similar manner to JAMS, the load requires the components of the catalogue location: namely, the owner, machine, shot,
date and sequence identifiers, in addition to the location of the template lookup file e.g.

.. highlight:: python

>>> template = jetto_tools.template.from_catalogue('fcasson', 'jet', 92398, 'dec1318', 1, './lookup.json')

Note that, unlike in JAMS, it is not required to supply a code identifier: the catalogue code is automatically set
...
Calling ``from_catalogue`` triggers a call to the ``retrieve`` script provided in the JET cluster environment. The
...
* The lookup file is validated against the lookup schema defined by the ``lookup`` module.
...
* The type of the parameter (given by the ``type`` field) must match the value of the parameter in the JSET file (or the
  value must be trivially convertible to that type). Empty values are not allowed.
...
.. autofunction:: jetto_tools.template.from_files
