...
This module does not support creation of a JETTO configuration *ab initio*: template files must be provided, in which
all of the settings that the client wishes to modify are already present. A ``RunConfig`` object can be created from
these files, and the client can modify the configuration using the interface provided by the ``RunConfig`` class.

The ``RunConfig`` class supports viewing and setting of scalar parameter values, extra namelist arrays, and also
supports performing scans over selected parameters. In addition, a variety of other specific operations can be
performed, including selection of the JETTO binary to use, setting of time ranges, swapping of exfiles etc.
...
existing JETTO run directory (at example path ``/path/to/jetto/run/dir``) is as follows:

.. highlight:: python

>>> import jetto_tools.template
>>> template = jetto_tools.template.from_directory('/path/to/existing/jetto/run/dir')
...
A JETTO template may contain a large number of files (termed *extra files*), in addition to the core JSET, namelist(s),
and lookup files. At the present time, the only extra file that can be modified via the configuration API is the
ex-file (``jetto.ex``). The ``RunConfig`` class provides an attribute for it:
...
.. autoclass:: jetto_tools.config.RunConfig
    :members: __init__, exfile, binary, userid, serialise, processors, export, __getitem__, __setitem__, walltime, start_time, end_time, esco_timesteps, profile_timesteps
