...
class AsgardIndex(object):
    ...
    async def get_index_path(self,
        ...
        path: str = await self.get_index_path_without_version(*index_prefix, base_path=base_path)

        if not append_version:
            return path

        if version is None:
            _version = str(await self.get_version(path))
        else:
            _version = version
        path = os.path.join(path, "version=%s" % _version)
        logger.info("Index path with version - {}".format(path))
        return path

    async def get_index_path_without_version(self,
                                             *index_prefix,
                                             base_path=None) -> str:
        """
        :param base_path:
        ...
        logger.info("Index path without version - {}".format(path))
        return path

    async def get_version(self, path, increment=False):
        ...
        try:
            if await exists(session=self._fs, path=path):
                return max(
                    [
                        int(version_pattern.search(_path).group(1).rstrip("/"))
                        for _path in await listdir(session=self._fs, dir_path=path)
                    ]
    ...
    async def ensure_index_path(self, *index):
        path = await self.get_index_path_without_version(*index, base_path=self.base_path)
        if await exists(self._fs, path):
            return True
        return False
