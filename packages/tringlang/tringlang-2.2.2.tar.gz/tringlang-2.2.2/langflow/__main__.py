...
def print_banner(host: str, port: int):
    ...
    if langflow_version is None:  # This means langflow.version was not imported
        try:
            ...
            langflow_base_version = metadata.version("tringlang")
            is_pre_release |= is_prerelease(langflow_base_version)  # Update pre-release status
            notice = build_version_notice(langflow_base_version, "tringlang")
            notice = stylize_text(notice, "tringlang", is_pre_release)
            if notice:
                notices.append(notice)
            package_names.append("tringlang")
            package_name = "tringlang"
