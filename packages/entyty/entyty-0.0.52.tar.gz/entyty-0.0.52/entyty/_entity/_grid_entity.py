...
class GridEntity(AbstractGridEntity):
    ...
    def move(self, cell_to_move_to: object = None, teleport: bool = False):
        if not teleport:
            ...
            if isinstance(cell_to_move_to, str):
                cell_to_move_to = self.grid[cell_to_move_to]
            if cell_to_move_to is not None and not cell_to_move_to.passable:
    ...
    def _extracted_from_move_10(self, cell_to_move_to):
        ...
        self.movements += 1
        return f'Energy: {energy} - {cost} = {self.movement_energy}'
