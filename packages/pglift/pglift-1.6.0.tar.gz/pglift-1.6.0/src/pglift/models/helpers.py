...
from ..types import AnsibleArgSpec, AnsibleConfig, Port, StrEnum, field_annotation
...
V = TypeVar("V", str, dict[str, Any])


def check_excludes(forbidden: tuple[str, str], value: V) -> V:
    """Make sure that `value` string doesn't contain forbidden string.

    Raises ValueError otherwise.

    >>> check_excludes(["/", "slashes"], "all good")
    'all good'
    >>> check_excludes(["/", "slashes"], "this/is/bad")
    Traceback (most recent call last):
        ...
    ValueError: must not contain slashes
    >>> check_excludes(["password", "a password"], {"password": "paszW0rD"})
    Traceback (most recent call last):
        ...
    ValueError: must not contain a password
    """
    if forbidden[0] in value:
        raise ValueError(f"must not contain {forbidden[1]}")
    return value


def check_conninfo(value: str, exclude: Sequence[tuple[str, str]] = ()) -> str:
