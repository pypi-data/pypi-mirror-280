...
class RCBBatchProcess(RCallback):
    ...
    def _combine_measured_data(self) -> pd.DataFrame:
        ...
        if self.normalize_intensity_flag == normalization_modes.RAW_INTENSITIES:
            ...
            first_mean_values = all_data.groupby(['experimental group', 'experiment index'], as_index=False, sort=False).head(1)['image mean'].values.repeat(self.ntp)  # use to multiply values after photobleaching correction, such that the photobleaching correction factor == 1 for the first image in the sequence.
