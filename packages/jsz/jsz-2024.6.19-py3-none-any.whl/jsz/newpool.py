...
class Pool:
    ...
    def __init__(self, n: int = None, use_process: bool = False):
        """
        线程池和进程池

        n: 线程数。默认为CPU核心数+4, 可以自定义线程数。
        use_process: 默认为线程池, True 为 进程池
        """
    ...
    def wait(self, timeout: float = None, return_when: str = "ALL_COMPLETED"):
        ...
        timeout: 超时秒数
        return_when: 完成标准，共三种，分别为 ALL_COMPLETED、FIRST_COMPLETED、FIRST_EXCEPTION
        """
