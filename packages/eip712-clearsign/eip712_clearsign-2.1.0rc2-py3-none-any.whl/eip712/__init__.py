    ...
    def mapper(self, **mapper_data) -> Iterable[EIP712FieldMapper]:
        match self.format:
            case EIP712Format.AMOUNT:
                if self.coinRef is None or self.assetPath is None:
                    raise ValueError(
                        f"EIP712 amount value should have coin_ref: {self}"
                    )
                # Amount format generates two instructions:
...
class EIP712Mapper(BaseModel):
    ...
    def mappers(self, **mapper_data) -> Iterator[EIP712BaseMapper]:
        field_mappers = [
            mapper for field in self.fields for mapper in field.mapper(**mapper_data)
        ]
