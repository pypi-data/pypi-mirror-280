...
def logdelta_prior_lpdf(log_delta):
    s2p = 100.0
    return -np.log(np.sqrt(2 * np.pi * s2p)) - np.square(log_delta - 20.0) / (2 * s2p)
...
def brent_max_LL(UTy, UT1, S, n):
    ...
    o = optimize.minimize_scalar(
        LL_obj, bounds=[-10, 10], method="bounded", options={"maxiter": 32}
    )
    max_ll = -o.fun
...
def lbfgsb_max_LL(UTy, UT1, S, n, Yvar=None):
    ...
    s2_logdelta = 1.0 / (derivative(LL_obj, np.log(max_delta), n=2) ** 2)
...
def const_fits(exp_tab):
    ...
    for g in range(G):
        ...
        max_ll = -0.5 * (
            n * np.log(max_s2_e_hat) + sum1 / max_s2_e_hat + n * np.log(2 * np.pi)
        )
...
def run(X, exp_tab, kernel_space=None):
    ...
    mll_results["pval"] = 1 - stats.chi2.cdf(2 * mll_results["LLR"], df=1)
