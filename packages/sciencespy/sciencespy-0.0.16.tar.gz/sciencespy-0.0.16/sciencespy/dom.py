...
class Building(IDObject):
    ...
    def get_ground_floor_temperature(self):
        """
        :return: ground floor surface temperature in the building.
        """
        n_zones = len(self.zones)
        ground_floor_surface_temperature = 0
        for n in range(0, n_zones):
            if not self.zones[n].ground_floor is None:
                return self.zones[n].ground_floor.get_surface_temperature()
