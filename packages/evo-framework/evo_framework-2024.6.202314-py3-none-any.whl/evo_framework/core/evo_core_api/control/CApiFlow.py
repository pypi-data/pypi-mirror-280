...
class CApiFlow:
    ...
    async def onResponse(
        ...
        try:
            IuLog.doDebug(__name__, f"onResponse:{eRequestInfo}")
            ...
            if isComplete:
                ...
                IuLog.doInfo(
                    __name__,
                    f"response:{eRequestInfo.id} partial time: {eRequestInfo.elapsedTimeStr} ",
                )
    ...
    async def doEAction(self, dataERequest) -> EAction:
        try:
            ...
            try:
                ...
                async for eActionOutput in CApiFlow.getInstance().onActionStream(
                    eApi, eActionTask
                ):
                    isComplete = False
                    if (
                    ...
                    dataResponse = await self.onResponse(
                        eRequestInfo, dataAction, isComplete
                    )
                    IuLog.doInfo(
                        __name__,
                        f"onActionStream id:{eActionOutput.id} dataStream:{len(dataResponse)} {eRequestInfo.doGetElapsedPartial()}",
                    )
                    yield dataResponse
            except Exception as exception:
                IuLog.doException(__name__, exception)
                eAction.doSetError(f"{exception}")
                dataResponse = await CApiFlow.getInstance().onResponse(
