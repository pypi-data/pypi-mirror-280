...
def run_step5(i, job_control, dict_of_simulation,job_info):
    ...
    for indtra, tra in enumerate(transitions_names):
        print("Marcus rate for transition " , indtra ," is ", rates[indtra], ' Hz')
    ...
    for ind, name in enumerate(State_list):
        if ind > 1:
            ...
            stroutput = stroutput +  str(Correction[State_list[ind-1]]) + ","  
            stroutput = stroutput + str(Coupling_list[ind-1])  + "," 
            stroutput = stroutput + str(Er_all[ind-1]) + "," 
            stroutput = stroutput + str(dE_all[ind-1]) + "," 
            stroutput = stroutput + str(rates[ind-1]) + "\n" 
    stroutput = stroutput + "\n"
...
def returnMarcusRate(traj_dir, MD_traj_state, Coupling_list, correction, state_list, temperature,system_name, ensemble='NVE'):
    ...
    for ind, coupling in enumerate(coupling_list):
        ...
        Er_list . append(Er) 
        dE_list . append(dE)
        print(arrow_str, 'Transition', transition[ind] ,'Marcus Parameters from all-atom simulation' )
        print("DeltaE = ", dE , " eV" )
        print("<U> = ", U , " eV" )
        print("Er = ", Er , " eV" )
        print("Gamma = ", coupling_list[ind],' eV')
        print("sigma_U = ", sigma_U, ' eV')
        print("Ea = ", 0.5 * U * U / sigma_U / sigma_U * kb * T , ' eV')
