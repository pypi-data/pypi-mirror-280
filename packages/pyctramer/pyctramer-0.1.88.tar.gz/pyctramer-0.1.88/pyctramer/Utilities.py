...
def write_to_output(i, job_control, dict_of_simulation, strr) :
    ...
    print(dict_of_simulation['workflow'], "FGR", job_control['status'],len(job_control),job_control )
    print("dict_of_simulation['workflow']" )
    if job_control['status'][0] == 0 and (dict_of_simulation['workflow'] == "alltheway" or dict_of_simulation['workflow'] == "Marcus") :
        fo = open(outfile, "w+")
        fo.write(header)
        fo.write(strr)
        fo.close()
