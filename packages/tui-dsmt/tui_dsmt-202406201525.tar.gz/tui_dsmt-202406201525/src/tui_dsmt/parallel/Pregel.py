...
class Pregel(JupyterAnimation):
    def __init__(self, graph: nx.DiGraph, pregel_fun: Callable[[Any, List[Any], List[Any]], Tuple[Any, bool]],
                 pos: Dict = None, node_value_key: str = 'value', node_init: Any = None,
        ...
        while any(nx.get_node_attributes(graph, 'active').values()):
            ...
            for node in graph.nodes:
                ...
                node_value, node_active = pregel_fun(node_value, received_from, received_values)
    ...
    def node_style(self, node):
        ...
        style = {
            'backgroundColor': f'rgb({color_primary})' if active else 'gray',
            'color': 'whitesmoke'
        }

        if self.partition_hash_fun is not None:
            style['boxShadow'] = f'inset 0 0 0 0.25rem rgb({colors[self.partition_hash_fun(node)]})'
