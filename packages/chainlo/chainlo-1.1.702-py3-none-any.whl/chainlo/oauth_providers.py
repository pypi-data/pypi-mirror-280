...
class KeycloakOAuthProvider(OAuthProvider):
    ...
    # Avoid trailing slash in domain if supplied
    domain = f"https://{os.environ.get('OAUTH_KEYCLOAK_DOMAIN', '').rstrip('/')}"

    def __init__(self):
        self.client_id = os.environ.get("OAUTH_KEYCLOAK_CLIENT_ID")
        self.client_secret = os.environ.get("OAUTH_KEYCLOAK_CLIENT_SECRET")
        self.authorization_server_id = os.environ.get(
            "OAUTH_KEYCLOAK_AUTHORIZATION_SERVER_ID", ""
        )
        self.authorize_url = (
            f"{self.domain}/realms/{self.get_authorization_server_path()}/protocol/openid-connect/auth"
        )
        self.authorize_params = {
            "response_type": "code",
            "scope": "openid profile email",
    ...
    async def get_token(self, code: str, url: str):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.domain}/realms/{self.get_authorization_server_path()}/protocol/openid-connect/token",
                data=payload,
            )
            response.raise_for_status()
            json_data = response.json()
    ...
    async def get_user_info(self, token: str):
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.domain}/realms/{self.get_authorization_server_path()}/protocol/openid-connect/userinfo",
                headers={"Authorization": f"Bearer {token}"},
            )
            response.raise_for_status()
            ...
            user = User(
                identifier=keycloak_user.get("email"),
