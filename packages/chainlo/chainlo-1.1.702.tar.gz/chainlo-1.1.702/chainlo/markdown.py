...
# Default chainlo.md file created if none exists
...
- **Documentation:** Get started with our comprehensive [Chainlit Documentation](https://docs.chainlo.io) 📚
...
To modify the welcome screen, edit the `chainlo.md` file at the root of your project. If you do not want a welcome screen, just leave this file empty.
"""


def init_markdown(root: str):
    """Initialize the chainlo.md file if it doesn't exist."""
    chainlit_md_file = os.path.join(root, "chainlo.md")
...
def get_markdown_str(root: str, language: str):
    """Get the chainlo.md file as a string."""
    translated_chainlit_md_path = os.path.join(root, f"chainlit_{language}.md")
    default_chainlit_md_path = os.path.join(root, "chainlo.md")
    ...
    if os.path.exists(translated_chainlit_md_path):
        ...
        logger.warning(
            f"Translated markdown file for {language} not found. Defaulting to chainlo.md."
        )
