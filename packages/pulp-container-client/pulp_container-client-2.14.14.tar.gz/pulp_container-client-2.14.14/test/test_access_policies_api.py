# coding: utf-8

"""
    Pulp 3 API

    Fetch, Upload, Organize, and Distribute Software Packages  # noqa: E501

    The version of the OpenAPI document: v3
    Contact: pulp-list@redhat.com
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest

import pulpcore.client.pulp_container
from pulpcore.client.pulp_container.api.access_policies_api import AccessPoliciesApi  # noqa: E501
from pulpcore.client.pulp_container.rest import ApiException


class TestAccessPoliciesApi(unittest.TestCase):
    """AccessPoliciesApi unit test stubs"""

    def setUp(self):
        self.api = pulpcore.client.pulp_container.api.access_policies_api.AccessPoliciesApi()  # noqa: E501

    def tearDown(self):
        pass

    def test_list(self):
        """Test case for list

        List access policys  # noqa: E501
        """
        pass

    def test_partial_update(self):
        """Test case for partial_update

        Update an access policy  # noqa: E501
        """
        pass

    def test_read(self):
        """Test case for read

        Inspect an access policy  # noqa: E501
        """
        pass

    def test_reset(self):
        """Test case for reset

        """
        pass

    def test_update(self):
        """Test case for update

        Update an access policy  # noqa: E501
        """
        pass


if __name__ == '__main__':
    unittest.main()
