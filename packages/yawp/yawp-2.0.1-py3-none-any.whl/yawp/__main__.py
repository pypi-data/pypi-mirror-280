...
def log_append(message):
    'append a message to the log file of arg.text_file'
...
def error(message, jline=None):
    if message:
        error_message = (f'ERROR IN LINE {jline}: ' if jline is not None else 'ERROR: ') + message
...
def ask_ok(title, message):
    "show a window, exit by 'OK' button or alt-F4 or 'x' button"
    lines = message.split('\n')
    text_width = max(FSIZE, max(len(line) for line in lines))
    text_height = len(lines)
    layout = [[psg.Text(message, size=(text_width, text_height))],
              [psg.Button('OK', tooltip='go on', size=BSIZE)]]
...
class Arguments:
    ...
    def read_from_argv(arg, argv):
        ...
        # export arguments
        sarg('-X', '--export-pdf',      'b', 'export and browse PDF file', "=export and browse, 'n'=no, 'e'=export")
        ...
        parg('target_file', '?', 'destination text file for CLI Copy mode (-M c) and CLI Move mode (-M m)')
        # arguments → arg.*
        parser.parse_args(argv[1:], arg)
        arg.text_file = longpath(arg.text_file)
        arg.target_file = longpath(arg.target_file)
        ...
        if arg.usage_mode == 'g' and len(argv) != 1 + bool(arg.text_file):
            error(f"With -M g no argument is allowed, except possibly text_file")
        if arg.usage_mode in 'cm' and (not arg.text_file or not arg.target_file):
            error(f'With -M {arg.usage_mode} text_file and target_file are mandatory')
        if arg.usage_mode != 'g' and not arg.text_file:
            error(f'With -M {arg.usage_mode} text_file is mandatory')
        if arg.usage_mode not in 'cm' and arg.target_file:
            error(f'With -M {arg.usage_mode} target_file is not allowed')
        if arg.usage_mode == 'g':
            arg.verbose = False
...
def gui_main_window():
    ...
    layout = [
        ...
        [block('Pages'),     psg.Text('-p --page-headers',    size=TSIZE),
                                 ...
                                 psg.Radio('double',   'p', default=arg.page_headers=='d', tooltip='…and double if level-1 on even page'),],
...
def copy_button():
    ...
    arg.write_into_args_file_of(old_text_file)
    lock.seize(new_text_file)
    inform(cp(old_text_file, new_text_file))
    inform(cp(args_file_of(old_text_file), args_file_of(new_text_file)))
    inform(rm(log_file_of(new_text_file)))
...
def move_button():
    ...
    arg.write_into_args_file_of(old_text_file)
    lock.seize(new_text_file)
    inform(mv(old_text_file, new_text_file)) 
    inform(mv(args_file_of(old_text_file), args_file_of(new_text_file)))
    inform(mv(log_file_of(old_text_file), log_file_of(new_text_file)))
    ...
    for old_back_file in back_files_of(old_text_file):
        ...
        inform(mv(old_back_file, new_back_file))
    lock.release(old_text_file)
    arg.text_file = new_text_file
