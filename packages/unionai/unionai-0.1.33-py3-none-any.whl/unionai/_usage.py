...
from flytekit.loggers import upgrade_to_rich_logging
...
UNIONAI_USAGE_STATS_ENABLED_ENV = "UNIONAI_USAGE_STATS_ENABLED"
UNIONAI_DEBUG_USAGE_STATS_ENV = "UNIONAI_DEBUG_USAGE_STATS"
...
# Only show exceptions caused by unionai
PACKAGE_PATHS_TO_TRACK = [*unionai.__path__]
...
def _filter_exceptions(event, hint):
    """Filter out exceptions that are not from the unionai SDK."""
    try:
...
def _configure_tracking_cli(main: click.Group) -> click.Group:
    """Configures the the unionai to catch exceptions."""
...
def _init_tracking():
    ...
    unionai_git_path = Path(unionai.__path__[0]).parent / ".git"
    in_dev_env = unionai_git_path.exists()
    ...
    enable_tracking_for_dev = getenv(UNIONAI_DEBUG_USAGE_STATS_ENV, "0") != "0" and in_dev_env

    # If UNIONAI_USAGE_STATS_ENABLED_ENV != "0", then enable user stats collection. (Default)
    # If UNIONAI_USAGE_STATS_ENABLED_ENV == "0", then disable user stats collection.
    enable_usage_stats = getenv(UNIONAI_USAGE_STATS_ENABLED_ENV, "1") != "0" and enable_tracking_for_dev
    ...
    release = "dev" if enable_tracking_for_dev else version("unionai")
    sentry_sdk.init(
        dsn=SENTRY_DSN,
