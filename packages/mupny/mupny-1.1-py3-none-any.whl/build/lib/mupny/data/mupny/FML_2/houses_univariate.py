...
linear = LinearRegression(n_features=x.shape[1], n_steps=100, learning_rate=0.01)
linear.theta = np.array([1, -0.9])

lineX = np.linspace(x[:, 1].min(), x[:, 1].max(), 8)
...
cost_history, cost_history_plot, theta_history = linear.fit(x, y, 9)
