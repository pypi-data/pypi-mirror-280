...
@overload
async def notify(*, token: str | None = None, attachment: bytes | None = None, **params: Unpack[HighPriorityParams]) -> Either[Err, PriorityOk]:
  ...
  """
@overload
async def notify(*, token: str | None = None, attachment: bytes | None = None, **params: Unpack[LowPriorityParams]) -> Either[Err, Ok]:
...
async def notify(*, token: str | None = None, attachment: bytes | None = None, **params): # type: ignore
  token = token or os.getenv('PUSHOVER_TOKEN')
  if not token:
    raise ValueError('Provide a token or set the PUSHOVER_TOKEN env. variable')
  ...
  try:
    async with httpx.AsyncClient() as client:
      ...
      if response.status_code == 200:
        model = PriorityOk if params['priority'] == 2 else Ok
