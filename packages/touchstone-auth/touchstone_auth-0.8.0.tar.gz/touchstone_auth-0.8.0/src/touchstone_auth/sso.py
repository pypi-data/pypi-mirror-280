...
Touchstone-protected MIT SSO site. This enables programmatic access
...
class TouchstoneSession:
    ...
    def __init__(self,
        ...
        *,
        ...
        if ((not initial_response.url.startswith(r'https://idp.mit.edu/idp'))
            and (not initial_response.url.startswith(r'https://okta.mit.edu/app'))):
        ...
        # Attempt new Touchstone flow
        match = re.match(
            r'https:\/\/okta\.mit\.edu\/app',
            initial_response.request.url
        )
        if match is not None:
            self.vlog('Performing Okta touchstone flow...')
            touchstone_response = self.perform_okta(initial_response.text)
            self.vlog("Done!")
            return
    ...
    def perform_okta(self, touchstone_proxy_response):
        """
        Performs Touchstone login via thew new Okta. This handles redirects to/from Duo

        Arguments
        ---------
        touchstone_proxy_response: a string containing the redirected SAML touchstone proxy.
        """
        if type(self._auth) != UsernamePassAuth:
        ...
        user_remediation_data = {
            "identifier": self._auth.username + '@mit.edu',
            "rememberMe": True,
            "credentials": {"passcode": self._auth.password}
        }

        for _ in range(5):
            if r.status_code != 200:
                raise TouchstoneError(f"Failed Okta remediation request to {r.request.url}")
            
            url_matches_duo = re.search(r"https:\/\/(?P<domain>[^/]+)(?P<path>[^?]+)\?sid=(?P<sid>[^&]+)\&tx=(?P<tx>.*)", r.url)
            if url_matches_duo is not None:
                break
            remediation_data = r.json()

            #print(remediation_data['remediation']['value'])

            remediation = select_remediation(remediation_data['remediation']['value'], user_remediation_data)

            self.vlog(f"Okta ({remediation.name}): {remediation.http_method}ing to {remediation.url.split('?')[0]}")


            if remediation.http_method == "POST":
                r = self._session.post(remediation.url, data=json.dumps(remediation.data), headers={"Content-Type": "application/json"})
    ...
    def perform_old_touchstone(self, conversation):
        ...
        return self.perform_duo(r)
    
    def perform_duo(self, response):
        """
        Performs Duo authentication, and returns the request that Duo responds with

        Arguments
        ---------
        response: a Response object for the initial Duo page 
        """

        duo_html = BeautifulSoup(response.text, features='html.parser')
        ...
        self.vlog('Duo: Decoded Touchstone transaction/akey/xsrf from redirect')
        ...
        if r.url.startswith('https://idp.mit.edu/idp/profile/SAML2/Redirect/SSO') or r.url.startswith('https://okta.mit.edu'):
            # We're done!
            self.vlog('Duo: 2FA not required: Duo cookie cached. Returning to Touchstone')
            return r
        ...
        self.vlog('Duo: Second factor auth required: requested Duo auth page')
        ...
        self.vlog(f'Duo: Requested second factor authentication ({factor})')
        ...
        if self._twofactor_type == TwofactorType.DUO_PUSH:
            self.vlog('Duo: Successfully pushed Duo push request. Blocking until response...')
            r = self._session.post(f"https://{duo_domain}/frame/v4/status",
                data=bytes(f"sid={duo_sid}&txid={prompt_response['response']['txid']}", 'utf-8'),
                headers=extra_prompt_headers)
            ...
            self.vlog('Duo: Second factor auth successful!')
        elif self._twofactor_type == TwofactorType.PHONE_CALL:
            self.vlog('Duo: Successfully pushed phone call request...')
            r = self._session.post(f"https://{duo_domain}/frame/v4/status",
                data=bytes(f"sid={duo_sid}&txid={prompt_response['response']['txid']}", 'utf-8'),
                headers=extra_prompt_headers)
            ...
            self.vlog("Duo: Two-factor call answered. Waiting for user input...")
