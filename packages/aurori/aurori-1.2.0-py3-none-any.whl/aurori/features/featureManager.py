...
class FeatureManager(object):
    ...
    def init_app(self, app, config, minimal = False, api_prefix="api/v1"):
        ...
        log_manager.info("")
        log_manager.info(
            f'Discover features in path : {self.feature_source_path}')
    ...
    def discover_permissions(self):
        ...
        for w in self.features:
            ...
            try:
                w.discover_permissions(self.feature_source_path)
            except Exception as e:
                traceback.print_exc(file=sys.stdout)
                log_manager.error(
                    f'  Feature "{w.uri}" unable to discover permissions  ({str(type(e).__name__)}:{e})'
                )
    ...
    def discover_pages(self):
        ...
        for w in self.features:
            log_manager.info(f'  For feature: "{w.name}"')
            feature_config_section_name = pathlib.Path(
    ...
    def discover_jobs(self):
        log_manager.info("")
        log_manager.info("Discover jobs:")
        ...
        for w in self.features:
            ...
            if feature_config_section_name in self.config and "disable_jobs" in self.config[
                    feature_config_section_name] and self.config[
                        feature_config_section_name]["disable_jobs"]:
                log_manager.info(f'    Jobs for "{w.name}" are disabled by config')
            else:
                # try to register jobs
                try:
                    w.discover_jobs(self.feature_source_path)
                except Exception as e:
                    traceback.print_exc(file=sys.stdout)
                    log_manager.error(
    ...
    def discover_apis(self):
        log_manager.info("")
        log_manager.info("Discover apis:")
            ...
            for child_pkg in child_pkgs:
                try:
                    ...
                    for _, modelpackagename, ispkg in pkgutil.iter_modules(
                        ...
                        feature_module = __import__(modelpackagename,
                                                      fromlist=['blah'])
                        clsmembers = inspect.getmembers(
                            feature_module, inspect.isclass)
                    ...
                    log_manager.info(f'No model found for {modelmodule}')

        if self.user_class is None:
            log_manager.warning(f'No User model derived from aurori.users.basemodels.UserBase found.')
            log_manager.warning(f'A aurori app needs a user model to work properly.')
            log_manager.warning(f'Please define a derived model named "User" in your features, if needed.')
            log_manager.warning(f'The default model will be used, please reference: aurori.users.defaultmodels.User ')
    ...
    def discover_features(self, source):
        ...
        for pkg_path in all_current_paths:
            ...
            for child_pkg in child_pkgs:
                ...
                featureInstance.path = os.path.dirname(
                ...
                if (feature_config_section_name
                        in self.config) and (
