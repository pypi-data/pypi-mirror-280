...
def get_value(
    ...
    type_: type[T],
    *,
    set_none_on_exception: Literal[True],
) -> T | None:
    pass
...
def get_value(
    ...
    type_: type[T],
    factory: Callable[[], T] | None,
    set_none_on_exception: Literal[False],
) -> T:
    pass
...
def is_all_type(itr: Iterable[Any], type_: type[T], use_isinstance: bool = False) -> TypeGuard[Iterable[T]]:
    """シーケンスの値すべてに対して型判定を行います。

    Args:
        itr (Iterable[Any]): 対象シーケンス。
        type_ (type[T]): 型。
        use_isinstance (bool, optional): isinstanceを使用して判定するか。 Defaults to False.

    Returns:
        TypeGuard[Iterable[T]]: 型保障。
    """
    return all(map(lambda x: is_type(x, type_, use_isinstance), itr))
