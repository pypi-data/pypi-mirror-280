...
class DetectionResults(InferenceResults):
    ...
    def _deduce_color(self, id, label, current_color_set):
        ...
        return self._overlay_color[(id + 1) % len(self._overlay_color)]
    ...
    def generate_overlay_color(model_params, label_dict) -> list:
        ...
        if not label_dict:
            ...
            return colors[: (model_params.OutputNumClasses + 1)]
            ...
            colors = colors[: min(len(label_dict) + 1, DetectionResults.max_colors)]
            return colors
        ...
        for res in self._inference_results:
            ...
            if mask is not None:
                ...
                render_mask = (mask > 0).astype(numpy.uint8) * (
                    (category_id + 1) % DetectionResults.max_colors
                )
                merge_condition = self.mask_result != 0
                self.mask_result = numpy.where(
                    merge_condition, self.mask_result, render_mask
                )
