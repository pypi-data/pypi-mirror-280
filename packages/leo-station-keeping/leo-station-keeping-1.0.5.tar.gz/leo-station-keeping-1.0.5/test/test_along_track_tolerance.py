# coding: utf-8

"""
    LEO Station Keeping API

    This mission allows to estimate the cost of station keeping maneuvers needed  to counter the effects of the atmospheric drag on the semi-major axis of a  spacecraft in low Earth orbit (up to an altitude of 1000 km).   <b>CHANGELOG</b> <ul>   <li><b>1.0.0</b> - 2024-06-07 - First version of the API</li> </ul> 

    The version of the OpenAPI document: 1.0.5
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from leo_station_keeping.models.along_track_tolerance import AlongTrackTolerance

class TestAlongTrackTolerance(unittest.TestCase):
    """AlongTrackTolerance unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AlongTrackTolerance:
        """Test AlongTrackTolerance
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AlongTrackTolerance`
        """
        model = AlongTrackTolerance()
        if include_optional:
            return AlongTrackTolerance(
                along_track_tolerance = 1.337
            )
        else:
            return AlongTrackTolerance(
        )
        """

    def testAlongTrackTolerance(self):
        """Test AlongTrackTolerance"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
