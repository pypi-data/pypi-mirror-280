...
from my_module.state_analysis import groupby_state, clean_states, merge_datasets, calculate_relative_values
...
def test_groupby_state_and_year():
    ...
    # Asegúrate de que la columna 'year' se creó correctamente
    assert 'year' in df_final.columns, "La columna 'year' no existe en el DataFrame"

    grouped_df = groupby_state_and_year(df_final)
    assert not grouped_df.empty, "El DataFrame agrupado está vacío"
