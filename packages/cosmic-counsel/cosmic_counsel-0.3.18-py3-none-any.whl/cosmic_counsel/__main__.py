...
#     resource_path = pkg_resources.files('cosmic_counsel').joinpath(resource_name)
#     if not os.path.exists(target_dir):
#         os.makedirs(target_dir)
#     with resource_path.open('rb') as src, open(os.path.join(target_dir, resource_path.name), 'wb') as dst:
...
#         resources = pkg_resources.files('cosmic_counsel').joinpath(resource_dir)
#         for resource in resources.iterdir():
#             if resource.is_dir():
#                 for subresource in resource.iterdir():
#                     copy_resource(subresource.relative_to('cosmic_counsel'), os.path.join(target_dir, resource.name))
#             else:
#                 copy_resource(resource.relative_to('cosmic_counsel'), target_dir)
...
def main():
    if args.directory:
        ...
        context_index_path = f'{parent_dir_path}/data/context_index.pkl'
        if not context_index_path.exists(): 
            with open(context_index_path, 'w') as file: 
                file.write("Hello, Geeks!") 
        with open(context_index_path, 'wb') as f:
    ...
    # Ensure context_index.pkl exists
    context_index_path = f'{parent_dir_path}/data/context_index.pkl'
    if not os.path.exists(context_index_path):
        os.makedirs(os.path.dirname(context_index_path), exist_ok=True)
        with open(context_index_path, 'wb') as f:
            pickle.dump({}, f)  # Create an empty dictionary
    ...
    if args.question:
        ...
        try:
            ...
            print("CUDA out of memory. Consider reducing the model size or batch size.")
