...
def create_property_accessor(property_name, doc = None):
    ...
    if base_property_name.endswith('_'):
        ...
        def getter(self):
            return self.get_mutable(base_property_name)
    else:
        def getter(self):
...
def _PropertyContainer_create_property(self, name, dtype=None, components=None, data=None):
    ...
    predefined names for the container type:
      ...
      * :py:class:`~ovito.data.Bonds`: :ref:`List of standard bond properties <bond-types-list>`
    ...
    :param str name: Name of the property to create.
    ...
    :param dtype: Data type of the user-defined property. Must be ``int``, ``float``, ``numpy.int8``, ``numpy.int32``, ``numpy.int64``, ``numpy.float32``, or ``numpy.float64``.
    :param int|list[str]|None components: Number of vector components of the user-defined property (1 if not specified) or a list of strings specifying the property's :py:attr:`~ovito.data.Property.component_names`.
    ...
    if existing_prop is None:
        ...
        if data is not None and isinstance(data, collections.Sized) and (not isinstance(data, numpy.ndarray) or data.ndim != 0):
