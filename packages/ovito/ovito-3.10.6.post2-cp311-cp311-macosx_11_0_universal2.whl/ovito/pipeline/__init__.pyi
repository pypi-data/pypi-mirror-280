...
In case of a :py:class:`FileSource`, returns the number of trajectory frames found in the input file or sequence of input files. In case of a :py:class:`StaticSource`, returns 1. In case of a :py:class:`PythonSource`, returns the result of the :py:meth:`PipelineSourceInterface.compute_trajectory_length` method. In case of a :py:class:`ModificationNode`, returns the number of frames generated by the :py:attr:`~ModificationNode.input` node, which may be altered by the associated :py:attr:`~ModificationNode.modifier`."""
        ...
...
```"""
    ...
    'Reference to the :py:class:`Modifier` that gets applied to the data as it flows through the pipeline. Several `ModificationNodes` may share the same modifier object, which allows using (applying) the same modifier in multiple pipelines. All uses of the modifier share the same parametrization in this case.'
...
For more advanced applications, a *class-based* programming interface is also avilable,
...
To export the generated data of the pipeline to an output file, simply call the :py:func:`ovito.io.export_file` function with the pipeline."""
    ...
    'This property field gets or sets the :py:class:`PipelineNode` that is responsible for producing (or loading) the input data for this pipeline. If this pipeline was created by a call to the :py:func:`ovito.io.import_file` function, the source is typically a :py:class:`FileSource` instance. You can replace the source of a pipeline if needed. Available source types are: :py:class:`FileSource`, :py:class:`StaticSource`, and :py:class:`PythonSource`. It is possible for several pipelines to share the same source node. \n\nDefault: ``None``'
    head: Optional[PipelineNode] = None
    "The final :py:class:`PipelineNode` of the pipeline, i.e., the last processing step, which produces the output data of the pipeline returned by the :py:meth:`compute` method. \n\nIf the pipeline contains no `ModificationNodes` (its :py:attr:`modifiers` list is empty), then the pipelines's :py:attr:`.head` is identical with its :py:attr:`.source`. \n\nDefault: ``None``"
