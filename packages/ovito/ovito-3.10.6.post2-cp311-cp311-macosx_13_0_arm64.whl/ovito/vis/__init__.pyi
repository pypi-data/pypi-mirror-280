...
@dataclass(kw_only=True)
class AnariRenderer:
    """Scientific visualization renderer based on the NVIDIA OptiX™ Ray Tracing Engine, which runs on CUDA-capable devices (`VisRTX <https://github.com/NVIDIA/VisRTX>`__). The renderer offers hardware accelerated ray-tracing and can generate high-fidelity scene renderings including global illumination effects and shadows. 

Please see :ref:`manual:rendering.visrtx_renderer` in the OVITO user manual for further information. The class name :py:class:`AnariRenderer` derives from the fact that OVITO uses the `Khronos ANARI <https://www.khronos.org/anari>`__ interface to communicate with NVIDIA's VisRTX backend. Future versions of OVITO may support additional ANARI-compliant rendering backends from other vendors. 
...
parameter panel in the UI."""
    ...
    transparency: float = 0.0
    'The level of semi-transparency for rendering the arrows. The valid parameter range is 0.0 -- 1.0 (fully opaque to fully transparent).This parameter is ignored when the ``Vector Transparency`` property is set for the particles to specify explicit per-particle vectortransparency values. \n\nDefault: ``0.0``'
    width: float = 0.5
