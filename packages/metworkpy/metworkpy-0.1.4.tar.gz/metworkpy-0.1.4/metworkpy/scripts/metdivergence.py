...
def parse_args(arg_list: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("-d", "--treatment-distribution", dest="treatment_distribution_file",
                        required=True,
    ...
    parser.add_argument("-f", "--input-format", dest="input_format", required=False, default="csv",
                        ...
                        type=str)
    parser.add_argument("-n", "--neighbors", dest="neighbors", required=False, default=5,
                        help="Number of neighbors to use for computing divergence. Should be an integer greater than "
                             "1, default is 5.", type=int)
    parser.add_argument("-t", "--divergence-type", dest="divergence_type", required=False, default="js",
                        help="Type of divergence to compute, either KL/Kullback-Leibler or JS/Jensen-Shannon",
                        type=str)
    parser.add_argument("--metric", dest="metric", required=False, default="euclidean",
                        help="Metric to use for computing distance during the divergence calculations, "
                             "can be Euclidean, Manhattan, or Chebyshev. Defaults to euclidean", type=str)
    parser.add_argument("-M", "--model", dest="model_file", required=False, default=None,
...
def main_run(arg_list: list[str] | None = None):
    ...
    rxn_list = [rxn for rxn in treatment_dist.columns if rxn in wt_dist.columns]
    if (len(rxn_list) != len(wt_dist.columns)) or (len(rxn_list) != len(treatment_dist.columns)):
        warnings.warn("Some reactions are only present in one of the input distributions, "
                      "using the intersection of the sets of reactions.")
    div_res = pd.DataFrame(0., index=rxn_list, columns=[f"{div_type}_divergence"])
    if args.verbose:
    ...
    if div_type == "js":
        ...
        print(f"Couldn't parse Divergence type, must be kl, Kullback-Leibler, js, or Jensen-Shannon, but "
              f"received {div_type}", file=sys.stderr)
        sys.exit()
    for rxn in tqdm(rxn_list, disable=(not args.verbose)):
        div_res.loc[rxn, f"{div_type}_divergence"] = (
            div_func(
                p=wt_dist[rxn],
    ...
    if args.model_file:
        ...
        model = metworkpy.read_model(args.model_file, file_type=args.model_format)
        rxn_names = []
        genes = []
        subsystems = []
        rxn_str = []
        ...
        for rxn in div_res.index:
            ...
            rxn_str.append(reaction.build_reaction_string(use_metabolite_names=True))
...
def _read_distributions(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    input_format = _parse_format(args.input_format)
    if input_format == "csv":
        treatment_dist = pd.read_csv(args.treatment_distribution_file, index_col=None, header=0)
        wildtype_dist = pd.read_csv(args.wildtype_distribution_file, index_col=None, header=0)
        ...
        if not importlib.util.find_spec("openpyxl"):
            print("Input format is excel, but openpyxl is not installed so can't read the excel file. Exiting",
                  file=sys.stderr)
            sys.exit()
        treatment_dist = pd.read_excel(args.treatment_distribution_file, sheet_name=args.sheet_name,
                                       index_col=None, header=0, )
        wildtype_dist = pd.read_excel(args.wildtype_distribution_file, sheet_name=args.sheet_name,
                                      index_col=None, header=0, )
        ...
        raise ValueError(f"Couldn't parse input file type, should be csv, parquet, feather, excel, or json, "
...
def _parse_div_type(metric_str: str) -> str:
    return _parse_str_args_dict(metric_str,
                                {
                                    "kl": ["kullback-leibler", "kullback leibler", "kullback_leibler", "kl"],
                                    "js": ["jensen-shannon", "jensen shannon", "jensen_shannon", "js"]
                                })


def _parse_format(format_str: str) -> str:
    return _parse_str_args_dict(format_str, {
        "csv": ["csv", "comma-seperated-values", "comma_seperated_values", "comma seperated values", ],
        "parquet": ["parquet"],
        "feather": ["feather"],
        "excel": ["excel", "xlsx"],
        "json": ["json"],
