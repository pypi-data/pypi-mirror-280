...
def main_run(arg_list: list[str] | None = None) -> None:
    ...
    model = metworkpy.read_model(model_path=args.model_file, file_type=args.model_format)
    ...
    if args.batches and not (out_format == "csv" or out_format == "parquet"):
        print(f"If batch sampling desired, output format must be csv or parquet, but "
              f"{out_format} was selected instead", file=sys.stderr)
        sys.exit()
    if args.batches and out_format == "parquet":
        if importlib.util.find_spec("fastparquet") is None:
            print("Batch sampling with parquet output requires fastparquet, but it is not installed/n", file=sys.stderr)
            sys.exit()
    if verbose:
        print("Creating Sampler")
    if method == "achr":
        sampler = cobra.sampling.achr.ACHRSampler(model=model, thinning=args.thinning,
                                                  seed=args.seed)
    elif method == "optgp":
        sampler = cobra.sampling.optgp.OptGPSampler(model=model, thinning=args.thinning,
                                                    processes=args.processes, seed=args.seed)
    else:
        raise ValueError(f"Method must be achr or optgp, but {method} was provided")
    if args.batches:
        if verbose:
            print("Starting Batch Sampling")
        _batch_sample(sampler=sampler,
                      out_file=out_file,
        ...
        if args.validate:
            valid = sampler.validate(samples) # Samples is a DataFrame, not a matrix
...
def _batch_sample(sampler: cobra.sampling.HRSampler,
    ...
    for sample in sampler.batch(samples_per_batch, batches):
        ...
        if out_format == "csv":
            ...
            if not os.path.exists(out_file):
                ...
                sample.to_parquet(out_file, index=False, engine="fastparquet", append=True)
        else:
...
def _parse_method(method_str: str) -> str:
    ...
    :rtype: str
    """
    return _parse_str_args_dict(method_str, {
        "achr": ["achr", "automatic-centering-hit-and-run", "automatic_centering_hit_and_run",
