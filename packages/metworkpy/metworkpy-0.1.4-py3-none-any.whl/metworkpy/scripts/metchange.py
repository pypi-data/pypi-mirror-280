...
def parse_args(arg_list: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("-o", "--output", dest="output_file", default="metchange_results.csv",
    ...
    parser.add_argument("-q", "--quantile-cutoff", dest="quantile_cutoff", default=0.15,
                        type=float, help="Quantile cutoff for converting gene expression values into gene weights. "
                                         ...
                                         "a weight of 0.2.",
                        required=False)
    parser.add_argument("--objective-tolerance", dest="objective_tolerance", default=5e-2,
                        type=float, help="The tolerance for the objective value. The objective value for "
    ...
    parser.add_argument("-v", "--verbose", dest="verbose", action="store_true",
                        help="Specify that a verbose output is desired", required=False)
    parser.add_argument("--extra-info", dest="extra_info", action="store_true",
...
def run(arg_list: list[str] | None = None) -> None:
    ...
    if (not args.metabolites_list) and (not args.metabolites_file):
        ...
        with open(args.metabolites_file, "r") as f:
            metabolites = [i.strip() for i in re.split(METABOLITE_SEPERATOR, f.read()) if i.strip()]
    else:
        raise ValueError("Couldn't understand --metabolites, or --metabolites-file. "
    ...
    for s in wt_samples:
        ...
        wt_rxn_weights.append(metworkpy.gpr.gene_to_rxn_weights(
            model=in_model,
    ...
    for sample_group in sample_groups:
        ...
        sample_rxn_weights_list.append(metworkpy.gene_to_rxn_weights(
            model=in_model,
    ...
    wt_metchange = pd.DataFrame(0., index=range(len(wt_rxn_weights)), columns=metabolites)
    ...
    metchange_res = pd.DataFrame(0., index=sample_names, columns=metabolites)
