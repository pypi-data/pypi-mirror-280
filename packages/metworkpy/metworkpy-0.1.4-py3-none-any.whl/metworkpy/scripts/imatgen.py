...
from metworkpy.scripts._script_utils import (_parse_samples, _parse_quantile, _parse_aggregation_method,
                                             _parse_sample_groups_and_names, _parse_format_to_extension)
...
def run_single(args: argparse.Namespace):
    ...
    gene_weights = metworkpy.utils.expr_to_imat_gene_weights(expression=gene_expression,
                                                             ...
                                                             aggregator=_parse_aggregation_method(
...
def run_multi(args: argparse.Namespace):
    if args.fold_change and not args.wildtype:
        print("Wildtype argument must be provided to use the fold change method for gene weights. Exiting.")
        sys.exit()
    if args.wildtype:
    ...
    if args.sample_groups:
        sample_groups, sample_names = _parse_sample_groups_and_names(args.sample_groups, args.sample_group_names)
    else:
        sample_groups = [[idx] for idx, _ in enumerate(gene_expression.index) if idx not in wildtype]
    ...
    if not args.fold_change:
        ...
        for group, name in zip(sample_groups, sample_names):
            log2fc = np.log2(gene_expression.iloc[group, :].apply(aggregator, axis=0).div(wt_expr) + 1e-10)
            g_weights = log2fc.apply(lambda x: -1 if x <= -fc else (1 if x >= fc else 0))
            rxn_weights[name] = metworkpy.gpr.gene_to_rxn_weights(
                model=in_model,
                gene_weights=g_weights,
                fn_dict={"AND": min, "OR": max},
    ...
    for name, r_weights in rxn_weights.items():
        ...
        metworkpy.write_model(model=out_model, model_path=output_path, file_type=args.output_format)
...
def parse_args(arg_list: list[str] | None) -> argparse.Namespace:
    ...
    # Create subparsers
    subparsers = toplevel_parser.add_subparsers(required=True, title="Modes", description="Choose a mode for imatgen",
                                                help="Choose between single (used for generating a single IMAT "
                                                     ...
                                                     "expression).")
    # Single parser
    single_parser = subparsers.add_parser("single", help="Generate a single IMAT model from gene expression data "
    ...
    # Multi parser
    multi_parser = subparsers.add_parser("multi", help="Generate multiple IMAT models, or use fold-change "
    ...
    # region Multi Args
    multi_parser.add_argument("-o", "--output-dir", dest="output_dir",
                              required=False, default="imatgen_results",
    ...
    multi_parser.add_argument("-s", "--sample-groups", type=str,
    ...
    multi_parser.add_argument("--fold-change", type=float,
                              ...
                              required=False)
    multi_parser.add_argument("-w", "--wildtype", type=str, dest="wildtype", default=None,
                              ...
                              required=False)
    multi_parser.add_argument("--sample-names", type=str, dest="sample_groups_names", default=None,
                              help="Names to give the sample groups. If provided, should include one name for "
                                   ...
                                   "Names will be included in the output model file name.", required=False)
...
def _add_common_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("-T", "--objective-tolerance",
                        ...
                        help="The tolerance for the objective value, "
                             ...
                             "the unconstrained objective value. Defaults to 0.05.", type=float)
