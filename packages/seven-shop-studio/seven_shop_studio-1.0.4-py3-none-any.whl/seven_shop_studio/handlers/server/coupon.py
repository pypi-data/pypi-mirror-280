...
:LastEditTime: 2024-06-21 16:32:20
...
class CouponRecordListHandler(SevenBaseHandler):
    ...
    def get_async(self, *args, **kwargs):
        ...
        record_list,record_count = CouponRecordModel().get_dict_page_list(field="id,user_code,coupon_info,use_price,add_time,status,use_time,end_time", page_index=page_index,page_size=page_size,order_by="id desc",where=condition,params=param_list)
        if record_list:
...
class CouponUserListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        record_conn = CouponRecordModel()
        record_list,record_count = record_conn.get_dict_page_list(field="id,coupon_info,add_time", page_index=page_index,page_size=page_size,order_by="id desc",where="coupon_source = 2 and user_code = %s and shop_id = %s",params=[user_code,shop_id])
        if record_list:
            for item in record_list:
                coupon_info = json.loads(item["coupon_info"])
                item["coupon_name"] = coupon_info["coupon_name"]
                item["coupon_type"] = coupon_info["coupon_type"]
        # 获取当前用户优惠券数量
        user_coupon_count = record_conn.get_total(where="user_code = %s and shop_id = %s and status = 0 and end_time > %s",params=[user_code,shop_id,TimeHelper.get_now_timestamp()])
        # 获取可发放优惠券列表
        coupon_list = CouponModel().get_dict_list(where="shop_id = %s and status = 1 and end_time > %s and coupon_inventory > record_number",params=[shop_id,TimeHelper.get_now_timestamp()],field="id,coupon_name,coupon_type,use_price,coupon_price,coupon_discount")
        ret_data = {
            "model_list": record_list,
            "count": record_count,
    ...
    def post_async(self):
        ...
        # 获取优惠卷限制商品
        goods_list = CouponGoodsModel().get_dict_list(where="coupon_id = %s", params=coupon_id)
