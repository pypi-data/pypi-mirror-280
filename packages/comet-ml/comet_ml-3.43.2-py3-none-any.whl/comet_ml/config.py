...
def get_global_experiment() -> "BaseExperiment":
    global experiment
    return experiment
...
def get_running_experiment() -> Optional["BaseExperiment"]:
    ...
    Returns: None or "BaseExperiment"
...
def init(
    ...
    **settings: Any
    ...
    Args:
        directory (`str`, *optional*): T location to save the
            .comet.config file. Typical values are "~/" (home)
            and "./" (current directory). Default is "~/" or
            COMET_CONFIG, if set.
        force (`bool`, *optional*, defaults to False): If True, forces the user to
            enter required information, such as the api key.
        anonymous (`bool`, defaults to False): If True, uses cComet temporarily without an api key.
