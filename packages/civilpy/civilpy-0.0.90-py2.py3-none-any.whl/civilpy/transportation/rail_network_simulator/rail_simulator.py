...
class World:
    ...
    def show_network(self, width=1, left_handed=1, figsize=(6, 6), network_font_size=10, node_size=6):
        ...
        signal_attributes_eb = {
            ...
            'NS_Yard': (self.LINKS[14].color, (1, 1), 'yard'),
            'main_0_org': (self.LINKS[18].color, (1, 3), self.LINKS[24].color),
            'main_1_org': (self.LINKS[25].color, (1, 5), self.LINKS[32].color),
            'main_2_org': (self.LINKS[33].color, (1, 7), self.LINKS[38].color),
            'main_3_org': (self.LINKS[39].color, (1, 8.75), self.LINKS[42].color),
            'yard_2_org': (self.LINKS[12].color, (1, 10.75), 'yard'),
            'yard_1_org': (self.LINKS[10].color, (1, 12.75), 'yard')
        }

        signal_attributes_wb = {
            # Destinations
            'setoff_track': (self.LINKS[16].color, (27, 2.75), 'yard'),
            'main0_dest': (self.LINKS[24].color, (27, 4.75), self.LINKS[18].color),
            'main1_dest': (self.LINKS[32].color, (27, 6.75), self.LINKS[25].color),
            'main2_dest': (self.LINKS[38].color, (27, 8.75), self.LINKS[33].color),
            'main3_dest': (self.LINKS[42].color, (27, 10.75), self.LINKS[39].color)
        }

        for signal in signal_attributes_eb:
            # Mainline Green/Green Case
            if signal_attributes_eb[signal][2] != 'yard' and signal_attributes_eb[signal][0] == 'green' and \
                    signal_attributes_eb[signal][2] == 'green':
                signal_icon = np.flipud(plt.imread(
                    f'./railroad icons/{signal_attributes_eb[signal][0]}_circle_{signal_attributes_eb[signal][2]}_square.png'))

            # Mainline Green/Red Case
            elif signal_attributes_eb[signal][2] != 'yard' and signal_attributes_eb[signal][0] == 'green' and \
                    signal_attributes_eb[signal][2] == 'red':
                signal_icon = np.flipud(plt.imread(f'./railroad icons/red_circle_yellow_square.png'))

            # Mainline Red/Green Case
            elif signal_attributes_eb[signal][2] != 'yard' and signal_attributes_eb[signal][0] == 'red' and \
                    signal_attributes_eb[signal][2] == 'green':
            ...
            ab = AnnotationBbox(im, signal_attributes_eb[signal][1], boxcoords="offset points",
                                bboxprops=dict(visible=False))
            signal_symbols.append(ab)

        for signal in signal_attributes_wb:
            # Mainline Green/Green Case
            if signal_attributes_wb[signal][2] != 'yard' and signal_attributes_wb[signal][0] == 'green' and \
                    signal_attributes_wb[signal][2] == 'green':
                signal_icon = plt.imread(
                    f'./railroad icons/{signal_attributes_wb[signal][0]}_circle_{signal_attributes_wb[signal][2]}_square.png')
            ...
            # Mainline Red/Green Case
            elif signal_attributes_wb[signal][2] != 'yard' and signal_attributes_wb[signal][0] == 'red' and signal_attributes_wb[signal][2] == 'green':
            ...
            im = OffsetImage(signal_icon, zoom=.5)
            ab = AnnotationBbox(im, signal_attributes_wb[signal][1], boxcoords="offset points",
                                bboxprops=dict(visible=False))
            signal_symbols.append(ab)
        ...
        for n in self.NODES:
            ...
            if network_font_size > 0:
                plt.text(n.x, n.y, n.name, c="white", horizontalalignment="right", verticalalignment="center",
                         zorder=20,
