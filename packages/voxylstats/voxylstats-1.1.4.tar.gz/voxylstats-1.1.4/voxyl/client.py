...
class Client:
    ...
    async def _retrieve_minecraft_player(
    ...
    async def _retrieve_voxyl_player_data(
        ...
        try:
            ...
            raise PlayerNotFound(
                ...
                f"Could not find voxyl data on '{player.name}'",
            )
    ...
    async def get_voxyl_player(
        ...
        try:
            ...
            raise PlayerNotFound(
                ...
                f"Could not find voxyl data on '{minecraft_player.name}'",
            )
    ...
    async def get_voxyl_player_guild(
        ...
        try:
            ...
            raise PlayerNotFound(
                "player",
                minecraft_player.formattedUUID,
                f"Guild not found for '{minecraft_player.name}'",
            )
    ...
    async def get_guild_from_player(
        self, player: Union[str, MinecraftPlayer, VoxylPlayer]
    ) -> Guild:
    ...
    async def get_daily_wins_leaderboard(self, game: str) -> PeriodicLeaderboard:
        lb_info = await self._get(
            "leaderboard_game",
            path_params=[game],
            query_params={"type": "wins", "period": "daily"},
        )

        return PeriodicLeaderboard(
            _data=lb_info["players"], game=game, type="wins", period="daily"
        )
    ...
    async def get_daily_winstreaks_leaderboard(self, game: str) -> PeriodicLeaderboard:
        lb_info = await self._get(
            "leaderboard_game",
            path_params=[game],
            query_params={"type": "winstreaks", "period": "daily"},
        )

        return PeriodicLeaderboard(
            _data=lb_info["players"], game=game, type="winstreaks", period="daily"
        )
    ...
    async def get_weekly_wins_leaderboard(self, game: str) -> PeriodicLeaderboard:
        lb_info = await self._get(
            "leaderboard_game",
            path_params=[game],
            query_params={"type": "wins", "period": "weekly"},
        )

        return PeriodicLeaderboard(
            _data=lb_info["players"], game=game, type="wins", period="weekly"
        )
    ...
    async def get_weekly_winstreaks_leaderboard(self, game: str) -> PeriodicLeaderboard:
        lb_info = await self._get(
            "leaderboard_game",
            path_params=[game],
            query_params={"type": "winstreaks", "period": "weekly"},
        )

        return PeriodicLeaderboard(
            _data=lb_info["players"], game=game, type="winstreaks", period="weekly"
        )
