...
class DataPipeLine:
    ...
    def records_to_df(self, records, columns: list):
        dict_dataframe = {
            True: pl.DataFrame(records, orient='row', schema=columns),
            False: pd.DataFrame(records, columns=columns)
        }
        self.df = dict_dataframe[self.use_polars]

        # message
        message = f"Data Shape: ({self.df.shape[0]:,.0f}, {self.df.shape[1]})"
        if self.count_rows:
            message = f"Total rows: {self.df.to_numpy()[0][0]:,.0f}"
        return message
    ...
    def run_presto_to_df(
        ...
        text = self.records_to_df(records, columns)
        if save_path:
