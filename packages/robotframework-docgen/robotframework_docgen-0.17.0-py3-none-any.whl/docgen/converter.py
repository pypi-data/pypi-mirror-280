...
class RestHtmlConverter(RestConverter):
    ...
    def convert(self, libdoc: LibraryDoc, output: str, relative: bool):
        ...
        elif int(ROBOT_VERSION[0]) >= 5:
            from robot.libdocpkg.htmlutils import DocFormatter

            formatter = DocFormatter(
                libdoc.keywords, libdoc.type_docs, libdoc.doc, libdoc.doc_format
            )
        else:
            from robot.libdocpkg.htmlutils import DocFormatter
        ...
        if int(ROBOT_VERSION[0]) >= 5:
            for type_doc in libdoc.type_docs:
                type_doc.doc = self._raw_html(formatter.html(type_doc.doc))
        elif int(ROBOT_VERSION[0]) >= 4:
            for data_type in libdoc.data_types:
                data_type.doc = self._raw_html(formatter.html(data_type.doc))
...
class LibspecConverter(BaseConverter):
    ...
    def convert(self, libdoc: LibraryDoc, output: str, relative: bool):
        if relative:
            curdir = Path(".").resolve()
            libdoc.source = str(Path(libdoc.source).relative_to(curdir))
            for ini in libdoc.inits:
                ini.source = str(Path(ini.source).relative_to(curdir))
            for kw in libdoc.keywords:
                kw.source = str(Path(kw.source).relative_to(curdir))
