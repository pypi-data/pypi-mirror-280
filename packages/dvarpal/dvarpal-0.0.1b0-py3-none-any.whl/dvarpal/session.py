...
class SessionManager:
    ...
    """
    Job to maintain session. On every execution, this job checks if a valid access_token exists. It fetches the user
    profile to see if an existing token is still valid. If token is invalid, it fetches a new access token. This job
    should run frequently to ensure a valid session is always available.
    """
    ...
    def __init__(self, config: SessionConfig = None):
        ...
        self.__config = self._load_config_from_file() if config is None else config
        self.__access_token = ''
