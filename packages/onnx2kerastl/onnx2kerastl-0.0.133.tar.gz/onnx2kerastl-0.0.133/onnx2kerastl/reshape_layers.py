...
def convert_shape(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if not is_unknown_tensor and (
        not K.is_keras_tensor(input_0)
        or not any([input_0.shape[i] == None for i in range(len(input_0.shape))])
    ):
        shapes = []
...
def optimize_constant_array_for_serialization(
    params: tf.Tensor, indices: Union[np.ndarray, tf.Tensor], logger
):
    """
    ...
    logger.debug(
        "onnx2keras.gather - Encountered long gather. "
...
def convert_gather(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if (
        is_numpy(layers[node.input[0]])
        and is_numpy(layers[node.input[1]])
        and not "is_embedding" in params
    ):
        ...
        if isinstance(layers[node.input[1]], (np.integer, int)) or (
            not isinstance(layers[node.input[1]], np.ndarray)
            and K.is_keras_tensor(layers[node.input[1]])
        ):
        ...
        if "is_embedding" in params:
            if len(input_0.shape) == 2:
                ...
                raise AttributeError(
                    "Cannot transform gather into embedding with non 2D array"
                )
        else:
            if tf.is_tensor(indices) and indices.dtype not in [
                tf.int16,
                tf.int32,
                tf.int64,
            ]:
            ...
            if isinstance(input_0, np.ndarray) or not K.is_keras_tensor(input_0):
                if len(input_0) > OPTIMIZE_ARRAY_LENGTH:
                    input_0, indices = optimize_constant_array_for_serialization(
                        input_0, indices, logger
                    )
...
def convert_concat(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if all([is_numpy(layers[node.input[i]]) for i in range(len(node.input))]):
        ...
        if len(layer_input) > 1:
            if not np.array(
                [
                    tf.is_tensor(layer_input[i]) and K.is_keras_tensor(layer_input[i])
                ...
                try:
                    layers[node_name] = tf.concat(
                        layer_input, axis=params["axis"], name=keras_name
                    )
                ...
                layer_input = unsqueeze_tensors_of_rank_one(
                    layer_input, axis=params["axis"]
                )
        ...
        if is_numpy(input_0):
            ...
            if params["change_ordering"]:
                input_0 = ensure_tf_type(
                    layers[node.input[0]], name="%s_const" % keras_name
                )
                ...
                )  # keras reshape ignores batch dimension but onnx axis do not
                ...
                if len(input_0_shape) > 0:
                    first_mismatch = np.argmin(
                        np.array(input_0_shape[: len(input_1)]) == input_1
                    )
                else:  # does not need to reshape dynamicalyy (skip next section)
                    first_mismatch = 0
                if (
                    (input_1 == None).any()
                    and (np.array(input_0_shape) == None).any()
                    and len(input_1) < len(input_0_shape)
                    and input_1[first_mismatch] == -1
                ):  # reshape end
                    end_match_arr = np.array(input_0_shape[-len(input_1) :]) == input_1
                    end_idx_match = np.argmax(
                        (np.array(input_0_shape[-len(input_1) :]) == input_1)
                    )
                    end_idx_match = (
                        end_idx_match + len(input_0_shape) - len(input_1)
                        if end_idx_match > first_mismatch
                        and end_match_arr[end_idx_match]
                        else len(input_0_shape) + 1
                    )
                    tf_shape = tf.shape(input_0)
                    layers[node_name] = tf.reshape(
                        input_0,
                        [*tf_shape[:first_mismatch], -1, *tf_shape[end_idx_match:]],
                    )
                else:
                    logger.debug(
                    ...
                    if (
                        len(np.int32(input_1[1:])) == 1
                        and np.int32(input_1[1:])[0] == -1
                    ):
                        ...
                        flatten = keras.layers.Reshape(
                            target_shape=input_1[1:], name=keras_name
                        )
                        ...
                        if len(input_0.shape) == 0 or (
                            input_0.shape[0] != input_1[0] and input_1[0] != 0
                        ):  # keras reshape don't work
                            ...
                            if dims_to_set_as_zero is not None:
                                ...
                                new_shape[dims_to_keep_unchanged] = np.array(
                                    input_0.shape
                                )[dims_to_keep_unchanged]
                            layers[node_name] = tf.reshape(
                                input_0, new_shape, name=keras_name
                            )
                        else:
                            reshape = keras.layers.Reshape(
                                np.int32(input_1[1:]), name=keras_name
...
def convert_unsqueeze(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if len(np.unique(params["axes"])) < len(params["axes"]):
        raise AttributeError(
            f"The specified axes contains duplicates values: {params['axes']}"
        )
...
def convert_slice(node, params, layers, lambda_func, node_name, keras_name):
    ...
    try:
        ...
        if (
            layers[node.input[0]].shape == None
        ):  # tensor with unknown shape (not the same as dynamic)
            max_len = max(axes) + 1
            if any([axis < 0 for axis in axes]):
                raise NotImplementedError(
                    "For a tensor with unknown shape, can't use negative axis"
                )
            else:
                axes_positives = axes
        else:
            raise NotImplementedError(
                f"Couldn't transform the axis in a slice layer {node_name}"
            )
    slice_spec_param = []
    is_dynamic = False
    for i in range(len(starts)):
        for index_li in [starts, steps, ends]:
            if (
                index_li[i] is not None
                and not isinstance(index_li[i], int)
                and not is_numpy(index_li[i])
                and K.is_keras_tensor(index_li[i])
            ):
                is_dynamic = True
    if not is_dynamic:
        ...
        if (
            is_numpy(layers[node.input[0]])
            and np.array([_shape is None for _shape in layers[node.input[0]]]).any()
            and len(layers[node.input[0]].shape) == 1
        ):  # slice numpy array which is a shape
        ...
        for axis in range(max_len):
            if axis in axes_positives:
                axis_index = axes_positives.index(axis)
                for res_list, input_list in zip(
                    [start_vec, step_vec, end_vec], [starts, steps, ends]
                ):
                    slice_index = input_list[axis_index]
                    if (
                        input_list[axis_index] is not None
                        and not isinstance(slice_index, int)
                        and not is_numpy(input_list[axis_index])
                        and input_list[axis_index].dtype != tf.int32
                    ):
                        slice_index = tf.cast(slice_index, tf.int32)
                    res_list[axis] = slice_index
        layers[node_name] = tf.strided_slice(
            input_0,
...
def convert_resize(node, params, layers, lambda_func, node_name, keras_name):
    ...
    # Validate axes
    axes = [
        a if a >= 0 else a + rank for a in axes
    ]  # Convert negative axes to positive
    if any(
        a < 0 or a >= rank for a in axes
    ):  # check that all axes values are within input rank
    ...
    if len(scales) > 0:
        for i, axis in enumerate(axes):
            if scales[i] != 1:
                tf_resize_shapes[axis - 2] = tf.cast(
                    scales[i] * tf.cast(tf_resize_shapes[axis - 2], tf.float32),
                    tf.int32,
                )
    else:
        for i, axis in enumerate(axes):
    ...
    if (
        ...
        resized = tf.image.resize(
            to_channel_last, size=resize_size, method=resize_method
        )
