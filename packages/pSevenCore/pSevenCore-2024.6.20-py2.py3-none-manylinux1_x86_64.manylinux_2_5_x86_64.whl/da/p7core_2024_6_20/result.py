...
class Result(object):
  ...
  def _copy_solutions_to_designs(self, problem, options):
    if not self._designs_data.size or ("stochastic" in self._fields) != ("stochastic" in self._designs_fields[2]):
      return

    designs_dim, solutions_dim = self._designs_data.shape[1], self._solutions_data.shape[1]
    solution_of_design = _numpy.empty(designs_dim, dtype=int) # -1 or index of corresponding solution column
    solution_of_design.fill(-1)

    design_fields = self._designs_fields[2]
    for k in self._fields:
      if k in design_fields:
        solution_of_design[design_fields[k]] = range(*self._fields[k].indices(solutions_dim))

    solution_of_design_mask = solution_of_design >= 0

    undefined_designs = _shared._find_holes(self._designs_data)
    undefined_designs = _numpy.logical_and(undefined_designs, solution_of_design_mask.reshape(1, -1))
    if not undefined_designs.any():
      return self

    basic_fields, extra_fields, fields_spec = self._designs_fields

    # In general case, the same design can be updated multiple times. (No, because the solution is unique.)
    # Holes search is only efficient in batch mode, so we mark updated designs and re-search for missing data only when we need it.
    reevaluate_design_mask = _numpy.zeros((undefined_designs.shape[0],), dtype=bool)

    with _make_writeable(self._designs_data):
      from .utils.designs import _input_sample_data

      solution_keys = _input_sample_data(self._fields, self._solutions_data)
      designs_keys = _input_sample_data(design_fields, self._designs_data)

      for design_idx, solution_idx in _shared._enumerate_equal_keys(designs_keys, solution_keys):
        design_mask, design, solution = undefined_designs[design_idx], self._designs_data[design_idx], self._solutions_data[solution_idx]

        for i, j in zip(self._payload_designs, self._payload_solutions):
          design[i] = self._payload_storage.join_encoded_payloads(design[i], solution[j])

        if reevaluate_design_mask[design_idx]:
          design_mask[:] = _numpy.logical_and(_shared._find_holes(design), design_mask)

        design[design_mask] = solution[solution_of_design[design_mask]]
        reevaluate_design_mask[design_idx] = True
  ...
  def _mark_undefined_designs(self, problem, auto_objective_type):
    ...
    if undefined_objectives.any():
      for kind, undefined in zip(_read_objectives_type(problem, (auto_objective_type or "minimize").lower()), undefined_objectives.T):
        if kind not in ("evaluate", "payload",) and undefined.any():
    ...
    undefined_constraints = _shared._find_holes(self._designs_data[:, self._designs_fields[2].get("c", slice(0))]).any(axis=1)
    if undefined_constraints.any():
      designs_flags[_numpy.logical_and(designs_flags == GT_SOLUTION_TYPE_INFEASIBLE, undefined_constraints)] = GT_SOLUTION_TYPE_INFEASIBLE_NAN
      designs_flags[_numpy.logical_and(designs_flags == GT_SOLUTION_TYPE_CONVERGED, undefined_constraints)] = GT_SOLUTION_TYPE_FEASIBLE_NAN
  ...
  def _postprocess_solution(self, problem, auto_objective_type, options, logger, intermediate_result=False):
    ...
    # The absence of the value of any objective specified in the `required_responses`` list marks the entire solution as missing
    if not auto_objective_type:
      # This is an optimization problem
      required_responses = [i for i, k in enumerate(objectives_type) if k in ("minimize", "maximize")]
    elif auto_objective_type == 'adaptive':
      # This is the Adaptive Design problem
      required_responses = [i for i, k in enumerate(objectives_type) if k in ("minimize", "maximize", "adaptive")]
    else:
      # This is either a space-filling problem and all objectives are optional,
      # or GFO minimization problem and we use Pareto optimal objectives as a required ones.
      required_responses = []
    ...
    if auto_objective_type:
      ...
      if pareto_optimal_objectives:
        required_responses = [i for (i, k) in pareto_optimal_objectives] # hack
    ...
    if responses_evaluation_allowed:
      try:
        ...
        if pareto_optimal_objectives and not evaluation_mask.all():
          ...
          if options.get("GTOpt/OptimalSetType", "").lower() == "strict":
            if not auto_objective_type or auto_objective_type == "minimize":
    ...
    if required_responses and objectives_values.size:
      ...
      if len(required_responses) < len(objectives_type):
        objectives_values = objectives_values[:, required_responses]
  ...
  def pprint(self, fields=None, filter_type=None, file=_sys.stdout, precision=8, designs=False, limit=None):
    ...
    if "flag" in regular_fields:
      n_points = len(fields_collection[0])
      initial_points = _numpy.zeros(n_points, dtype=bool)
      potentially_optimal = _numpy.zeros(n_points, dtype=bool)
    ...
    named_flags = {
      ...
      GT_SOLUTION_TYPE_INFEASIBLE_NAN: "infeasible, undefined",
      GT_SOLUTION_TYPE_BLACKBOX_NAN:   "infeasible",
      GT_SOLUTION_TYPE_NOT_EVALUATED:  "undefined",
      GT_SOLUTION_TYPE_FEASIBLE_NAN:   "feasible, undefined",
