...
class Solver(object):
    ...
    try:
      ...
      if self._test_categorical_only_problem(problem, doe_mode):
        ...
        with _shared._suppress_history(problem):
          result = self._build_full_factorial(problem=problem, mode=mode, sample_x=sample_x, sample_f=sample_f, sample_c=sample_c, compatibility=compatibility)
  ...
  def _build_full_factorial(self, problem, mode, sample_x, sample_f, sample_c, compatibility, factor_types=("categorical",)):
    ...
    factor_types_str = ", ".join(factor_types)
    warn_message = "All problem variables are %s or have a fixed value. The solution will be selected from a full factorial design." % factor_types_str
    ...
    solutions_subsets = {"new": slice(0, len(designs[0])),
