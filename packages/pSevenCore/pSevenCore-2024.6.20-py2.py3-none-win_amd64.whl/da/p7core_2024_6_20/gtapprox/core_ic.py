...
class _MultiRoute(object):
  ...
  def extended_copy(self, n_parts, catvars, builder, min_n_parts=3):
    ...
    n_points_dx = (max_n_parts - 2) / max(self.dx_nrm2.ptp(), np.finfo(float).eps)
...
def _calc_angle(x, triples):
  x1 = x[triples[:, 1]]
  v1 = np.array(x1 - x[triples[:, 0]], dtype=float, copy=False)
  v2 = np.array(x1 - x[triples[:, 2]], dtype=float, copy=False)
