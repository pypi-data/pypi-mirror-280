...
def as_matrix(array, shape=(None, None), dtype=float, ret_is_vector=False, order=None, detect_none=False, name=None, copy=False):
  ...
  try:
    array = _numpy.array(array, dtype=dtype, order=order, copy=copy)
  except:
  ...
  if options is not None:
    ...
    for i, (dtype, labels, initial_enumerators) in _six.iteritems(categorical_x_map):
      template.setdefault(key_x, [{} for _ in range(_n_columns(x))])[i].update({
        ...
        'labels': labels,
        'enumerators': labels if _numpy.issubdtype(dtype, _numpy.float_) else numerical_encoding(labels, initial_enumerators),
      })
...
def get_labels_type(labels, direction, variable_index, validate):
  try:
    ...
    if validate:
      for label in labels:
        if not any(_numpy.issubdtype(type(label), _) for _ in (_numpy.number, _numpy.bool_, _numpy.str_, _numpy.unicode_)):
...
def _encode_categorical_variable(dtype, labels, enumerators, values_in, values_out=None, forbidden_labels=None, logger=None, variable_name=None):
  ...
  values_in = _numpy.array(values_in, dtype=dtype, copy=False, ndmin=1)
...
def decode_categorical_values(sample, categorical_variables_map, inplace=True):
  ...
  sample = _numpy.array(sample, ndmin=2, copy=False)
  decoded_dtype = _numpy.result_type(*(categorical_variables_map[i][0] for i in categorical_variables_map))
  decoded_sample = _numpy.array(sample, copy=False, dtype=decoded_dtype) # no copy op if decoded_dtype equals to sample.dtype
  ...
  for i, (dtype, labels, enumerators) in _six.iteritems(categorical_variables_map):
    labels = _numpy.array(labels, dtype=dtype, copy=False)
    enumerators = _numpy.array(enumerators, dtype=float, copy=False)

    mapping_mask = _numpy.not_equal(enumerators, labels)
...
def _find_holes(matrix_data):
  try:
    matrix_data = _numpy.array(matrix_data, copy=False)
...
def convert_to_1d_array(array, name='array', nan=_NONE):
  ...
  try:
    ...
    try:
      original_array = array
      array = _numpy.array(array, dtype=_numpy.float64, copy=False)
...
def _scalar(value):
  return _numpy.array(value, copy=False).reshape(1)[0]
