...
def corrected_meta(output: Selected | Extracted) -> ImageMeta:
  match output.tag:
    case 'selected':
...
class Runner:
  ...
  async def delete_blobs(self, id: str, output: Extracted | Selected):
    keys = (await self.blobs.keys()).unsafe()
...
class Artifact(Protocol):
  def __call__(self, *, blobs: KV[bytes], logger: Logger) -> Coroutine:
    ...

class Preoutput(Task[Input, Output, Artifact]):
  ...
  def run(self, queues: Task.Queues[Input, Output]) -> Artifact:
    def bound(*, blobs: KV[bytes], logger: Logger):
      return Runner(blobs=blobs, logger=logger, **queues)()
    return bound
