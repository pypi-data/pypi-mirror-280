from typing import Any, Protocol, Coroutine
from dataclasses import dataclass
import asyncio
from uuid import uuid4
from haskellian import funcs as F, either as E, Either
...
class Runner:
  ...
  async def extract(self, id: str, task: Input): 
    ...
    corr_mat, cnts = re.extract(mat, model).unsafe()
...
class Artifact(Protocol):
  def __call__(self, *, logger: Logger, blobs: KV[bytes]) -> Coroutine[None, None, None]:
    ...

class Extract(Task[Input, Output, Artifact]):
  ...
  def run(self, queues: Queues):
    def bound(*, logger: Logger, blobs: KV[bytes]):
      return Runner(logger=logger, blobs=blobs, **queues)()
