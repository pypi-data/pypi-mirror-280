...
from botrun_ask_folder.split_txts import get_page_number_prefix, get_page_number


def create_qdrant_client(qdrant_host, qdrant_port):
...
def build_field_conditions(file_path_contains, page, keywords):
    must_conditions = [
        models.FieldCondition(
            key="file_path",
            match=models.MatchText(text=file_path_contains)
        ),
        models.FieldCondition(
            key="file_path",
...
def query_page_range_with_qdrant_client(qdrant_client,
                                        collection_name,
                                        file_path_contains,
                                        start_page,
                                        end_page,
    ...
    # 取得所有結果，並根據頁碼進行排序
    sorted_results = sorted(results[0], key=lambda x: get_page_number(x.payload["file_path"]))

    print("文件總頁數: ", get_max_page_number(qdrant_client, collection_name, file_path_contains))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--collection_name", type=str, default='1_a3kY_i54kQzB-jCf8BZDTJlMKSJ_21A')
    ...
    qdrant_client = create_qdrant_client(qdrant_host=args.qdrant_host, qdrant_port=args.qdrant_port)
