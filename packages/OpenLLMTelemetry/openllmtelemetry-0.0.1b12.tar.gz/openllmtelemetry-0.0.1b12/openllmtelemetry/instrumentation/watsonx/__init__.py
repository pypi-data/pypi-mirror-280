...
def _wrap(
    ...
    def blocked_message_factory(eval_result: Optional[EvaluationResult] = None, is_prompt=True):
        if is_prompt:
            blocked_message = "Prompt blocked by WhyLabs Secure"
        else:
            blocked_message = "Response blocked by WhyLabs Secure"

        if eval_result.action.block_message:
            blocked_message += f"{blocked_message}: {eval_result.action.block_message}"
        return {
            "model_id": "whylabs/guardrails",
            "created_at": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            "results": [
                {
                    "generated_text": blocked_message,
