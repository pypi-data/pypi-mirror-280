...
def get_share_url(share_msg: str) -> str | None:
    ...
    """
    url_re_search = re.search(r'(https://pan\.baidu\.com/s/[\w-]*)', share_msg)
    if url_re_search is None:
        url_re_search = re.search(r'(https://pan\.baidu\.com/share/[/?=&\w-]*)', share_msg)
    if url_re_search is None:
        return
    share_url = url_re_search.group(1)
    if 'pwd=' in share_url:
        return share_url
    pwd_re_search = re.search(r'提取码:(\w{4})', share_msg)
    if pwd_re_search:
