...
from afeng_tools.fastapi_tool.template.template_render_service.template_render_tools import _render_template_file
...
def get_template_top_bar(group_code: str, context_data_func: Callable[[], TemplateTopBarData],
                         app_code: str = 'common', reload_freq: ReloadFreqEnum = ReloadFreqEnum.always,
                         is_mobile: bool = True,
                         template_file: str = None) -> tuple[str, str]:
    """
    页面顶部topbar模板
...
def get_template_logo_search(group_code: str, context_data_func: Callable[[], TemplateLogoSearchData],
                             app_code: str = 'common', reload_freq: ReloadFreqEnum = ReloadFreqEnum.always,
                             is_mobile: bool = True, template_file: str = None) -> tuple[str, str]:
    """
    页面顶部logo_search模板
    :param is_mobile:
    :param app_code:
...
def get_template_page_header(group_code: str, context_data_func: Callable[[], TemplatePageHeaderData],
    ...
    def context_data_func_wrapper():
        ...
        context_data = {
            ...
            'template_top_bar': get_template_top_bar(app_code=app_code, group_code=group_code,
                                                     context_data_func=lambda: data.topbar_data,
                                                     reload_freq=reload_freq, is_mobile=is_mobile),
            'template_logo_search': get_template_logo_search(app_code=app_code, group_code=group_code,
                                                             context_data_func=lambda: data.logo_search_data,
                                                             reload_freq=reload_freq, is_mobile=is_mobile),
            'template_fix_nav': get_template_fix_nav(app_code=app_code, group_code=group_code,
