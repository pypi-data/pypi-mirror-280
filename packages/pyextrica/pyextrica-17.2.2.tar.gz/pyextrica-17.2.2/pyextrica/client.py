...
class TrinoQuery(object):
    ...
    def execute(self, additional_http_headers=None) -> TrinoResult:
        ...
        This is the HTTP request sent to the coordinator.
    ...
    def split_query(self, actualQuery):
        pattern = re.compile(r'(Prepare .*?FROM)(.*SELECT.*)', re.IGNORECASE | re.DOTALL)
        
        match = pattern.match(actualQuery)
        ...
        if match:
            ...
            return None, actualQuery
    
    def is_show_query(self, query):
        show_pattern = re.compile(r'\bSHOW\b', re.IGNORECASE)
        return bool(show_pattern.search(query))
    ...
    def fetch(self) -> List[List[Any]]:
        ...
        try:
            ...
            raise pyextrica.exceptions.TrinoConnectionError("failed to fetch: {}".format(e))
        status = self._request.process(response)
        self._update_state(status)
        logger.debug(status)
        if status.next_uri is None:
    ...
    def cancel(self) -> None:
        ...
        try:
            ...
            raise pyextrica.exceptions.TrinoConnectionError("failed to cancel query: {}".format(e))
...
class RowMapper:
    ...
    def _map_value(self, value, value_mapper: ValueMapper[T]) -> Optional[T]:
        try:
            ...
            raise pyextrica.exceptions.TrinoDataError(error_str) from e
