...
class custom_logger():
    ...
    def get_(command):
        ...
        if result.returncode == 0:
            return generate_outcome_message("success",result.stdout)
        else:
    ...
    def initialise_database(self):
        ...
        try:
            ...
            self.logging_table_name = table_name
            ...
            return generate_outcome_message("success",f"{e}",the_type="others")
        except Error as e:
            self.change_storage_location()
            return generate_outcome_message("success",f"\n\n ---Error message---\n\n{e.pgerror}\n\nSwitching storage to {self.storage_location}...",the_type="others")
    ...
    def store_log(self):
        ...
        if self.storage_location == "database":
            ...
            if outcome["outcome"] == "error" and outcome["output"]["status"] == True:
                outcome2 = self.store_log_in_notion(stringing)
    ...
    def store_log_in_notion(self,log_data):
        if self.client == None:
            return generate_outcome_message("error","no database client initialised...",the_type="custom")
        ...
        # How do I determine if there is a block within?

        # Get the first block from the 
