...
class ProfilesLocal(GenericCRUD, metaclass=MetaLogger, object=PROFILE_LOCAL_PYTHON_LOGGER_CODE):
    ...
    def insert(self, *, profile_dict: Dict[str, any], main_person_id: int) -> int:  # noqa
        ...
        valid_columns = ['profile_id', 'number', 'identifier', 'profile.name', 'title', 'title_approved',
                         'profile.main_user_id', 'main_person_id', 'is_main',
                         'visibility_id', 'is_approved', 'profile_type_id', 'preferred_lang_code',
                         'experience_years_min', 'main_phone_id', 'profile.main_email_address', 'is_rip', 'profile.gender_id',
                         'stars', 'last_dialog_workflow_state_id', 'is_system', 'internal_description', 'is_test_data',
                         'is_business_profile']
        profile_dict = {k: v for k, v in profile_dict.items() if k in valid_columns}
        profile_dict_ml = profile_dict.copy()
        if 'profile.name' not in profile_dict:
