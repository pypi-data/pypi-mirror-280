...
class Trades:
    ...
    def generate(
        ...
        trades = pd.DataFrame(
            {
                ENTRY: arr_data[:, 0].astype(int),
                EXIT: arr_data[:, 1].astype(int),
                TYPE: arr_data[:, 2].astype(str)
            }
        )
...
class Plot:
    ...
    def returns_signals(returns: pd.Series, index: np.ndarray | pd.Series = None) -> None:
        ...
        try:
            ...
            with np.testing.suppress_warnings() as sup:
                ...
                curve = np.poly1d(np.polyfit(x, returns_pct, min(len(x), 7)))(x)
    ...
    def returns_pie(returns: pd.Series) -> None:
        ...
        ax1.pie(
            returns_counts, explode=([0, 0.06] if len(returns_counts) == 2 else [0]),
            startangle=90, autopct='%1.2f%%'
        )
        ax1.legend(
            labels=[
                f'Wins {returns_counts.get(True, 0)}',
                f'Losses {returns_counts.get(False, 0)}'
            ]
        )
        ...
        returns_sizes = returns_sizes[returns_sizes != 0.0]
        ax2.pie(
            returns_sizes, explode=([0, 0.1] if len(returns_sizes) == 2 else [0]),
            startangle=90, autopct='%1.2f%%'
        )
        plt.show()
    ...
    def price_signals(
        ...
        plt.plot(
            ...
            label=f'bid {bid.iloc[-1] if len(bid) > 0 else np.nan}',
        )
        plt.plot(
            index[ask.index], ask, lw=1, alpha=0.875,
            label=f'ask {ask.iloc[-1] if len(ask) > 0 else np.nan}',
        )
        plt.plot(
            index[ask.index], mid, lw=0.75,  alpha=0.75,
            label=f'mid {round(mid.iloc[-1], 5) if len(mid) > 0 else np.nan}'
        )
