...
import glob
import json
# from . import AnalysisGUI, IO_ResourceGUI, GridLayout, DisplayGUI, Analysis
import os
import sys
...
class ImageReader(object):
    ...
    """
    def __init__(self, ControlPanel=None, inout_resource_gui=None, displaygui=None, analysisgui=None):
        ...
        if not (self.inout_resource_gui is None):
    ...
    def READ_FROM_METADATA(self, metadatafilename):
        ...
        Meta_Data_df = pd.DataFrame(rows, columns = df_cols)
        ...
        for i in range(items_mrf.length):
            ...
            if not (self.displaygui is None):
